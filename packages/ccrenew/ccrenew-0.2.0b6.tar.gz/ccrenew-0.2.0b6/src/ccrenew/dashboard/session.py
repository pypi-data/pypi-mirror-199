...
class DashboardSession:
    ...
    def process_project(self, project_name:str|Project|list, reprocess:bool = False,
                        datasub:bool = False, use_solcast:bool = True, **kwargs) -> Project:
        """
        ...
        if isinstance(project_name, list):
            ...
            self.process_projects(project_list=project_list, method='series',
                                  reprocess=reprocess, datasub=datasub, use_solcast=use_solcast)
            ...
            try:
                project._process_data(neighbor_sensor_data=neighbor_sensor_data,
                                      datasub=datasub, use_solcast=use_solcast)
            except Exception:
                print('\n')
                print('###########################')
    ...
    def process_projects(self, project_list:str|Project|list, method:str = 'series',
                         reprocess:bool = False, datasub:bool = False, use_solcast:bool = True):
        ...
        if isinstance(project_list, (str, Project)):
            ...
            project = self.process_project(project_name, reprocess, datasub=datasub, use_solcast=use_solcast)
    ...
    def process_all_projects(self, method:str = 'parallel', reprocess:bool = True,
                             datasub:bool = False, use_solcast:bool = True):
        """
        ...
        self.process_projects(project_list, method=method, reprocess=reprocess, datasub=datasub, use_solcast=use_solcast)
    

    def export_project(self, project_name:str|list, server:str = 'dev', dest:str|list = 'all',
                       export_type:str = 'monthly', save_pickle:bool = True, **kwargs) -> None:
        """
        ...
        Args:
            ...
            export_type (str): Option to select which export type to run. Options are `monthly`, `ul`, and `weekly`.
            save_pickle (bool): Option to save pickle after exporting.
            **kwargs: Optional arguments passed to [process_project()][ccrenew.dashboard.project.Project.process_project].
        ...
        if isinstance(project_name, list):
            ...
            if dest != 'bool':
                ...
                if server == 'datasub':
                    datasub = True
                self.process_project(project_name, datasub=datasub, **kwargs)

            func_args.pop('project_name')
            project._export(**func_args)
            print(f"{project.project_name} successfully exported.")
        else:
            raise TypeError("`project_name` must be a string or Project object! Please reformat input and try again.")

  
    def export_projects(self, project_list:list|str, server:str = 'dev', dest:str|list = 'all',
                        expot_type:str = 'monthly', save_pickle:bool = True, method:str = 'series', **kwargs) -> None:
