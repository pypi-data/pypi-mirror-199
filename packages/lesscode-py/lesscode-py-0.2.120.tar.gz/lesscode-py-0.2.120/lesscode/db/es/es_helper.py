...
class EsHelper:
    ...
    def sync_send_es_post(self, bool_must_list=None, param_list=None, route_key="", sort_list=None, size=10,
        ...
        try:
            if options.correction_param.get("enable"):
                self.correction_param(params, route_key)
        except:
    ...
    def correction_param(self, param, route_key):
        for k in param:
            if k in ["terms", "term", "range", "wildcard"]:
                param[k] = self.correction_field(route_key, param[k], keyword_flag=True)
            elif k in ["match", "match_phrase"]:
                param[k] = self.correction_field(route_key, param[k])
            else:
                if isinstance(param[k], dict):
                    self.correction_param(param[k], route_key)
                elif isinstance(param[k], list):
                    for i in param[k]:
                        self.correction_param(i, route_key)

    def correction_field(self, route_key, value, keyword_flag=False):
        column = list(value.keys())[0]
        data_value = value[column]
        column = column.replace(".keyword", "")
        es_mapping_value = self.get_es_filed_mapping(route_key, column)
        if es_mapping_value:
    ...
    def get_es_filed_mapping(self, route_key, field):
        res = self.pool.es_selector_way(url_func_str=self.pool.format_es_field_mapping_url,
                                        param_dict={"field": field, "route_key": route_key},
                                        request_way=EsRequest.format_es_get).get(route_key, {}).get("mappings",
                                                                                                    {}).get(
            field, {}).get("mapping", {}).get(field.split(".")[-1], {})
        if res.get("fields"):
            result = {
                res["type"]: 1,
