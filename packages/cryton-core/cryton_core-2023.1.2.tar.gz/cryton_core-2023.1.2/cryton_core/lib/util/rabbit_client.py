...
from cryton_core.lib.util import constants, exceptions, logger
...
class RpcClient:
    def __init__(self, channel: amqpstorm.Channel = None):
        ...
        self.timeout = config.DEFAULT_RPC_TIMEOUT
    ...
    def open(self) -> None:
        ...
        if self.channel is None:  # Create new connection and channel if not given
            ...
            self.connection = amqpstorm.Connection(config.RABBIT_HOST, config.RABBIT_USERNAME, config.RABBIT_PASSWORD,
                                                   config.RABBIT_PORT)
