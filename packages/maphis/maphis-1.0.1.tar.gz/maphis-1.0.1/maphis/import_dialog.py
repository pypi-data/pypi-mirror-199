...
from PySide6.QtGui import QColor, QPixmap, QRegularExpressionValidator, QIcon
...
from maphis.common.units import Value, UnitStore
...
class ImportDialog(QDialog):
    ...
    def handle_double_click_on_table(self, index: QModelIndex):
        if index.column() == COL_SRC_SIZE or index.column() == COL_DST_SIZE:
            ...
            self.image_list_model2.initialize(self._state.storage.image_paths, self.thumbnail_storage, 0, self._state.storage)
            ...
            photo = self._state.image_list_model.fetch_photo(index.row())
