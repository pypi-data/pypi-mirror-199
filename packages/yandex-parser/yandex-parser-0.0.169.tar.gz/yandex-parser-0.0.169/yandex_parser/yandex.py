...
class YandexParser(object):
    ...
    def get_current_search_query(self):
        ...
        match = re.search(
            r'<a class="[^"]+?(?:pager__item_kind_page|Pager-Item)[^"]*?"[^>]+?href="[^"]+?\?text=([^&]+)&',
            self.content
        )
        if match:
            encoded_query = match.group(1).encode('utf8')
            query = urllib.unquote_plus(encoded_query)
            query = query.replace('&nbsp;', ' ')
            return query

        match = re.search(
            r'<input class="[^"]*?mini-suggest__input[^"]*?"[^>]+?value="([^"]*)"',
            self.content
        )
        if match:
    ...
    def get_current_page(self):
        ...
        match = re.search(
            r'<span class="[^"]+?Pager-Item_current[^"]*?"[^>]*?>\s*(\d+)\s*</span>',
            self.content
        )
        if match:
            return int(match.group(1))
    ...
    def is_yandex(cls, content):
        return \
            '<meta property="og:site_name" content="Яндекс">' in content \
            or '<a class="logo__link" href="//www.yandex.' in content \
    ...
    def _ignore_block(self, sn):
        ...
        if 'z-' in sn.attrib['class'] \
            ...
            or 'RsyaGuarantee' in sn.attrib['class']:
            #реклама
            return True
        ...
        # Нашлось результатов попало в блок serp-item
        if 'serp-adv__found' in html:
            return True
    ...
    def get_captcha_data(self):
        ...
        if re.search(ur'CheckboxCaptcha-Button', self.content):
            ...
            patterns = [
                ...
                re.compile(u'<div class="AdvancedCaptcha-View"><img src=\"([^\"]+)\"'),
            ]
