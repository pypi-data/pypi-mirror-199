...
class AuthorizationEndpoint(AuthorizationServerEndpoint):
    ...
    async def get(
        ...
        email = params.email or self.session.get('email')
        ...
        # the login endpoint.
        if self.session.claims.email is None:
            return await self.on_login_required(params)
    ...
    async def on_downstream_required(
        ...
        prompt_url = settings.OAUTH2_DOWNSTREAM_PROMPT_URL + '?' + urllib.parse.urlencode(q)
        response = fastapi.responses.RedirectResponse(
            status_code=303,
            url=prompt_url
        )
        ...
        response.set_cookie(key='oauth2.return_url', value=prompt_url)
        return response
