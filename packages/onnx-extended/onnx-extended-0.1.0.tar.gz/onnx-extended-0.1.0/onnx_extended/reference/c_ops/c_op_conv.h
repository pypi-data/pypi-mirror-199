...
void ConvPoolCommonShape::compute_kernel_shape(
  ...
  if (kernel_shape_.size() > 0) {
    ...
    kernel_shape =
        std::vector<int64_t>(weight_dims.begin() + 2, weight_dims.end());
  }
}
...
void ConvPoolCommonShape::infer_output_shape(
    ...
    std::vector<int64_t> &output_shape, bool ForceSymmetricAutoPadding) const {
  conv_infer_output_shape(input_shape, kernel_shape, strides_p, dilations_p,
                          pads_p, output_shape, ForceSymmetricAutoPadding,
                          auto_pad_);
}

void ConvPoolCommon::init(const std::string &auto_pad,
                          py_array_t<int64_t> dilations, int64_t group,
                          py_array_t<int64_t> kernel_shape,
...
void ConvPoolCommon::initcpp(const std::string &auto_pad,
...
public:
  ...
  py::array_t<T>
  compute(py::array_t<T, py::array::c_style | py::array::forcecast> X,
...
py::array_t<T> Conv<T>::compute(
  ...
  py::array_t<T, py::array::c_style | py::array::forcecast> Y(y_dims);
  {
    ...
    compute_gil_free(X, W, B, Y, input_shape, output_shape, kernel_shape, pads,
                     dilations, strides, x_dims, y_dims, w_dims);
  }
  return Y;
}
...
void Conv<T>::compute_gil_free(
  ...
  for (int image_id = 0; image_id < N; ++image_id) {
    for (int group_id = 0; group_id < group_; ++group_id) {
      if (kernel_rank == 2) {
        Im2col_NCHW<T>(Xdata + group_id * X_offset, C / group_, input_shape[0],
                       input_shape[1], kernel_shape[0], kernel_shape[1],
                       dilations[0], dilations[1], pads[0], pads[1], pads[2],
                       pads[3], strides[0], strides[1], col_buffer_data);
      } else {
        Im2colNd_NCHW<T>(
            Xdata + group_id * X_offset, &image_shape[0],
            col_buffer_shape.data(), C * input_image_size, col_buffer_size,
            &kernel_shape[0], strides.data(), &dilations[0], &pads[0],
