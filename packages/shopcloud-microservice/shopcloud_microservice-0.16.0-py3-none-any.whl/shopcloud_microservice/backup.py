...
def export_database(
    config: Config, project: str, instance_id: str, db: str, simulate: bool
) -> int:
    now = datetime.now()
    bucket_name = f"{instance_id.lower()}-db-exports"
    backup_name = f"{now.strftime('%Y-%m-%d')}/{project}-{instance_id}-{db}.sql"
...
def cli_main(args, config) -> int:
    if args.action == "sql-init":
        ...
        # liste instanzen mit liste der dbs
        def run_export(config, instance, is_simulate=False):
            databases = [
                {
                    'name': x,
                    'status': export_database(config, instance.get('project'), instance.get('instance_id'), x, is_simulate)
                }
        ...
        datas = Parallel(backend="threading" ,n_jobs=-1)(delayed(run_export)(config, x, args.simulate) for x in datas)
