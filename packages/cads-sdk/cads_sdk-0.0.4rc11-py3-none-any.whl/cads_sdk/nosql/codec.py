        ...
        for row in partitionData:
            ...
            if self.resize_mode == 'padding':
                row_dict = {
                    ...
                    'image': self.padding(img, (self.s[0], self.s[1]))
                }
    ...
    def create_dataframe(self, 
        ...
        with materialize_dataset(spark, output_path, ImageSchema, ROWGROUP_SIZE_MB):
            if self.resize_mode:
                ...
                self.write_to_path(
                            spark_df = spark.createDataFrame(spark_df.rdd.mapPartitions(self.row_generator), ImageSchema.as_spark_schema()),
...
class ConvertFromFolderVideo:
    ...
    def create_dataframe(self, spark, VideoSchema, input_files,
        ...
        spark_df = spark.createDataFrame(pd.DataFrame(input_files, columns=['path']))
        self.unischema = VideoSchema
        self.write_to_path(
                    spark_df = spark.createDataFrame(spark_df.rdd.mapPartitions(self.row_generator), VideoSchema.as_spark_schema()),
                    output_path = output_path,
                    table_name = table_name, 
                    database = database,
                    numPartition = numPartition,
