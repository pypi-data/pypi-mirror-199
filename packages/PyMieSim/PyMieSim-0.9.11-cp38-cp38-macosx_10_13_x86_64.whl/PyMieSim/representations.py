...
from MPSPlots.Math import spherical_to_cartesian, rotate_on_x
...
class Stokes():
    ...
    def plot(self, show_ource=True, show_axes=True):
        ...
        coordinates = spherical_to_cartesian(r=phi_mesh * 0 + 0.5, phi=phi_mesh, theta=theta_mesh)

        figure = Scene3D(shape=(1, 4), window_size=[2500, 800])

        for plot_number, field, name in zip([(0, 0), (0, 1), (0, 2), (0, 3)],
                                     [self.I, self.Q, self.U, self.V],
                                     ['I', 'Q', 'U', 'V']):
            figure.add_mesh(plot_number=plot_number,
            ...
            figure.__add__text__(plot_number=plot_number, text=f'{name} field')
...
class SPF():
    ...
    def plot(self, show_source=True, show_axes=True):
        ...
        coordinates = spherical_to_cartesian(r=scalar_mesh, phi=phi_mesh, theta=theta_mesh)
        ...
        figure.__add_axes__(plot_number=plot_number)
        figure.__add__text__(plot_number=plot_number, text='Scattering phase function')
...
class S1S2():
    ...
    def plot(self):
        ...
        S1_Ax.add_artist(S1_artist)
        S2_Ax.add_artist(S2_artist)

        Figure.add_axes(S1_Ax, S2_Ax)
