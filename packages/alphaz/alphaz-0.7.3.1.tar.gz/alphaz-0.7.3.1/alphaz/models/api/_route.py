    ...
    def get_return(self, forceData=False, return_status=None):
        ...
        if "pagination" in format_ and is_pagination:
            headers["X-pagination"] = {
                ...
                "next_page": (page + 1 if page < total_pages else total_pages) if (total_pages is not None and page is not None) else None,
                "per_page": per_page,
            }

        for key, value in headers.items():
            returned.headers[key] = json.dumps([str(v) for v in value.split("\n")]) if "\n" in str(value) else value
