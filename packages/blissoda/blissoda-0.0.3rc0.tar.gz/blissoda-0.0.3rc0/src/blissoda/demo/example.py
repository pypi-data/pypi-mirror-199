...
def example_job(redis=False):
    ...
    user_print("Ewoks events:", events_url)
    ...
    user_print(f"Wait for job {job_id} to finished ...")
    workflow_results = future.get(timeout=3, interval=0.1)
    user_print("Results:", workflow_results)
    ...
    try:
        ...
        user_print(f"Cannot read ewoks event URL's: {e}")
    else:
