"""Automatic pyfai integration for every scan (when enabled)"""
...
class XrpdProcessor(
    ...
    def get_configuration(self, scan):
        return {
            ...
            "data": json.dumps(self.get_workflow()),
            "options": json.dumps(self.get_options(scan)),
        }

    def get_options(self, scan):
        return {"inputs": self.get_inputs(scan)}
