...
def ValidString():
    def f(str_data):
        string_valid(str_data, msg=None)
        return str_data
    return f

def ValidInt():
    def f(int_data):
        int_valid(int_data, msg=None)
        return int_data
    return f

def ValidAmount():
    def f(float_data):
        amount_valid(float_data, msg=None)
        return float_data
    return f

def ValidBoolean():
    def f(bool_data):
        boolean_valid(bool_data, msg=None)
...
def ValidDateTime():
    ...
    def f(datetime_text):
        datetime_valid(datetime_text, msg=None)
        return datetime_text
    return f

def ValidTime(msg=None):
    def f(time_text):
        time_valid(time_text, msg=None)
        return time_text
    return f
...
def AllowedPassword():
    ...
    ValidString()
    def f(password):
        password_valid(password, msg=None)
        return str(password)
    return f

def OnlyDigits(msg=None):
    def f(text):
        string_of_integers_valid(text, msg=None)
        return text
    return f
...
def datetime_valid(str_data, msg=None):
    string_valid(str_data, msg)
    try:
        datetime.strptime(str_data, datetime_format)
    except ValueError:
        raise Invalid(msg or 'DateTime should be of the format {}'.format(datetime_pattern))

def email_valid(str_data, msg=None):
    string_valid(str_data, msg)
    if not re.match("[\w\.\-]*@[\w\.\-]*\.\w+", str_data):
        raise Invalid(msg or ('Please use a valid Email ID'))

def password_valid(str_data, msg=None):
    string_valid(str_data, msg)
    reg_exp = r'[a-zA-Z0-9~` !@#$%^&*()-_=+{}[]:;"\'<,>.?/|\]*'
    if not re.match(reg_exp, str_data):
        raise Invalid('Invalid character(s) in password')

def string_of_integers_valid(str_data, msg=None):
    string_valid(str_data, msg)
