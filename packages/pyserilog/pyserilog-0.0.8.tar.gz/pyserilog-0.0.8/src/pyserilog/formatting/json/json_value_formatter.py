...
class JsonValueFormatter(LogEventPropertyValueVisitor[StringIOWritable, bool]):
    def __init__(self, type_tag_name: str | None = "_typeTag"):
        self._type_tag_name = type_tag_name

    def format(self, value: LogEventPropertyValue, output: StringIOWritable):
        self.visit(output, value)

    def _visit_scalar_value(self, state: StringIOWritable, value: ScalarValue) -> bool:
        Guard.against_null(value)
        self._format_literal_value(value.value, state)
        return False

    def _visit_sequence_value(self, output: StringIOWritable, sequence: SequenceValue) -> bool:
    ...
    def _visit_structure_value(self, state: StringIOWritable, structure: StructureValue) -> bool:
        Guard.against_null(structure)

        def write_property(event_property: log_event_property):
    ...
    def _visit_dictionary_value(self, state: StringIOWritable, dictionary: DictionaryValue) -> bool:
    ...
    def _format_literal_value(self, value, output: StringIOWritable):
        if value is None:
    ...
    @staticmethod
    def _format_null_value(output: StringIOWritable):
        output.write("null")

    @staticmethod
    def _format_string_value(output: StringIOWritable, value: str):
        JsonValueFormatter.write_quoted_json_string(value, output)

    @staticmethod
    def write_quoted_json_string(value: str, output: StringIOWritable):
        output.write('"')
    ...
    def _format_literal_object_value(self, value, output: StringIOWritable):
        Guard.against_null(value)
    ...
    def _format_number_value(self, value, output: StringIOWritable):
        if math.isnan(value):
