...
class JsonFormatter(ITextFormatter):
    ...
    def format(self, log_event, output: StringIOWritable):
    ...
    def __write_timestamp(self, timestamp: datetime, delim, output: StringIOWritable):
        self.__write_json_property("timestamp", timestamp, delim, output)

    def __write_json_property(self, name: str, value, preceding_delimiter, output: StringIOWritable):
    ...
    def __write_literal_value(self, value, output: StringIOWritable):
        self.write_string(str(value), output)

    def __write_literal(self, value, output: StringIOWritable, force_quotation: bool = False):
        if value is None:
            output.write("null")
            return
    ...
    @staticmethod
    def __write_to_string(number, quote: bool, output: StringIOWritable):
        if quote:
            output.write('"')
    ...
    def __write_properties(self, props: dict[str, LogEventPropertyValue], output: StringIOWritable):
        output.write(",\"properties\":{")
        self.__write_properties_values(props=props, output=output)
        output.write("}")

    def __write_properties_values(self, props: dict[str, LogEventPropertyValue], output: StringIOWritable):
    ...
    @staticmethod
    def __write_datetime(value: datetime, output: StringIOWritable):
        output.write('"')
    ...
    @staticmethod
    def write_string(value: str, output: StringIOWritable):
        JsonValueFormatter.write_quoted_json_string(value, output)

    def __write_sequence(self, elements: list, output: StringIOWritable, force_quotation: bool = False):
        output.write("[")
        delim = JsonFormatter.Delimiter()
    ...
    def __write_dictionary(self, elements: list[tuple[ScalarValue, LogEventPropertyValue]], output: StringIOWritable):
        output.write("{")
        delim = JsonFormatter.Delimiter()
