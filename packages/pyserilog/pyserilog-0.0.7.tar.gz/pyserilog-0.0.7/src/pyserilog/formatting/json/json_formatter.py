...
class JsonFormatter(ITextFormatter):
    ...
    def __write_timestamp(self, timestamp: datetime, delim, output: StringIO):
        self.__write_json_property("timestamp", timestamp, delim, output)

    def __write_json_property(self, name: str, value, preceding_delimiter, output: StringIO):
    ...
    def __write_literal_value(self, value, output: StringIO):
        self.write_string(str(value), output)

    def __write_literal(self, value, output: StringIO, force_quotation: bool = False):
        if value is None:
            output.write("null")
            return
    ...
    @staticmethod
    def __write_to_string(number, quote: bool, output: StringIO):
        if quote:
            output.write('"')
    ...
    def __write_properties(self, props: dict[str, LogEventPropertyValue], output: StringIO):
        output.write(",\"properties\":{")
        self.__write_properties_values(props=props, output=output)
        output.write("}")

    def __write_properties_values(self, props: dict[str, LogEventPropertyValue], output: StringIO):
    ...
    @staticmethod
    def __write_datetime(value: datetime, output: StringIO):
        output.write('"')
    ...
    @staticmethod
    def write_string(value: str, output: StringIO):
        JsonValueFormatter.write_quoted_json_string(value, output)

    def __write_sequence(self, elements: list, output: StringIO, force_quotation: bool = False):
        output.write("[")
        delim = JsonFormatter.Delimiter()
    ...
    def __write_dictionary(self, elements: list[tuple[ScalarValue, LogEventPropertyValue]], output: StringIO):
        output.write("{")
        delim = JsonFormatter.Delimiter()
