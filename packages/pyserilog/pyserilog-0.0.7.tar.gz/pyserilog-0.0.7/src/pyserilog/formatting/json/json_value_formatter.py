...
class JsonValueFormatter(LogEventPropertyValueVisitor[StringIO, bool]):
    def __init__(self, type_tag_name: str | None = "_typeTag"):
        self._type_tag_name = type_tag_name

    def format(self, value: LogEventPropertyValue, output: StringIO):
        self.visit(output, value)

    def _visit_scalar_value(self, state: StringIO, value: ScalarValue) -> bool:
        Guard.against_null(value)
        self._format_literal_value(value.value, state)
        return False

    def _visit_sequence_value(self, output: StringIO, sequence: SequenceValue) -> bool:
    ...
    def _visit_structure_value(self, state: StringIO, structure: StructureValue) -> bool:
    ...
    def _visit_dictionary_value(self, state: StringIO, dictionary: DictionaryValue) -> bool:
    ...
    def _format_literal_value(self, value, output: StringIO):
        if value is None:
    ...
    @staticmethod
    def _format_null_value(output: StringIO):
        output.write("null")

    @staticmethod
    def _format_string_value(output: StringIO, value: str):
        JsonValueFormatter.write_quoted_json_string(value, output)

    @staticmethod
    def write_quoted_json_string(value: str, output: StringIO):
        output.write('"')
    ...
    def _format_literal_object_value(self, value, output: StringIO):
        Guard.against_null(value)
    ...
    def _format_number_value(self, value, output: StringIO):
        if math.isnan(value):
