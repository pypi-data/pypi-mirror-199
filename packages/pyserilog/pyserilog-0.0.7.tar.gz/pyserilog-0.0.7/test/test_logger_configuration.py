...
class TestLoggerConfiguration:
    ...
    def test_disposable_sinks_are_not_disposed_along_with_contextual_loggers(self):
        ...
        with LoggerConfiguration() \
                .write_to.sink(sink) \
                .create_logger() \
                .for_context(class_type=TestLoggerConfiguration):
            pass
    ...
    def test_higher_minimum_level_overrides_are_propagated(self):
        ...
        logger = logger_conf.write_to.sink(sink).create_logger()

        logger.write_event(Some.information_event())
        logger.for_context(property_name=Constants.SOURCE_CONTEXT_PROPERTY_NAME, value="Microsoft.AspNet.Something") \
            .write_event(Some.information_event())
        logger.for_context(class_type=TestLoggerConfiguration).write_event(Some.information_event())
    ...
    def test_lower_minimum_level_overrides_are_propagated(self):
        ...
        logger.for_context(property_name=Constants.SOURCE_CONTEXT_PROPERTY_NAME, value="Microsoft.AspNet.Something") \
            .write_event(Some.information_event())
        logger.for_context(class_type=TestLoggerConfiguration) \
            .write_event(Some.information_event())
