...
class Deployment(Model):
    ...
    status = models.CharField(max_length=8, choices=STATUS_CHOICES, default='Todo', null=True, blank=True)

    def __str__(self):
        try:
            return "{} - {} ({})".format(self.date, self.application.friendly_name, self.application.urlid)
        except:
            return self.urlid
...
class ComponentExtension(Model):
    ...
    def __str__(self):
        try:
            return "{} + {} ({})".format(
                self.base_component.application.friendly_name, self.component.friendly_name, self.urlid
            )
        except:
            return self.urlid
...
class ComponentExtensionParameter(Model):
    ...
    def __str__(self):
        try:
            return "{} -> {} ({})".format(
                self.component.base_component.application.friendly_name, self.component.component.friendly_name, self.urlid
            )
        except:
            return self.urlid
