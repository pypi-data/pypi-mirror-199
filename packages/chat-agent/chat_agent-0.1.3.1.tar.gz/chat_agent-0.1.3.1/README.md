...
CHAT_AGENT_SSL_CERT_FILE_FULL_PATH_KEY：证书文件完整路径，默认cert.pem
CHAT_AGENT_SSL_KEY_FILE_FULL_PATH_KEY：证书key文件完整路径，默认key.pem
CHAT_AGENT_STATIC_PATH_KEY：静态文件路径，用于进行指定路径的隐藏文件路由，可用于进行certbot的CA服务验证使用
```
