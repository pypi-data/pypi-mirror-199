...
1. Server-side (API Key): for quickly getting started, testing, and production scenarios
    where LangChain-Lite will only use actions exposed in the developer's Zapier account
...
2. User-facing (Oauth): for production scenarios where you are deploying an end-user
    facing application and LangChain-Lite needs access to end-user's exposed actions and
...
from lib.bots import initialize_bot
from lib.bots.bot_toolkits import ZapierToolkit
from lib.utilities.zapier import ZapierNLAWrapper
...
bot = initialize_bot(
    toolkit.get_tools(),
    llm,
    bot="zero-shot-react-description",
    verbose=True
)

bot.run(("Summarize the last email I received regarding Silicon Valley Bank. "
...
from chatgpt_tool_hub.tools.dev.zapier.prompt import BASE_ZAPIER_TOOL_PROMPT
from chatgpt_tool_hub.tools.dev.utilities import ZapierNLAWrapper
