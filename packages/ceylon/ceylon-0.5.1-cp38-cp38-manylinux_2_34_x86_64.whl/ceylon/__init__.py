...
class AgentWrapper:
    ...
    async def run(self):
        if hasattr(self.agent, "setup_method"):
...
class CeylonAIAgent:
    ...
    def __start__(self):
        ...
        for i in range(self.__number_of_agents):
            ...
            agent = AgentWrapper(agent_class, f"{self.__agent__name}-{i}", background_processors,
                                 message_processors)
            # agent_tx.append(asyncio.create_task(agent.run()))

            tx = multiprocessing.Process(target=asyncio.run, args=(agent.run(),))
            agent_tx.append(tx)
