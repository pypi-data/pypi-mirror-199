...
class ImageDiscern:
    def __init__(self, target_image, grade=0.8, gauss_num=111) -> None:
        ...
        """
        self.source_image = os.path.join(os.path.abspath('report/screenshot'), 'SourceImage.png')
        if not os.path.exists(self.source_image):
            raise FileNotFoundError(f"文件: {self.source_image} 不存在")
        self.target_image = os.path.join(os.path.abspath('data'), target_image)
        ...
        if not os.path.exists(self.target_image):
            self.target_image = os.path.join(parent_dir, 'data', target_image)
            if not os.path.exists(self.target_image):
                self.target_image = os.path.join(patent_patent_dir, 'data', target_image)
                if not os.path.exists(self.target_image):
                    self.target_image = os.path.join(patent_patent_patent_dir, 'data', target_image)
                    if not os.path.exists(self.target_image):
...
if __name__ == '__main__':
    print(os.path.abspath('report/screenshot'))
