...
"""A library of functions related to input/output."""
...
def determine_imaging_content(folder: Union[str, pathlib.Path]) -> Tuple[int, int, int, int, int]:
    """
    ...
    def _extract_file_identifiers(str_to_split: str):
    ...
    # apparently * on a return is illegal for python 3.7
...
def repackage_bruker_tiffs(input_folder: Union[str, pathlib.Path], output_folder: Union[str, pathlib.Path],
    ...
    def save_image(images_, path_, type_=np.uint16):
        if len(images_.shape) == 2:
            with TiffWriter(path_) as tif:
                tif.write(np.floor(images).astype(type_))
            return
    ...
    if not_over_4gb():
        ...
        save_single_tiff(_images, os.path.join(output_folder, "compiled_video_0_of_0.tif"))
        return
    else:
        # noinspection PyTypeChecker
...
@convert_permitted_types_to_required(permitted=(str, pathlib.Path), required=pathlib.Path, pos=0)
def load_voltage_recording(path: pathlib.Path) -> pd.DataFrame:
    """
    Import analog data from an imaging folder or individual file. By convention, these files contain
    "VoltageRecording" in the name

    :param path: folder containing analog data
    :type path: str or pathlib.Path
    :return: dataframe containing time (index) x data [0:8]
    :rtype: pandas.DataFrame
    """
...
@convert_permitted_types_to_required(permitted=(str, pathlib.Path), required=pathlib.Path, pos=0)
def load_ephys_data(path: pathlib.Path) -> pd.DataFrame:
    """
    Import ephys data from a folder or individual file
    ...
    """
    if path.is_file():
        # TODO ADD BRUKER STD FILE CONVENTION IF APPLICABLE
        return _import_csv(path)
    elif not path.is_file():
        ephys_file = [file for file in path.glob("*.csv")]
        assert (len(ephys_file) == 1), "Too many files meeting parsing requirements"
        return _import_csv(ephys_file)
    else:
        raise FileNotFoundError


def load_xml(folder: str) -> None:
    pass


@convert_permitted_types_to_required(permitted=(str, pathlib.Path), required=str, pos=0)
@validate_extension(required_extension=".csv", pos=0)
def write_csv(path: str, data: np.ndarray, sampling_rate: int = 10000) -> None:
    """
    Write a CSV file for custom electrophysiology stimuli in PrairieView.

    :param path: filepath to save
    :type path: str or pathlib.Path
    :param data: 1D or 2D array of command signal and time (2D array)
    :type data: numpy.ndarray
    :param sampling_rate:
    :type sampling_rate: int = 10000
    :rtype: None
    """
    # Create time if not provided
    if len(data.shape) < 2:
        # Convert a time of data point from seconds to milliseconds
        time_point = 1 / int(sampling_rate) * 1000
        holding_time = np.full(len(data), time_point, dtype=np.double)
        data = np.vstack((holding_time, data)).T
