...
from foalorm.thirdparty.decorator import decorator as _decorator

if foalorm.MODE.startswith('GAE-'): localbase = object
...
def throw(exc_type, *args, **kwargs):
    ...
    exc.__cause__ = None
    try:
        if not (foalorm.MODE == 'INTERACTIVE' and options.CUT_TRACEBACK):
            raise exc
        else:
            raise exc  # Set "foalorm.options.CUT_TRACEBACK = False" to see full traceback
    finally: del exc
...
def absolutize_path(filename, frame_depth):
    ...
    if not is_absolute_path(code_filename):
        if code_filename.startswith('<') and code_filename.endswith('>'):
            if foalorm.MODE == 'INTERACTIVE': raise ValueError(
