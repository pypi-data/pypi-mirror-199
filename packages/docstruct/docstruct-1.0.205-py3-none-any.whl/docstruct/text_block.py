...
class Cell(TextBlock):
    ...
    def __init__(
        ...
        super().__init__(bounding_box=bounding_box, children=words)
    ...
    def set_words(self, words: list[Word]):
        ...
        self.set_children(sorted_words)
...
class Table(Area):
    ...
    def get_columns(self, cells: list[Cell]) -> list[TableColumn]:
        num_columns = max(cell.col_index for cell in cells) + 1
        columns: list[list[Cell]] = [None] * num_columns
    ...
    # def get_text(self):
    #     """
    #     This method assumes the headers of the table are in the first row
    #     """
    ...
    #             header = headers[j]
    #             cell_str = f"{header}: {str(cell)}"
    #             cells_str.append(cell_str)
    #         row_str = ", ".join(cells_str)
