...
class TextBlock(metaclass=abc.ABCMeta):
    ...
    @staticmethod
    def old(text_blocks: list["TextBlock"], height_offset: float) -> list["TextBlock"]:
        """
        Sort the list of text blocks by their relative order.
        """
        if len(text_blocks) <= 1:
            return text_blocks
        sorted_text_blocks_by_height = sorted(
            text_blocks,
            key=lambda text_block: text_block.get_bounding_box().get_center().y,
            reverse=True,
        )

        same_line = [sorted_text_blocks_by_height[0]]
        sorted_text_blocks = []
        for i in range(len(sorted_text_blocks_by_height[:-1])):
            if (
                abs(
                    sorted_text_blocks_by_height[i].get_bounding_box().get_center().y
                    - sorted_text_blocks_by_height[i + 1]
                    .get_bounding_box()
                    .get_center()
                    .y
                )
                ...
                same_line.append(sorted_text_blocks_by_height[i + 1])

            else:
                ordered_same_line = sorted(
                    same_line,
                    key=lambda text_block: text_block.get_bounding_box().get_left(),
                )
                sorted_text_blocks.extend(ordered_same_line)
                same_line = [sorted_text_blocks_by_height[i + 1]]

        ordered_same_line = sorted(
            same_line, key=lambda text_block: text_block.get_bounding_box().get_left()
        )
        sorted_text_blocks.extend(ordered_same_line)
    ...
    def _sort(text_blocks: list["TextBlock"], height_scale: float) -> list[int]:
        ...
        for indexes_group in indexes_groups:
            indexes_group = sorted(indexes_group, key=lambda index: centers[index].x)
        return [index for group in indexes_groups for index in group]
