...
class TestSelection:
    ...
    def test_single_element(self):
        df_answer = ick.DataFrame({'e': np.array([2])})
        assert_df_equals(df[1, 'e'], df_answer)

    def test_all_row_selections(self):
        df1 = ick.DataFrame({'a': np.array([True, False, True]),
                             'b': np.array([1, 3, 5])})
        with pytest.raises(ValueError):
            df[df1, 'e']
        ...
        df_answer = ick.DataFrame({'c': c[[True, False, True]]})
        assert_df_equals(df_result, df_answer)

        df_result = df[[1, 2], 0]
        df_answer = ick.DataFrame({'a': a[[1, 2]]})
        assert_df_equals(df_result, df_answer)
    ...
    def test_new_column(self):
        df_result = ick.DataFrame({'a': a, 'b': b, 'c': c, 'd': d, 'e': e})
        f = np.array([1.5, 23, 4.11])
        df_result['f'] = f
        df_answer = ick.DataFrame({'a': a, 'b': b, 'c': c, 'd': d, 'e': e, 'f': f})
        assert_df_equals(df_result, df_answer)
        ...
        f = np.array([1.5, 23, 4.11])
        df_result['c'] = f
        df_answer = ick.DataFrame({'a': a, 'b': b, 'c': f, 'd': d, 'e': e})
        assert_df_equals(df_result, df_answer)
        ...
        with pytest.raises(ValueError):
            df1 = ick.DataFrame({'a': np.random.rand(5)})
            df['a'] = df1
...
df2 = ick.DataFrame({'a': a2, 'b': b2, 'c': c2})
...
class TestAggregation:
    ...
    def test_min(self):
        df_result = df1.min()
        df_answer = ick.DataFrame({'a': np.array(['a'], dtype='O'),
                                   'b': np.array([5]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_max(self):
        df_result = df1.max()
        df_answer = ick.DataFrame({'a': np.array(['c'], dtype='O'),
                                   'b': np.array([11]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_mean(self):
        df_result = df1.mean()
        df_answer = ick.DataFrame({'b': np.array([8.]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_sum(self):
        df_result = df1.sum()
        df_answer = ick.DataFrame({'a': np.array(['abc'], dtype='O'),
                                   'b': np.array([24]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_var(self):
        df_result = df1.var()
        df_answer = ick.DataFrame({'b': np.array([b1.var()]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)

    def test_std(self):
        df_result = df1.std()
        df_answer = ick.DataFrame({'b': np.array([b1.std()]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)

    def test_all(self):
        df_result = df2.all()
        df_answer = ick.DataFrame({'a': np.array([False]),
                                   'b': np.array([True]),
                                   'c': np.array([False])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_any(self):
        df_result = df2.any()
        df_answer = ick.DataFrame({'a': np.array([True]),
                                   'b': np.array([True]),
                                   'c': np.array([True])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_argmax(self):
        ...
        df_answer = ick.DataFrame({'a': np.array([2]),
                                   'b': np.array([0]),
                                   'c': np.array([1])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_argmin(self):
        ...
        df_answer = ick.DataFrame({'a': np.array([0]),
                                   'b': np.array([1]),
                                   'c': np.array([1])})
        assert_df_equals(df_result, df_answer)
...
df3 = ick.DataFrame({'a': a3, 'b': b3, 'c': c3})

a4 = np.array(['a', 'a', 'c'], dtype='O')
b4 = np.array([11, 5, 5])
c4 = np.array([3.4, np.nan, 3.4])
df4 = ick.DataFrame({'a': a4, 'b': b4, 'c': c4})
...
class TestOtherMethods:
    ...
    def test_isna(self):
        df_result = df3.isna()
        df_answer = ick.DataFrame({'a': np.array([False, True, False]),
    ...
    def test_rename(self):
        ...
        df_answer = ick.DataFrame({'A': a4, 'b': b4, 'C': c4})
        assert_df_equals(df_result, df_answer)

    def test_drop(self):
        df_result = df4.drop(['a', 'b'])
        df_answer = ick.DataFrame({'c': c4})
        assert_df_equals(df_result, df_answer)


a42 = np.array([-11, 5, 3])
b42 = np.array([3.4, 5.1, -6])
df42 = ick.DataFrame({'a': a42, 'b': b42})
...
class TestNonAgg:
    ...
    def test_abs(self):
        df_result = df42.abs()
        df_answer = ick.DataFrame({'a': np.abs(a42), 'b': np.abs(b42)})
        assert_df_equals(df_result, df_answer)

    def test_cummin(self):
        df_result = df42.cummin()
        df_answer = ick.DataFrame({'a': np.array([-11, -11, -11]),
                                   'b': np.array([3.4, 3.4, -6])})
        assert_df_equals(df_result, df_answer)

    def test_cummax(self):
        df_result = df42.cummax()
        df_answer = ick.DataFrame({'a': np.array([-11, 5, 5]),
                                   'b': np.array([3.4, 5.1, 5.1])})
        assert_df_equals(df_result, df_answer)

    def test_cumsum(self):
        df_result = df42.cumsum()
        df_answer = ick.DataFrame({'a': np.array([-11, -6, -3]),
                                   'b': np.array([3.4, 8.5, 2.5])})
        assert_df_equals(df_result, df_answer)

    def test_clip(self):
        df_result = df42.clip(0, 4)
        df_answer = ick.DataFrame({'a': np.array([0, 4, 3]),
                                   'b': np.array([3.4, 4, 0])})
        assert_df_equals(df_result, df_answer)

    def test_round(self):
        df_result = df42.round(0)
        df_answer = ick.DataFrame({'a': np.array([-11, 5, 3]),
                                   'b': np.array([3, 5, -6])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_diff(self):
        ...
        df_answer = ick.DataFrame({'a': np.array([np.nan, 16, -2]),
                                   'b': np.array([np.nan, 1.7, -11.1])})
        assert_df_equals(df_result, df_answer)

    def test_pct_change(self):
        df_result = df42.pct_change(1)
        df_answer = ick.DataFrame({'a': np.array([np.nan, 16 / -11, -2 / 5]),
...
class TestOperators:
    ...
    def test_add(self):
        df_result = df5 + 3
        df_answer = ick.DataFrame({'a': a5 + 3, 'b': b5 + 3})
        assert_df_equals(df_result, df_answer)
    ...
    def test_sub(self):
        ...
        df_result = 3 - df5
        df_answer = ick.DataFrame({'a': 3 - a5, 'b': 3 - b5})
        assert_df_equals(df_result, df_answer)
    ...
    def test_pow(self):
        ...
        df_result = 2 ** df5
        df_answer = ick.DataFrame({'a': 2 ** a5, 'b': 2 ** b5})
        assert_df_equals(df_result, df_answer)
    ...
    def test_gt_lt(self):
        ...
        df_result = df5 < 2
        df_answer = ick.DataFrame({'a': a5 < 2, 'b': b5 < 2})
        assert_df_equals(df_result, df_answer)
...
df6 = ick.DataFrame({'a': a6, 'b': b6})

a7 = np.array(['b', 'a', 'a', 'a', 'b'])
b7 = np.array([3.4, 5.1, 2, 1, 6])
df7 = ick.DataFrame({'a': a7, 'b': b7})
...
class TestMoreMethods:
    ...
    def test_sort_values_two_desc(self):
        ...
        a = np.array(['a', 'a', 'a', 'b', 'b'])
        b = np.array([1, 2, 5.1, 3.4, 6])
        df_answer = ick.DataFrame({'a': a[::-1], 'b': b[::-1]})
        assert_df_equals(df_result, df_answer)

    def test_sample(self):
        df_result = df7.sample(2, seed=1)
        df_answer = ick.DataFrame({'a': np.array(['a', 'a'], dtype=object),
                                   'b': np.array([2., 5.1])})
        assert_df_equals(df_result, df_answer)

        df_result = df7.sample(frac=.7, seed=1)
        df_answer = ick.DataFrame({'a': np.array(['a', 'a', 'b'], dtype=object),
                                   'b': np.array([2., 5.1, 6.])})
        assert_df_equals(df_result, df_answer)
...
df8 = ick.DataFrame({'a': a8, 'b': b8, 'c': c8})


class TestGrouping:

    def test_value_counts(self):
        df_temp = ick.DataFrame({'state': np.array(['texas', 'texas', 'texas', 'florida', 'florida', 'florida', 'florida', 'ohio']),
                                 'fruit': np.array(['a', 'a', 'a', 'a', 'b', 'b', 'b', 'a'])})
        df_results = df_temp.value_counts()
        df_answer = ick.DataFrame({'state': np.array(['florida', 'texas', 'ohio'], dtype=object),
                                   'count': np.array([4, 3, 1])})
        assert_df_equals(df_results[0], df_answer)

        df_answer = ick.DataFrame({'fruit': np.array(['a', 'b'], dtype=object),
                                   'count': np.array([5, 3])})
    ...
    def test_pivot_table_rows_or_cols(self):
        ...
        df_answer = ick.DataFrame({'a': np.array(['a', 'b'], dtype=object),
                                   'size': np.array([5, 3])})
        assert_df_equals(df_result, df_answer)
        ...
        df_answer = ick.DataFrame({'A': np.array([4]),
                                   'B': np.array([4])})
        assert_df_equals(df_result, df_answer)

        df_result = df8.pivot_table(columns='a', values='c', aggfunc='sum')
        df_answer = ick.DataFrame({'a': np.array([22]), 'b': np.array([14])})
        assert_df_equals(df_result, df_answer)
...
df_string = ick.DataFrame({'movie': movie, 'num': num})
