...
class Connector(connector.SQLConnector):
    ...
    def create_stage(self, allowed_location):
        integration_name = 'tinybird_integration'
        stage_name = 'tinybird_stage'
        ...
        integration_properties = self._query(f'describe integration {integration_name}')
        account = list(filter(lambda p: p['property'] == 'STORAGE_GCP_SERVICE_ACCOUNT', integration_properties))[0]['property_value']

        return { "stage": f'{database}.{schema}.{stage_name}', "gcp_account": account }
