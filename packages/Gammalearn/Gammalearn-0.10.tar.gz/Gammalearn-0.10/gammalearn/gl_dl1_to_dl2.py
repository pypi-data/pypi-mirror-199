...
def create_dataset(file_queue,
    ...
    while True:
        file = file_queue.get()
        if file != 'Done':
            dl1_ds = create_dataset_worker(file,
                                           dataset_class,
                                           train=False,
                                           **kwargs)
            dl1_queue.put(dl1_ds)
        else:
            break
...
def main():
    ...
    experiment.dataset_parameters['transform'] = experiment.data_module_train['transform']
    ...
    for file in tqdm.tqdm(dl1_file_list):
        file_queue.put(file)
    for _ in range(settings.preprocessing_workers):
        file_queue.put('Done')
    ...
    logger.info('get dl1 files and populate dataset queue')
    for _ in range(args.preprocess_workers):
        Process(target=partial(create_dataset,
                               dataset_class=experiment.dataset_class,
                               **experiment.dataset_parameters),
                args=(file_queue, dl1_queue)).start()
    ...
    for ii, dl1_filename in tqdm.tqdm(enumerate(dl1_file_list)):
        dset = dl1_queue.get()
        dloader = DataLoader(dset, batch_size=args.batch_size, num_workers=args.dataloader_workers)

        if model is None:
            logger.info('Load model')
            geometries = []
            get_dataset_geom(dset, geometries)
            ...
            experiment.net_parameters_dic = inject_geometry_into_parameters(experiment.net_parameters_dic,
                                                                          geometries[0])
            # Load checkpoint
            checkpoint_path = experiment_path.joinpath(args.checkpoint)
            model = LitGLearnModule.load_from_checkpoint(checkpoint_path, experiment=experiment)
            model.eval()
            model.to(device)
        ...
        output_path = dl2_path.joinpath(dl2_filename(dset.hdf5_file_path))
        if os.path.exists(output_path) and args.overwrite:
            os.remove(output_path)
        write_dl2_file(dl2_params, dset, output_path)
