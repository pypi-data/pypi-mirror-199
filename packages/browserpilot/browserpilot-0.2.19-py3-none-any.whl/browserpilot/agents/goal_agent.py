...
class GoalAgent:
    ...
    def _get_first_instruction(self) -> str:
        ...
        self.commands.append(first_instruction)
    ...
    def _get_natbot_response(self) -> str:
        ...
        prompt = PROMPT_FOR_REST_OF_INSTRUCTIONS.format(
            curr_url=curr_url, html=elements, objective=self.objective, last_command=self.commands[-1]
        )
        next_instruction = self._get_completion(prompt=prompt, model=self.model)
        next_instruction = next_instruction.strip()
        self.commands.append(next_instruction)
    ...
    def _get_compiled_instruction(self, natbot_instruction) -> str:
        ...
        def get_code_for_selecting_element(element):
            ...
            Please default to using single quotes in your xpath selector. If you use double quotes, then
