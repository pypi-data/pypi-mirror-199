...
class AnimoTrainerController:
    def __init__(self, session: AnimoTrainingSession):
        ...
        animo_env = AnimoEnv(session)
        self.env_manager = SimpleEnvManager(animo_env, EnvironmentParametersChannel())
    ...
    def advance(self, env_manager: EnvManager) -> int:
        ...
        self.step += 1
        if self.session.should_check_daemon and self.step % DAEMON_CHECK_PERIOD == 0 and not psutil.pid_exists(self.session.daemon_pid):
            raise KeyboardInterrupt
