{% materialization incremental, adapter='spark' -%}
  ...
  {% if existing_relation is none %}
    {% if file_format == 'openhouse' %}
      {# -- To support Openhouse, instead of running CTAS directly, run CTAS limit 0; INSERT INTO ... #}
      {% do run_query(spark__create_table_as_limit0(False, target_relation, sql)) %}
      {% set build_sql = spark__insert_workaround(target_relation, sql) %}
    {% else %}
  ...
  {% else %}
    {% if file_format == 'openhouse' %}
      {% do run_query(spark__create_table_as_limit0(True, tmp_relation, sql)) %}
    {% else %}
