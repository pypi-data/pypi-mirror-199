# -*- coding: utf-8 -*-
#---------------------------------------------------------------------#
#   FLApy: Forest Light Analysis python package                       #
#   This module: Some functions and tools related to data management  #
#   Version: 1.0                                                      #
#   Developer: Bin Wang (Yunnan University, Kunming, China)           #
...
class StudyFieldLattice(UniformGrid):
    ...
    def __init__(self, workspace = None, *args, **kwargs):
        ...
        if workspace is None:
            self._workspace = os.path.abspath(os.path.dirname(__file__))
        else:
            self._workspace = workspace

        self.temPath = str(self._workspace + '/tem./.temFile.vtk')
    ...
    def m2p(cls, inMesh):
        ...
        return _point
    @classmethod
    def read_CsvData(self, filePath, skiorows = 1, add2DC = True):
        obs = np.loadtxt(filePath, dtype=np.float, delimiter=',', skiprows=skiorows)
        if add2DC:
            self._obs = obs
        else:
            return obs
    ...
    def gen_SFL(self, bbox, resolution, bufferSize = 100):
        ...
        xMin = min(keptPoints[:, 0])
        yMin = min(keptPoints[:, 1])
        ...
        self._cellPoints_TerrainNormalization = self.normlization_height(self._obsSet)[:, 2]



        self._SFL.field_data['PTS'] = self.point_cloud_buffered
        self._SFL.field_data['OBS_SFL'] = self._obsSet
        self._SFL.field_data['DEM'] = self._DEMp.points
        self._SFL.field_data['DTM'] = self._DTMp.points

        if self._obs is None:

            self._SFL.field_data['OBS_extra'] = np.zeros(10)

        else:
            self._SFL.field_data['OBS_extra'] = self._obs

        custom_var_pickle = pickle.dumps(self.temPath)
        self._SFL.field_data['tem'] = np.array([custom_var_pickle], dtype=np.object)

        self._SFL.cell_data['Z_normed'] = self._cellPoints_TerrainNormalization
    ...
    def clip_points(self, points, bbox):
        return self.clip_Points(points, bbox)
    ...
    @staticmethod
    def get_ValueByGivenPointsOnRasterMatrix(x, y, raster):
        ...
        da = raster.sel(x=tgt_x, y=tgt_y, method="nearest")
