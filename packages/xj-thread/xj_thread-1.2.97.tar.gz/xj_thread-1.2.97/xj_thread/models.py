...
class ThreadCategory(models.Model):
    ...
    platform_code = models.CharField(verbose_name="平台码", max_length=16, blank=True, null=True, default="")
    value = models.CharField(verbose_name='值', max_length=50, help_text='')
    name = models.CharField(verbose_name='名称', max_length=255, blank=True, null=True, help_text='')
...
class Thread(models.Model):
    ...
    show = models.ForeignKey(verbose_name='展示ID', to=ThreadShow, default=1, null=True, blank=True, on_delete=models.DO_NOTHING, related_name='+', help_text='如果没有传入显示类型，则使用分类表中的默认显示类型')  # 如果没有传入显示类型，则使用分类表中的默认显示类型
