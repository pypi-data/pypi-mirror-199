...
class SimbaProjectPopUp(ConfigReader, PopUpMixin):
    def __init__(self,
        ...
        append_roi_features_by_animal = Button(label_extractfeatures, text='APPEND ROI DATA TO FEATURES: BY ANIMAL (CAUTION)', fg='red',command=lambda: SettingsMenu(config_path=self.config_path, title='APPEND ROI FEATURES'))
        append_roi_features_by_body_part = Button(label_extractfeatures, text='APPEND ROI DATA TO FEATURES: BY BODY-PARTS (CAUTION)', fg='orange',command=lambda: AppendROIFeaturesByBodyPartPopUp(config_path=self.config_path))
        append_roi_features_by_animal.grid(row=10, column=0, sticky=NW)
        append_roi_features_by_body_part.grid(row=11, column=0, sticky=NW)
        ...
        visualize_annotation_img_btn = Button(lbl_tools_frm, text='Visualize annotations', command=lambda: ExtractAnnotationFramesPopUp(config_path=self.config_path))
