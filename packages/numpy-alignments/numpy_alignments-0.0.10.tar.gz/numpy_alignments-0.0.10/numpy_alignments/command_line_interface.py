...
def get_correct_rates(args):
    ...
    for name, rate in rates.items():
        ...
        elif args.report_type in ["recall", "MappingRecall"]:
            print(recall)
        elif args.report_type in ["one_minus_precision", "MappingOneMinusPrecision"]:
            print(one_minus_precision)
        elif args.report_type in ["f1_score", "MappingF1Score"]:
            f1 = 2 * precision * recall / (precision + recall)
            print(f1)
        else:
            raise Exception("Invalid report type %s" % args.report_type)
