import os
from dnbc4tools.tools.utils import rm_temp,str_mkdir,start_print_cmd,judgeFilexits,change_path,python_path,logging_call,read_json
...
class Data:
    ...
    def run(self):
        ...
        scRNA_parse_cmd = ['%s %s/rna/src/star_anno.py --name %s --cDNAfastq1 %s --cDNAfastq2 %s --oligofastq1 %s --oligofastq2 %s --threads %s --chemistry %s --darkreaction %s --outdir %s --genomeDir %s --gtf %s '\
            %(new_python,__root_dir__,self.name,self.cDNAr1,self.cDNAr2,self.oligor1,self.oligor2,self.threads,self.chemistry,self.darkreaction,self.outdir,genomeDir,gtf)]
        if chrmt != 'None':
        ...
        final_sort_cmd = 'samtools sort -@ %s %s/01.data/final.bam -o %s/01.data/final_sorted.bam'\
            %(self.threads,self.outdir,self.outdir)
