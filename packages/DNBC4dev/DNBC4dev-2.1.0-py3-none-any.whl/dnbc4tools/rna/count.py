import os
from dnbc4tools.tools.utils import str_mkdir,python_path,logging_call,judgeFilexits,change_path,rscript_path
...
class Count:
    ...
    def run(self):
        ...
        cellCalling_cmd = '%s %s/rna/src/cell_calling.R --matrix %s/01.data/raw_matrix --outdir %s/02.count/ --method %s --expectcells %s --forcecells %s --minumi %s'\
            %(new_rscript,__root_dir__,self.outdir,self.outdir,self.calling_method,self.expectcells,self.forcecells,self.minumi)
        ...
        def create_index(threads,bam):
            try:
                bam_index_cmd = 'samtools index -@ %s %s'%(threads,bam)
                logging_call(bam_index_cmd,'count',self.outdir)
            except Exception as e:
                print('build csi index for bam')
                bam_index_cmd = 'samtools index -c -@ %s %s'%(threads,bam)
                logging_call(bam_index_cmd,'count',self.outdir)
        create_index(self.threads,'%s/02.count/anno_decon_sorted.bam'%self.outdir)

        ### cal saturation
        saturation_cmd = '%s %s/rna/src/saturation.py -i %s/02.count/anno_decon_sorted.bam -o %s/02.count -f %s/02.count/cellCount_report.csv --quality 20 --threads %s'\
            %(new_python,__root_dir__,self.outdir,self.outdir,self.outdir,self.threads)
