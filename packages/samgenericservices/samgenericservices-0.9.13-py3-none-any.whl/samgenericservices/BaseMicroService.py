...
class BaseMicroService:
    ...
    def __init__(self,
        ...
        :param psqluser: optional user name for PostgreSQL server to connect, by default value of dbname is taken as
                         login name to connect with
        """
        # Constants
        self._TTL = pikattl
        self._SSL = pikassl
        ...
        # Load secrets
        with open(f"/home/{dbname}/.secret/.secret", "r") as _secret_file_fd:
        ...
        if psqluser is None:
            self._psqluser = self._dbname
        else:
            self._psqluser = psqluser
        # Connection pool
    ...
    async def _conn_rmq(self) -> None:
        ...
        self._rmq_snd_conn = await aio_pika.connect_robust(host=self._config["rmqhost"],
                                                           port=_rmq_port,
                                                           client_properties={
                                                               "connection_name": (f"{self._dbname}"
