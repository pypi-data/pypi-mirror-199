...
"""
try:
    from re2 import compile, search
except ImportError:
...
RE_ALBUM_ID = compile(r'act=audio_playlist(-?\d+)_(\d+)')
RE_ACCESS_HASH = compile(r'access_hash=(\w+)')
...
class VkAudio(object):
    ...
    def get_albums_iter(self, owner_id=None):
        ...
        while True:
            ...
            if response.status_code == 302 and retries:
                self._vk.http.get('https://m.vk.com/')
                logger.warning(f"Re-try get user {owner_id} playlists.")
                retries -= 1
                continue
    ...
    def search_user(self, owner_id=None, q=''):
        ...
        json_response = loads(response.text.replace('<!--', ''))
    ...
    def get_popular_iter(self, offset=0):
        ...
        json_response = loads(scrap_json(response.text))
...
def scrap_json(html_page):
    ...
    fr = search(find_json_pattern, html_page).group(1)
...
def scrap_ids_from_html(html, filter_root_el=None):
    ...
    for audio in root_el.find_all('div', {'class': 'audio_item'}):
        ...
        data_audio = loads(audio['data-audio'])
...
def scrap_tracks(ids, user_id, http, convert_m3u8_links=True):
    ...
    for ids_group in [ids[i:i + 10] for i in range(0, len(ids), 10)]:
        delay = RPS_DELAY_RELOAD_AUDIO - (time() - last_request)
