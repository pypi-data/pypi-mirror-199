...
RE_LOGIN_TO = compile(r'"to":"(.*?)"')
RE_LOGIN_IP_H = compile(r'name="ip_h" value="([a-z0-9]+)"')
RE_LOGIN_LG_H = compile(r'name="lg_h" value="([a-z0-9]+)"')
RE_LOGIN_LG_DOMAIN_H = compile(r'name="lg_domain_h" value="([a-z0-9]+)"')

RE_CAPTCHAID = compile(r"onLoginCaptcha\('(\d+)'")
RE_NUMBER_HASH = compile(r"al_page: '3', hash: '([a-z0-9]+)'")
RE_AUTH_HASH = compile(r"Authcheck\.init\('([a-z_0-9]+)'")
RE_TOKEN_URL = compile(r'location\.href = "(.*?)"\+addr;')

RE_PHONE_PREFIX = compile(r'label ta_r">\+(.*?)<')
RE_PHONE_POSTFIX = compile(r'phone_postfix">.*?(\d+).*?<')
...
class VkApi(object):
    ...
    def _pass_twofactor(self, auth_response):
        ...
        data = loads(response.text.lstrip('<!--'))
        status = data['payload'][0]

        if status == '4':  # OK
            path = loads(data['payload'][1][0])
