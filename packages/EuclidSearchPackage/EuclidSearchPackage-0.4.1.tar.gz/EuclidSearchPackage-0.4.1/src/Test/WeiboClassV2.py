...
class WeiboClassV2:
    ...
    def get_url_list(self, beginTime: str, endTime: str):
        ...
        while NetPage:
            ...
            onePageList = ESP.Get_item_url_list(targetUrl)
    ...
    def main(self, beginTime, endTime, ColName=None):
        ...
        if self.Mongo:
            _COL = ESP.MongoClient('Weibo', ColName)
        else:
            _COL = ESP.CsvClient('Weibo', ColName)
        ...
        while NewEndTime > beginTime:
            ...
            for mblogid in tqdm(self.UrlList):
                try:
                    data_json = ESP.Get_single_weibo_data(mblogid.split("/")[-1])
