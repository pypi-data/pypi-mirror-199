...
class CaseOpti(GenMapManager):
    ...
    def __init__(self, splash=False, init_wx=True, *args, **kw):
        if init_wx:
            super().__init__(splash=splash, *args, **kw)
        # super().__init__(splash=splash, *args, **kw)

        self.launcherDir = ""
        self.withGUI = init_wx
    ...
    def read_param(self, dir, copyDefault=False, callback=None, workingDir=""):
        ...
        self.launcherParam = Wolf_Param(to_read=True, filename=os.path.join(self.launcherDir,"launcher.param"),title="launcher", toShow=False, initwx=self.withGUI)
...
class Optimisation(wx.Frame):
    ...
    def new(self, event):
        ...
        self.optiParam = Wolf_Param(to_read=True, filename=os.path.join(self.workingDir,"test_opti.param"),title="test_opti",toShow=False,initwx=self.withGUI)
        ...
        newCase = CaseOpti(init_wx=self.withGUI)
        ...
        self.comparHowParam = Wolf_Param(to_read=True,filename=os.path.join(self.workingDir,"compare.how.param"),title="compare.how",toShow=False,initwx=self.withGUI)
        self.saParam = Wolf_Param(to_read=True,filename=os.path.join(self.workingDir,"sa.param"), title="sa",toShow=False,initwx=self.withGUI)
        ...
        try:
            ...
            newCase.mydro = HydrologyModel(dir=curDir,splash=False, in_gui=self.withGUI)
            newCase.mydro.Hide()
    ...
    def load(self, event, workingDir:str="", fileName:str=""):
        ...
        # Read the main opti file
        self.optiParam = Wolf_Param(to_read=True, filename=fileOpti, title="test_opti",toShow=False, initwx=self.withGUI)
        ...
        for i in range(nbcases):
            ...
            if self.withGUI:
                ...
                if(iItem==wx.NOT_FOUND):
                    ...
                    if isOk<0:
                        print("ERROR : in path of launcherDir")
                    newCase.mydro = HydrologyModel(dir=refDir, in_gui=self.withGUI)
                    newCase.mydro.Hide()
                    self.Bind(wx.EVT_MENU, newCase.show_mydro, guiHydroCase)
                    curCase = paramMenu.Append(newId, curName, caseMenu)
                else:
        ...
        for i in range(self.nbParams):
            ...
            if(int(myType)>0):
                ...
                if "Convertion Factor" in myModelDict[int(myType)]:
                    ...
                    convFact = 1.0
                tmpWolf = Wolf_Param(to_read=True, filename=filePath+fileName,toShow=False,initwx=self.withGUI)
