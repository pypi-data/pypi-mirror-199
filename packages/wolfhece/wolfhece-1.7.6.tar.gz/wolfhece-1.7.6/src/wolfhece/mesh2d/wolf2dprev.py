...
class bloc_file():
    ...
    def __init__(self, parent) -> None:
        ...
        self.my_blocks = []
        self.my_vec_blocks = Zones()  # #< vecteur des objets blocs
    ...
    def read_file(self):
        ...
        external_border = bloc_external_border(is2D=True, name='external border')  ##< vecteur du contour externe
        ...
        # Si nb est négatif, il existe une troisième colonne qui dit si le segment est utile ou pas (maillage de
        # zones intérieures)
        if nb < 0:
            self.interior = True
    ...
    def write_file(self):
        ...
        with open(self.filename, 'w') as f:
            ...
            f.write('{},{}\n'.format(self.nb_blocs,self.max_size_cont))
...
class bloc_external_border(vector):
    ...
    def set_limits(self):
        ...
        self.find_minmax()
        self.mylimits = ((self.minx, self.maxx), (self.miny, self.maxy))

    def verify_limits(self):

        self.find_minmax()

        if self.minx < self.mylimits[0][0] or self.maxx > self.mylimits[0][1] or \
                self.miny < self.mylimits[1][0] or self.maxy > self.mylimits[1][1]:
