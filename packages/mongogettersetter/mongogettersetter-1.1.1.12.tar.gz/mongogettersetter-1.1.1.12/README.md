...
- `__getattr__(self, key)`: Returns a `MongoDataWrapper` instance for the given `key`. See below for the capabalities of `MongoDataWrapper`
    ...
    Here, we initialize MyClass with `_collection` and `_filter_query` as mandatory attributes for MongoGetterSetter to function properly. These 2 arrtibutes are used internally to do further manipulations to MongoDB
