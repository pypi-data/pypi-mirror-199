...
def specific_stages_all_chapters(save_stats: dict[str, Any], level_ids: dict[str, Any], tre_id: str) -> dict[str, Any]:
    """Handler for editing treasure levels"""

    
    chapter_ids = level_ids
    print(tre_id)
    treasure_stats = save_stats["treasures"]

    stage_ids = list(range(0, 48))
    treasure_data = [-1] * 48
    for i, chapter_id in enumerate(chapter_ids):
        chapter_id = main_story.format_story_id(chapter_id)
        if i == 0:
            treasure_data = handle_all_at_once2(
                stage_ids,
                True,
                treasure_data,
                list(range(0, 48)),
