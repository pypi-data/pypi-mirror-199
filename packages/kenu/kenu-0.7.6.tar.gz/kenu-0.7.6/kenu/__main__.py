    ...
    try:
        if("kenu" in module):
            ...
            if not os.path.exists(name):
                os.makedirs(name)
            urllib.request.urlretrieve(f"https://kenucorp.com/modules/" + module + ".py", f"{name}\{module}.py")
            urllib.request.urlretrieve(f"https://kenucorp.com/modules/{module}requirements.txt", name + f"{module}\\requirements.txt")
            try:
                with open(module + "\\requirements.txt", 'r') as file:
                    line = file.readline()
                    while line:
                        os.systs
                        line = file.readline()
