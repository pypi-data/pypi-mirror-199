...
class FileHandler:
    ...
    def write_to_local_filesystem(self, local_path, content):
        with open(f"{local_path}/{os.path.basename(content)}", 'wb') as f:
            f.write(content)
        logger.success(f"file saved into: {local_path}")
