...
class MigrationLog(LoggerBuilder):
    ...
    def log_skipped_survey(survey_name: str, reason: str):
        MigrationLog.logger.info(f"{LogConsts.SKIP} -- Survey: skipping migration of survey {survey_name} because {reason}.")
    ...
    def log_no_file_label_error(mb_file: MbFile):
        MigrationLog.logger.error(f"no valid file label identified for file {mb_file.data_file} with ngdc_id {mb_file.ngdc_id}")
