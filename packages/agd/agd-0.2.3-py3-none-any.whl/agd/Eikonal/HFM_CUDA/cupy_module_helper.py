...
#			source.append(f"#define {key}_macro")
		...
		elif isinstance(value,tuple) and len(value)==2 and isinstance(value[1],type):
			val,dtype=value
		...
		elif isinstance(value,np.ndarray) and value.dtype==np.int:
			s = f"const int {key}";
			for i in value.shape: s+=f"[{i}]"
			s+='='+np.array2string(value,separator=',').replace('[','{').replace(']','}')+';'
			source.append(s)
		else: 
