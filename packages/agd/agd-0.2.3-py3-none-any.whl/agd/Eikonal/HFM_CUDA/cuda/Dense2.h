...
/* Represents x = a+<b,h>+<h,c h>/2 +o(|h|^2), where h is an infinitesimal perturbation. */
...
/* Represents x = a+<b,h>+o(|h|), where h is an infinitesimal perturbation. */
template<typename Scalar, int ndim>
struct Dense1 : DifferentiationTypeOperators<Dense1<Scalar,ndim>,Scalar> {
	...
	static void solve(Dense1 in[ndim], Scalar out[ndim]){ // in : Ah+b, out = -A^{-1}b
