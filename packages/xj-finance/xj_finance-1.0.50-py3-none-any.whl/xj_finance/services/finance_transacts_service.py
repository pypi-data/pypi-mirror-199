...
class FinanceTransactsService:
    ...
    @staticmethod
    def finance_standing_book(enroll_id):
        transact_obj = Transact.objects
        if not sys.modules.get("xj_enroll.service.enroll_services.EnrollServices"):
            from xj_enroll.service.enroll_services import EnrollServices
            enroll_set, err = EnrollServices.enroll_detail(enroll_id)
            if err:
                return None, err
            print(enroll_set)
        transact_filter_obj = transact_obj.filter(
            enroll_id=enroll_id, sand_box__isnull=False).values("order_no", "transact_no", "goods_info",
                                                                "account_id",
                                                                "enroll_id",
                                                                "their_account_id",
                                                                "platform_id",
