...
class MRXLinkMagics(LoggingMixin, Magics):
    ...
    def __init__(self, *args: List[Any], **kwargs: Dict[str, Any]) -> None:
        ...
        self.link_ipykernel_id: str = get_kernel_id()

        self.link_license: MRXLinkMagicsLicenseChecker = MRXLinkMagicsLicenseChecker()
        self.link_pipeline_callback: MRXLinkPipelineCallback = MRXLinkPipelineCallback(
            endpoint="http://localhost:8888"
        )
        self.link_pipeline: MRXLinkPipeline = MRXLinkPipeline(
            identifier=self.link_ipykernel_id, callback=self.link_pipeline_callback, shell=get_ipython()
        )
    ...
    @line_magic
    def mrxlink_set_components_to_execute_once(self, line: str) -> None:
        """Set queue."""
        args = parse_argstring(self.mrxlink_set_components_to_execute_once, line)
        commands.mrxlink_set_components_to_execute_once(link_magic=self, args=args)

    @magic_arguments()
    @line_magic
    def mrxlink_reset_components_to_execute_once(self, line: str) -> None:
        """Reset queue."""
        args = parse_argstring(self.mrxlink_reset_components_to_execute_once, line)
        commands.mrxlink_reset_components_to_execute_once(link_magic=self, args=args)
