...
class Insights:
    ...
    def _to_df(resp: Response, strip_html: bool) -> DataFrame:
        ...
        if strip_html:
            if "headline" in df.columns:
                ...
                df["headline"] = df["headline"].apply(lambda s: html.unescape(s).replace("\n", " "))  # type: ignore
            if "body" in df.columns:
                df["body"] = df["body"].str.replace(r"<.*?>", "", regex=True)  # type: ignore
                df["body"] = df["body"].apply(lambda s: html.unescape(s).replace("\n", " "))  # type: ignore
            if "lead" in df.columns:
                df["lead"] = df["lead"].str.replace(r"<.*?>", "", regex=True)  # type: ignore
                df["lead"] = df["lead"].apply(lambda s: html.unescape(s).replace("\n", " "))  # type: ignore
    ...
    def get_heards(
        ...
        *,
        ...
        --------
        **Free text search**
        >>> Insights().get_heards(q="Suez")

        **Stripping HTML Tags**
        >>> Insights().get_heards(q="Suez", strip_html=True)

        **Including other fields**
        >>> Insights().get_heards(q="Suez", field="body,commodity,geography")

        **Using List**
        >>> from datetime import datetime
        >>> Insights().get_heards(page_number=["0300", "0100"], service_line="PCA", updated_date_gte=datetime(2023,3,1))

        **Using Enum**
        >>> Insights().get_heards(content_type=Insights.HeardsContentType.Tenders)
        """
    ...
    def get_stories(
        ...
        **Stripping HTML Tags**
        >>> Insights().get_stories(q="Suez", strip_html=True)
    ...
    def get_subscriber_notes(
        ...
        **Stripping HTML Tags**
        >>> Insights().get_subscriber_notes(q="steel", strip_html=True)
