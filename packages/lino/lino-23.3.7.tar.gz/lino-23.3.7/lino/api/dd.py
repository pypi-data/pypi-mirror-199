    ...
    class e_catcher:
        ...
        def __call__(self, *args, **kwargs):
            try:
                ...
                logging.getLogger('linod').exception(e)
                from channels.layers import get_channel_layer
                from asgiref.sync import async_to_sync
                from lino.modlib.linod.utils import LINOD
                async_to_sync(get_channel_layer().send)(LINOD, {'type': 'deferred.job', 'name': fn.__name__})
...
def schedule_often(every=10):
    def decorator(func):
        if settings.SITE.use_linod:
            schedule.every(every).seconds.do(job_schedule(func))
        return func
    return decorator
...
def schedule_daily(at="20:00"):
    def decorator(func):
        if settings.SITE.use_linod:
            schedule.every().day.at(at).do(job_schedule(func))
        return func
    return decorator
