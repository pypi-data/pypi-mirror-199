...
'</td>')""" % (rh.actor.row_height * 11)
    ...
    def js_render_wrapped_action(self, rh, ba):
        ...
        if isinstance(original_ba.action, ShowDetail):
            yield "\tvar pk = panel.get_current_record().id;"
            yield "\tif (pk === -99998) pk = 0;"
            yield "\tLino.%s.run(null, {record_id: pk});" % original_ba.full_name()
        elif isinstance(original_ba.action, (ShowTable, ShowEmptyTable, ShowInsert)):
            yield "\tLino.%s.run(null);" % original_ba.full_name()
        else:
            if original_ba.action.select_rows:
                h = "\tLino.row_action_handler(%s, %s, %s)(panel, btn, step);"
            else:
                h = "\tLino.list_action_handler(%s," % py2js(
                    original_ba.actor.actor_url()) + " %s, %s, %s)(panel, btn, step);"
            yield h % (py2js(original_ba.action.action_name),
                py2js(original_ba.action.http_method),
                original_ba.action.preprocessor)
        yield "};"
