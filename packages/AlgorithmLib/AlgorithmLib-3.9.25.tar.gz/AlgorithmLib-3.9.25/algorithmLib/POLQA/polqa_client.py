...
from commFunction import getip,log_time,global_result,sftp_connect,sftp_put,sftp_disconnect,get_file_path,get_data_array,get_rms,make_out_file
import shutil
...
def polqa_client_test(src,test,samplerate,mode=0):
    ...
    if mode == 1:
        curFileName = curpath + '//' + curdata['testFile']
        curaudiodata,curfs,curch = get_data_array(curFileName)
        ...
        make_out_file(curFileName,urframe,curfs,curch)
...
if __name__ == '__main__':
    ...
    print(sys.platform)
    polqa_client_test(src='src.wav',test='2.wav',samplerate=48000,mode=1)
