...
with operations.session_scope() as db_session:
    ...
    assistant_user_query_id = 1

    assistant_user_query_instance = (
        db_session.query(AssistantUserQueryModel)
        .filter(
            AssistantUserQueryModel.session_id == assistant_session_id,
            AssistantUserQueryModel.id == assistant_user_query_id,
        )
        .first()
    )
    pprint(AssistantUserQueryResourceSchema().dump(assistant_user_query_instance))
