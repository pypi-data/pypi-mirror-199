...
# https://github.com/starkware-libs/stark-perpetual/tree/d1edb2203440b54e71458b10bf0bb646d8597caf         #
##########################################################################################################
...
# PEDERSEN_HASH_POINT_FILENAME='pedersen_params.json'
...
def verify(msg_hash: int, r: int, s: int, public_key: Union[int, ECPoint]) -> bool:
    ...
    if isinstance(public_key, int):
        ...
        return verify(msg_hash, r, s, (public_key, y)) or verify(msg_hash, r, s, (public_key, (-y) % FIELD_PRIME))
    else:
...
def get_price_msg(oracle_name: int, asset_pair: int, timestamp: int, price: int, hash_function=pedersen_hash):
