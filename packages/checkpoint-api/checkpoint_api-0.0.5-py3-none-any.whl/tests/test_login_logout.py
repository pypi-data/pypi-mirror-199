...
from checkpoint_api.exceptions import CheckpointMissingCredentialsException
...
def test_login_credentials():
    with pytest.raises(CheckpointMissingCredentialsException):
        with CheckpointSession(
                "10.100.30.25", logfile=""
        ) as cps:
            pass
    with pytest.raises(CheckpointMissingCredentialsException):
        with CheckpointSession(
                "10.100.30.25", "test_user", logfile=""
        ) as cps:
            pass
...
def test_login_handling_03():
    ...
    with CheckpointSession(
            "10.100.30.25", "api", "pass", domain="CPP", logfile=""
    ) as cps:
        assert cps.sid == login_json_ok["sid"]
...
def test_unmatched_endpoint_raises_connection_error():
    ...
    with pytest.raises(requests.exceptions.ConnectionError):
        with CheckpointSession(
                "10.100.30.251", "api", "pass", logfile=""
        ) as cps:
            pass
...
def test_unmatched_endpoint_raises_generic_error():
    ...
    with pytest.raises(Exception):
        with CheckpointSession(
                "10.100.30.25", "api", "pass", domain="CPP2", logfile=""
        ) as cps:
            pass
...
def test_using_a_callback_for_dynamic_responses():
    ...
    responses.add_callback(
        responses.POST, "https://10.122.0.112/web_api/login",
    ...
    with pytest.raises(Exception) as excinfo:
        with CheckpointSession(
                "10.122.0.112", "api", "pass", domain="CPP2", logfile=""
        ) as cps:
            pass
