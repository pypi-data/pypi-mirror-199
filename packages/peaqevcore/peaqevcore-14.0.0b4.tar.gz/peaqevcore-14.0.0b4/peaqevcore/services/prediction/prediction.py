...
class Prediction:
    ...
    @property
    async def predictedenergy(self) -> float:
        return await Prediction._predicted_energy(
            datetime.now().minute,
            datetime.now().second,
    ...
    @property
    async def predictedpercentageofpeak(self) -> float:
        if hasattr(self._hub.sensors, "current_peak"):
            return await Prediction._predicted_percentage_of_peak(
    ...
    @staticmethod
    async def _predicted_percentage_of_peak(
            peak: float,
