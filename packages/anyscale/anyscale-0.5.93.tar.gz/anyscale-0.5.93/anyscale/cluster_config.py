...
def _with_safe_cluster_config(
    ...
    instance: Optional[Any],  # noqa: ARG001
    args: Any,
    kwargs: Any,
) -> Any:
...
def _configure_cluster_name(
    ...
    if api_client:
        ...
        name = send_json_request(f"/api/v2/sessions/{session_id}/details", {})[
            "result"
        ]["cluster_name"]
...
def _configure_ssh_key(
    ...
    if api_client:
        ...
        ssh_key = send_json_request(f"/api/v2/sessions/{session_id}/ssh_key", {})[
            "result"
        ]
        key_path = _write_ssh_key(ssh_key["key_name"], ssh_key["private_key"])
...
def _configure_autoscaler_credentials(
    ...
    if api_client:
        ...
        credentials = send_json_request(
            f"/api/v2/sessions/{session_id}/autoscaler_credentials", {},
        )["result"]["credentials"]

    cloud_provider = cluster_config["provider"]["type"]
    cluster_config["provider"][f"{cloud_provider}_credentials"] = credentials
...
def configure_for_session(
    ...
    _DO_NOT_USE_RAY_UP_ONLY_cluster_config: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """
    ...
    if _DO_NOT_USE_RAY_UP_ONLY_cluster_config is not None:
        ...
        if api_client:
            ...
            _resp: Dict[str, Any] = send_json_request(
                f"/api/v2/sessions/{session_id}/cluster_config", {}
            )["result"]
