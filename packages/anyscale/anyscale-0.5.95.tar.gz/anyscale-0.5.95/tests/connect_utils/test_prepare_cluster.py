...
def test_get_cloud_id(
    ...
    mock_get_organization_default_cloud = Mock(
        return_value="default_cloud_name" if default_cloud_exists else None
    )
    mock_get_last_used_cloud = Mock(return_value="last_used_cloud_name")
    ...
    if cloud_name:
        ...
        mock_get_last_used_cloud.assert_called_once_with(
            "test_project_id", mock_prepare_cluster_block.anyscale_api_client
        )
        mock_get_cloud_id_and_name.assert_called_once_with(
