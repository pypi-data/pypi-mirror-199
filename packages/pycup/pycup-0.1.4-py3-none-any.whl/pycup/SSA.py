...
def PDUpdate(X, PDNumber, ST, Max_iter, dim):
    ...
    PDNumber: proportion of producers -> int
    ST: alarm value -> float, typically 0.6
    Max_iter: maximum num. of iterations -> int
    dim: dimension(num. of parameters) -> int

    :return
    X_new: The updated population -> np.array, shape = (pop, dim)
    """
    X_new = copy.copy(X)
    R2 = random.random()
...
def SDUpdate(X, pop, SDNumber, fitness, BestF):
    ...
    X: samples -> np.array, shape = (pop, dim)
    pop: population size -> int
    SDNumber: proportion of scouters -> int
    fitness: the fitness of population in the last iteration
...
def run(pop, dim, lb, ub, MaxIter, fun, RecordPath = None, args=()):
    ...
    for t in range(iter,MaxIter):
        ...
        X = PDUpdate(X, PDNumber, ST, MaxIter, dim)
...
def runMP(pop, dim, lb, ub, MaxIter, fun, n_jobs, RecordPath = None, args=()):
    ...
    lb = -100 * np.ones(30)
    ub = 100 * np.ones(30)
    cp.SSA.runMP(pop = 1000, dim = 30, lb = lb, ub = ub, MaxIter = 30, fun = uni_fun1, n_jobs = 5)
    """
    print("Current Algorithm: SSA (Multi-Processing)")
    print("Elite Opposition:{}".format(EliteOppoSwitch))
    print("Iterations: 1 (init.) + {}".format(MaxIter))
    print("Dim:{}".format(dim))
    print("Population:{}".format(pop))
...
def run_MV(pop, dims, lbs, ubs, MaxIter, fun,RecordPath = None, args=()):
    ...
    """
    print("Current Algorithm: SSA (Multi-Variables)")
    ...
    for t in range(iter,MaxIter):
        ...
        for n in range(num_var):
            BestF = fitness[n][0]
            X[n] = PDUpdate(X[n], PDNumber, ST, MaxIter, dims[n])

            X[n] = JDUpdate(X[n], PDNumber, pop, dims[n])
