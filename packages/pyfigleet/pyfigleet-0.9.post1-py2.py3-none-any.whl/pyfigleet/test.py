...
class Test(object):
    ...
    def outputUsingFigletorToilet(self, text, font, fontpath):
        if os.path.isfile(fontpath + '.flf'):
            cmd = ('figlet', '-d', 'pyfigleet/fonts', '-f', font, text)
        elif os.path.isfile(fontpath + '.tlf'):
            cmd = ('toilet', '-d', 'pyfigleet/fonts', '-f', font, text)
        else:
    ...
    def validate_font_output(self, font, outputFiglet, outputpyfigleet):
        if outputpyfigleet == outputFiglet:
            win('[OK] %s' % font)
            self.ok += 1
        ...
        self.show_result(outputFiglet, outputpyfigleet, font)

    def show_result(self, outputFiglet, outputpyfigleet, font):
        if self.opts.show is True:
            print('[PYTHON] *** %s\n\n' % font)
            dump(outputpyfigleet)
            print('[FIGLET] *** %s\n\n' % font)
            dump(outputFiglet)
            raw_input()
    ...
    def check_font(self, text, font):
        if font in self.skip:
            return
        fontpath = os.path.join('pyfigleet', 'fonts', font)

        self.f.setFont(font=font)

        outputpyfigleet = self.f.renderText(text)
        ...
        if not strict:
            outputpyfigleet = outputpyfigleet.strip('\n')
            outputFiglet = outputFiglet.strip('\n')

        self.validate_font_output(font, outputFiglet, outputpyfigleet)
