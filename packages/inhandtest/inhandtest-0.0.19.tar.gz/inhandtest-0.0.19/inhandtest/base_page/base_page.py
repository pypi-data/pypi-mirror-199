...
from inhandtest.base_page._table_tr import Table
import allure
import logging
import re
...
class BasePage:
    ...
    def __init__(self, host: str, username: str, password: str, protocol='https',
                 port=443, model='VG710', language='en', playwright_=None, **kwargs):
        """
        ...
        :param kwargs:  'page'|'http_credentials'|'browser'
                      ...
                      http_credentials: True|False http页面认证，比如VG710该设备就应该开启
        ...
        self.__http_credentials = kwargs.get('http_credentials')  # 开启该项后，表示用户名和密码需要配置到浏览器的context， 在这里用户名和密码就无效
    ...
    @allure.step("用户登录")
    def login(self, username=None, password=None, status='success'):
        """
    ...
    def access_menu(self, menu: str, wait_time=None):
    ...
    def fill(self, locator: Locator, value: str or int, log_desc=None, force=False):
        """ 封装公共的输入操作
    ...
    def click(self, locator: Locator, log_desc=None, dialog_message: str = None, tip_messages: str or list = None,
              wait_for_time: int = None, tip_messages_timeout=30):
        """ 封装公共的点击操作
    ...
    def check(self, locator: Locator, action='check', log_desc=None, tip_messages: str or list = None):
        """ 封装公共的单选操作
        :param locator:  元素定位
        :param log_desc: 功能描述，用英文 如 Static Routing Destination
    ...
    @allure.step('下拉选择')
    def select_option(self, locator: Locator, value: str or int, log_desc=None):
        """ 封装公共的下拉选择操作
    ...
    @allure.step('{log_desc} {action}')
    def switch_button(self, locator: Locator, action: str = 'enable', log_desc=None):
        """控制通用开关按钮开关， 如拨号的开关
    ...
    @allure.step('伸缩按钮')
    def expand(self, left_text: str, action: str = 'expand'):
        """ 伸缩按钮
    ...
    @allure.step('上传文件')
    def upload_file(self, locator: Locator, path_, dialog_massage=None):
        """
    ...
    @allure.step('下载文件')
    def download_file(self, locator: Locator, file_path, file_name=None):
        """默认为日志路径
        ...
        if file_path is not None:
            if os.path.isdir(file_path) and os.path.exists(file_path):
                ...
                logging.info(f'Device {self.host} download {download.suggested_filename} to path {file_path} successful')
            else:
                logging.error(f'{file_path} Does Not Exist.')

    @allure.step("校验dialog message")
    def dialog_massage(self, f, message: str = ''):
    ...
    def tip_messages(self, messages: str or list = None, timeout=30):
    ...
    def text_messages(self, messages: str or list = None, timeout=10):
    ...
    def title_messages(self, messages: str or list = None, timeout=10):
        """ 对元素的属性title做内容验证，
        :param messages: str or list 文本内容，如果有多个，使用列表传入
        :param timeout: 校验超时时间
        :return:
    ...
    @allure.step('设置页面翻页')
    def page_refresh(self, refresh_time: str, select_locator: Locator):
        """
    ...
    def table_tr(locators: dict, value: list, log_desc=None):
        """
    ...
    def agg_in(self, locators: list, action_dict: dict):
        """封装公共的整合输入操作
    ...
    def eval_inner_text(self, expect_: dict, locators: list):
        """对页面特定元素值做判断
    ...
    def pc_ping(self, src, host_or_ip: str or list or tuple = 'www.baidu.com', number: int = 4,
                lost_packets=False, assert_result=True):
        """ 验证在PC机上ping某个地址是否丢包， 仅判断丢包

        :param lost_packets:
        :param src: 验证的源IP地址 'lq28_s2'|'lq28'|'en00_s4'|'fq58'|'192.168.2.100'
        ...
        if host_or_ip:
            ...
            for host in host_or_ip:
                logging.info(f'command is ping -S {src} {host} -n {number}')
                if assert_result:
                    result = os.popen(f'ping -S {src} {host} -n {number}').read()
                    logging.info(result)
                    ...
                    os.popen(f'ping -S {src} {host} -n {number}')
                if not tag_result:
                    break
        return tag_result
...
if __name__ == '__main__':
    ...
    with sync_playwright() as playwright:
        my_base = BasePage('10.5.24.206', 'adm', '123456', 'http', 80, 'VG710', 'en', playwright, http_credentials=True,
                           )
        user_table = Table([('name', 'input'), ('value', 'input')],
                           my_base.page.frame_locator('#window_content').locator('#userdata-grid'), ['name'])
        my_base.access_menu('app_user_data.user_data_management')
        user_table.edit({'name': 'test2', 'value': 222}, {'name': 'test1111', 'value': '333333'})
