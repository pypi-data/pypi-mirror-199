...
class DmInterface:
    ...
    def delete_device(self, sn: str or list):
        """
...
class IcsInterface(DmInterface):
            ...
            if response.get('total') == 1:
                ...
                result.append(
                    {'sn': sn_, 'online': res.get('online'), 'iccid': res.get('metadata').get('iccid'),
                     ...
                     'id': res.get('id'), 'connected': res.get('connected')})
            else:
                result.append(
                    {'sn': sn_, 'online': None, 'iccid': None, 'imei': None, 'imsi': None, 'swVersion': None,
                     'hwVersion': None, 'bootVersion': None, 'id': None, 'connected': None})
        return result
    ...
    def send_openvpn_config(self, sn: str):
