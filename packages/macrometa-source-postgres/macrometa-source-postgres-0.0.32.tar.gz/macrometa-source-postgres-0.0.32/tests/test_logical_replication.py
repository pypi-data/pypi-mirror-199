...
from macrometa_source_postgres.sync_strategies.logical_replication import UnsupportedPayloadKindError
...
class TestLogicalReplication(unittest.TestCase):
    ...
    def test_locate_replication_slot_by_cur(self):
        ...
        cursor = PostgresCurReplicationSlotMock(existing_slot_name='macrometa_some_db')
        ...
        cursor = PostgresCurReplicationSlotMock(existing_slot_name='macrometa_some_db_some_tap')
        self.assertEqual(logical_replication.locate_replication_slot_by_cur(cursor,
                                                                            'some_db',
                                                                            'some_tap'),
                         'macrometa_some_db_some_tap')
    ...
    @patch('macrometa_source_postgres.logical_replication.singer.write_message')
    @patch('macrometa_source_postgres.logical_replication.sync_common.send_schema_message')
    @patch('macrometa_source_postgres.logical_replication.refresh_streams_schema')
...
class TestAdditionalLogicalReplication(unittest.TestCase):
    ...
    @patch("macrometa_source_postgres.sync_strategies.logical_replication.get_pg_version")
    @patch("psycopg2.connect")
    def test_fetch_current_lsn(self, mocked_connect, mocked_pg_version):
        ...
        # Look at macrometa_source_postgres.sync_strategies.logical_replication.lsn_to_int to find out why!
    ...
    def test_add_automatic_properties_if_debug_lsn_is_off(self):
        ...
        with self.assertLogs(level='DEBUG') as foo_log:
            ...
            self.assertEqual(['DEBUG:macrometa_source_postgres:debug_lsn is OFF'], foo_log.output)
    ...
    def test_add_automatic_properties_if_debug_lsn_is_on(self):
        ...
        with self.assertLogs(level='DEBUG') as foo_log:
            ...
            self.assertDictEqual(expected_stream, actual_stream)
            self.assertEqual(['DEBUG:macrometa_source_postgres:debug_lsn is ON'], foo_log.output)
    ...
    def test_locate_replication_slot(self, mocked_connect):
        ...
        expected_output = f'macrometa_{self.conn_info["dbname"]}'
    ...
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.refresh_streams_schema')
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.sync_common.send_schema_message')
    ...
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.locate_replication_slot')
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.get_pg_version')
    @patch("psycopg2.connect")
    ...
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.datetime.datetime')
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.locate_replication_slot')
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.get_pg_version')
    @patch("psycopg2.connect")
    ...
    def test_sync_tables_if_break_at_end_lsn_and_msg_data_start_greater_than_end_lsn(self,
        ...
        expected_log_message = 'INFO:macrometa_source_postgres:Breaking - latest wal message ' \
    ...
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.locate_replication_slot')
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.get_pg_version')
    @patch('macrometa_source_postgres.sync_strategies.logical_replication.select')
    @patch("psycopg2.connect")
