...
class DAT:
    ...
    *_indexs: {uid_str: _Cards}
    ...
    dc_bus_card_set = {'BZ', 'BZ+', 'BA', 'BA1', 'BA2', 'BB'}
    ...
    def describe(self):
        ...
        print(f"节点统计: {len(self.bus_indexs)}, \n" +
              textwrap.fill(f"{', '.join([str(k) + 'kV及以上节点数: ' + str(v) for k, v in vol_count.items() if v > 0])}",
                            width=120) +
              f"其中，交流节点卡: {Counter([b[0].type for b in ac_bus])}\n" +

              f"其中，直流节点卡: {Counter([b[0].type for b in dc_bus])}")
        print(f'\t参与统计的卡类型: {self.bus_card_set}')
        ...
        print(f"支路统计: {len(self.branch_indexs)}, 支路卡总计: {len(self.branch)}" +
              f"\n参与计算的支路卡统计  : {bt1}" +
              f'\n不参与计算的支路卡统计: {bt2}' +
              f"其中，交流支路卡: {Counter([b[0].type for b in ac_branch])}\n" +

              f"其中，直流支路卡: {Counter([b[0].type for b in dc_branch])}")
