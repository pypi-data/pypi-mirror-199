...
from ..utils.utils import LHSGenerator, TopoDataGenerator, IslandFilter
import psutil
...
def subprocess_call(key: str, mode: str, *args, **kwargs):
    ...
    time.sleep(time_resolution * 0.1)
    # 第3版，
    outs, errs = None, None
    try:
        for step in range(int(time_tolerance // time_resolution)):
            ...
            if hwnd != 0:
                btnhdl = win32gui.FindWindowEx(hwnd, None, 'Button', '是(&Y)')
...
if __name__ == '__main__':
    ...
    id1 = '7'
    for id2 in ['pt0', 'pt1', 'pt2', 'pt3', 'pt4', 'pt5']:
        prj_dir = f'E:\Data/transient_stability/39/bpa/34_n90/{id1}/{id2}/'
        prj = NRSCPProject(prj_dir,
                           base_dat=DAT.build_from(prj_dir), base_swi=SWI.build_from(prj_dir), base_lsd=LSD.build_from(prj_dir),  # 在项目里面预先放入
                           project_name='milktest',
                           organizer=DefaultOrganizer)
