...
class Typer:
    ...
    def Print(message: any, Refresh: Optional[bool] = None, Enter: Optional[bool] = None, isTimer: Optional[bool] = None): # type: ignore
        if Refresh is None and Enter is None:
            raise Error('Refresh True or False / Enter True or False')
        if Enter is True:
    ...
    def Line(Normal: Optional[bool] = None, Bold: Optional[bool] = None):
        if Normal is None and Bold is None:
            raise Error('Normal True or False / Bold True or False')
        if Normal is True:
            sys.stdout.write(
        ...
        if Bold is True:
            sys.stdout.write(
            f'{Typer.Parse_Text_Line("=", Bold=True)}{Color.BLUE}[{Color.YELLOW}{datetime.datetime.now().strftime("%d/%m/%Y, %H:%M:%S")}{Color.BLUE}]{Color.WHITE}{Typer.Parse_Text_Line("=", Bold=True)}{Color.WHITE}\n')
    ...
    def Input(message: any, Integer: Optional[bool] = None, String: Optional[bool] = None): # type: ignore
        if Integer is None and String is None:
            raise Error('Integer True or False / String True or False')
        if Integer is True:
            while True:
                try:
                    return int(input(f'{Color.WHITE}{message}{Color.WHITE}'))
                except KeyboardInterrupt:
                    raise Stoper('Exit')
        if String is True:
            while True:
                try:
                    return str(input(f'{Color.WHITE}{message}{Color.WHITE}'))
                except KeyboardInterrupt:
                    raise Stoper('Exit')