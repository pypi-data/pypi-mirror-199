...
class Scraper(object):
    ...
    def getImageList(self, deep):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_url = {
                executor.submit(self.makeImageList, num, deep): num for num in range(5)
            }
            for future in concurrent.futures.as_completed(future_to_url):
                num = future_to_url[future]
                try:
    ...
    def makeImageList(self, num, deep):
        """
        ...
        while count > 0:
            for url in z:
                if not deep and round(time.time()) - int(str(url["upload_date"])[:-3]) > 1209600:
                    continue
                if latestCache is not None:
                    if (
                        str(url["upload_date"])[:-3]
...
def parser():
    ...
    parser.add_argument(
        ...
        "--deep",
        action="store_true",
        help="Deep scrape of user with an infinite lookback",
    )
...
def main():
    ...
    if args.getImages:
        scraper = Scraper(args.username)
        if args.deep:
            scraper.getImages(True)
        else:
            scraper.getImages(False)
        os.chdir(vsco)
    ...
    if args.multiple:
        ...
        for z in y:
            try:
                os.chdir(vsco)
                if args.deep:
                    Scraper(z).getImages(True)
                else:
                    Scraper(z).getImages(False)
                print()
            except:
                print("%s crashed" % z)
                pass
