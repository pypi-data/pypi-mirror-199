...
from .const import HEATING_MANUFACTURER, VIA_DEVICE, HEATING_DEVICE_TYPE
...
class SC2XMLReader:
    ...
    def __init__(self, url: str, username: str, password: str, withWarmWaterStation=True, withSolar=True, withEastWest=False, withOven=False) -> None:
        ...
        self.model = HEATING_DEVICE_TYPE.SolvisMax_6.name
        self.name = None
    ...
    def create_array_from_data(self, data_sc2: str):
        ...
        data_array["type"] = self.create_data_entry(
                    "type", "Devicetype", value_int, ""
                )
        data_sc2 = data_sc2[4:]

        # Systemnummer
        value_int = self.convert_data_to_int(data_sc2, 4)
        data_array["number"] = self.create_data_entry(
                    "number", "Devicenumber", value_int, ""
                )
        data_sc2 = data_sc2[4:]
