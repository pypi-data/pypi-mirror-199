...
class InteractiveTable(widgets.DOMWidget):
    """
        ...
        Parameters:
            ...
            - render_html:
                Renders HTML text iside the table. Defaults to False.
    ...
    render_html = _Bool(False).tag(sync=True)
    ...
    # Basic validator for the floater value
    @_validate('render_html')
    def _valid_render_htmlt(self, proposal):
        if isinstance(proposal['value'], bool):
            return proposal['value']
        raise _TraitError('Invalid render_html. Provide Boolean value')
