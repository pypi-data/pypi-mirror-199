...
var InteractiveTableView = widgets.DOMWidgetView.extend({
    createElements: function(data, td) {
      var self = this;
      var render = self.model.get('render_html');
      if(render){
        console.log(data)
        let parser = new DOMParser();
        let doc = parser.parseFromString(data, 'text/html');
        let collection = doc.getElementsByTagName('body');
        let nodes = collection[0].childNodes;
    ...
    render: function() {
      ...
      $.each(table_data,(i, value) => {
        ...
        Object.keys(value).forEach(function(key) {
          ...
          if(key == "group"){
            ...
            td = self.createElements(temp_value.replace('&lt;','<').replace('&gt;', '>'), td);
          }else{
            td = self.createElements(temp_value, td);
          }
      ...
      $.each(self.cols,(i, value) => {
        ...
        if(value != "group" && value != "over_length"){
          ...
          append_first = true;
          td = self.createElements(value.replace('&lt;','<').replace('&gt;', '>'), td);
          tr.appendChild(td)
        }
        else{
          append_first = true;
          ...
          function format ( d, index ) {
            ...
            $.each(d, (i, value) => {
              if(value.length > self.model.get("text_limit")){
                ...
                td_col = self.createElements(column_index[i], td_col);
                td_value = self.createElements(value.replace('&lt;','<').replace('&gt;', '>') , td_value);
            ...
            if(t_row.hasOwnProperty('group')){
              $.each(t_row["group"], (i, value) => {
                if(!child_select){
                  ...
                  td_col = self.createElements(i, td_col);
                  td_value = self.createElements(value.replace(/&lt;/g, '<').replace(/&gt;/g, '>'), td_value);
                  ...
                  label = self.createElements(" "+value.replace('&lt;','<').replace('&gt;', '>'), label)
