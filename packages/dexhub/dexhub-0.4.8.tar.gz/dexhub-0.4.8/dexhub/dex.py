...
class JoeV2Route:
    ...
    def get_id_from_price(_web3:Web3,_route_address:str,_lb_pair,_price):
        return _web3.eth.contract(address=_route_address, abi=JoeV2Route.joe_route_v2_abi).functions.getIdFromPrice(_lb_pair,_price).call()

    def get_price_from_id(_web3:Web3,_route_address:str,_lb_pair,_id):
        return _web3.eth.contract(address=_route_address, abi=JoeV2Route.joe_route_v2_abi).functions.getPriceFromId(_lb_pair,_id).call()

    #return [amount_in,fee]
    def get_swap_in(_web3:Web3,_route_address:str,_lb_pair,_amount_out,_swap_for_y:bool):
        return _web3.eth.contract(address=_route_address, abi=JoeV2Route.joe_route_v2_abi).functions.getSwapIn(_lb_pair,_amount_out,_swap_for_y).call()
    
    #return [amount_out,fee]
    def get_swap_out(_web3:Web3,_route_address:str,_lb_pair,_amount_in,_swap_for_y:bool):
        return _web3.eth.contract(address=_route_address, abi=JoeV2Route.joe_route_v2_abi).functions.getSwapOut(_lb_pair,_amount_in,_swap_for_y).call()

    def swap_exact_tokens_for_tokens(_web3:Web3,_route_address:str,
                                     _amount_in,_amount_out_min,_pair_bin_steps,_token_path,_to,_deadline,
                                     _public_key,_private_key):
        nonce=_web3.eth.get_transaction_count(_public_key)
        route_contract=_web3.eth.contract(address=_route_address, abi=JoeV2Route.joe_route_v2_abi)
    ...
    def swap_tokens_for_exact_tokens(_web3:Web3,_route_address:str,
