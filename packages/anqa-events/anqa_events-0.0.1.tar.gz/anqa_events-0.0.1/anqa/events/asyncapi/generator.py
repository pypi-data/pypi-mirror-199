...
def get_all_models_schema(service: MessageService):
    all_models = [
        m.event_type  # type: ignore
        for m in chain(service.consumers, PUBLISH_REGISTRY.values())
    ]
    return all_schemas(all_models, ref_prefix=PREFIX).get("definitions", [])


def populate_channel_spec(service: MessageService):
...
def get_async_api_spec(service: MessageService) -> AsyncAPI:
