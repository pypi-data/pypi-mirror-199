...
from anqa.core.mixins.logger import LoggerMixin

if TYPE_CHECKING:
    from anqa.events import Broker, CloudEvent, Consumer, MessageService

    from .types import RawMessageT


class MessageMiddleware(LoggerMixin):
    ...
    async def before_service_start(self, broker: Broker, service: MessageService):
        """Called before service starts"""

    async def after_service_start(self, broker: Broker, service: MessageService):
        """Called after service starts"""

    async def before_service_stop(self, broker: Broker, service: MessageService):
        """Called before service stops"""

    async def after_service_stop(self, broker: Broker, service: MessageService):
        """Called after service stops"""

    async def before_consumer_start(
        self, broker: Broker, service: MessageService, consumer: Consumer
    ) -> None:
