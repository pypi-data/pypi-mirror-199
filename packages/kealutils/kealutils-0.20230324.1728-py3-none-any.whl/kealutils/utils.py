...
def get_katis_env() -> dict():
    ...
    # 실행환경 설정 파일 읽기
    katis_file_path = get_katis_json_file_path()
...
def get_katis_json_file_path() -> str:
    ...
    katis_json_file_path = ""
    # while True:
    for ii in range(5):
        # print("[{0}] {1}".format(ii, curr_dir))

        if (os.path.isfile(curr_dir + "/" + KATIS_CONFIG_FILE_NAME) == True):
            katis_json_file_path = os.path.join(
                curr_dir, KATIS_CONFIG_FILE_NAME)
            # print("찾았다!", katis_json_file_path)
            break
        else:
            ...
            if (curr_dir == os.path.abspath(os.path.join(curr_dir, '..'))):
                # print("못찾았다!")
                raise Exception(KATIS_CONFIG_FILE_NAME + " 파일을 찾을 수 없습니다.")
                break
            else:
                curr_dir = os.path.abspath(os.path.join(curr_dir, '..'))

    # print("get_katis_json_file_path() -> [{0}]".format(katis_json_file_path))
    return katis_json_file_path
