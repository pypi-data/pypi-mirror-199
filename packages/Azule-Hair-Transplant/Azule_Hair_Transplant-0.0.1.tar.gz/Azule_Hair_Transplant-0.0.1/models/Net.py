...
class Net(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.generator = Generator(
            opts.size, opts.latent, opts.n_mlp, channel_multiplier=opts.channel_multiplier)
        self.cal_layer_num()
        self.load_weights()
        self.load_PCA_model()
    ...
    def build_PCA_model(self, PCA_path):
        ...
        with torch.no_grad():
            ...
            pulse_space = torch.nn.LeakyReLU(5)(
                self.generator.style(latent)).numpy()
        ...
        np.savez(PCA_path, X_mean=X_mean, X_comp=X_comp,
                 X_stdev=X_stdev, X_var_ratio=X_var_ratio)
    ...
    def load_PCA_model(self):
        ...
        self.X_stdev = torch.from_numpy(
            PCA_model['X_stdev']).float().to(device)
