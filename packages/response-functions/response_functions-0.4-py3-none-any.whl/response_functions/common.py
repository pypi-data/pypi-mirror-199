...
def delta_function(x, eta, shape="fermi"):
    ...
    if shape == 'wigner':
        return 2./(np.pi * eta**2) * np.real(np.sqrt(eta**2 - x**2 +0.j))

def theta_function(x, eta, shape="fermi"):
    """returns different types of function converging to a Dirac delta
    function as the parameter eta goes to 0"""
    if shape == "fermi":
        return 0.5 * (1 + np.tanh(x/(2.*eta)))
    if shape == "lorentz":
        return 0.5 + 1/np.pi * np.arctan(x/eta)
    if shape == "gauss":
        return 0.5 * (1 + erf(x/(np.sqrt(2)*eta)))
    if shape == 'wigner':
        return 0.5 + 1/np.pi*np.real(np.arcsin(x/eta+0.j)+ x/eta**2*np.sqrt(eta**2-x**2+0.j))

def average_maldague(funct, 
                     chemical_potential, 
                     temperature, 
                     sampling = None, 
                     weights = None, 
    ...
    if (sampling is None) and (weights is None):
        if quadrature == 'gauss-legendre':
            ...
            y = np.linspace(-1+1./num,1-1./num,num)
            sampling = 2 * np.arctanh(y)
            weights = np.ones([num])/num
        else:
            raise ValueError('Unknown quadrature type {}'.format(quadrature))
    I = np.tensordot(weights, np.array([funct(chemical_potential + temperature * xi) for xi in sampling]), axes = 1)
    return I
