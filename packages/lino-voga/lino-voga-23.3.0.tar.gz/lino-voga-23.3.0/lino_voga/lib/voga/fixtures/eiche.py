...
class Loader1(object):
    ...
    def objects(self):
        ...
        self.course_fees = Category(**dd.str2kw(
            'name', _("Participation fees")))
        yield self.course_fees

        self.trips = Category(**dd.str2kw('name', _("Trips")))
        # et="Väljasõidud", de="Ausflüge", fr="Excursions"))
        yield self.trips

        kw = dd.str2kw('name', _("Journeys"))
        self.journeys_cat = Category(**kw)
        yield self.journeys_cat

        kw.update(sales_price="295.00")
        self.journey_fee = Product(category=self.journeys_cat, **kw)
        yield self.journey_fee

        rent = Category(**dd.str2kw('name', _("Room renting")))
        # et="Ruumiüür", de="Raummiete", fr="Loyer"))
        yield rent
        # other = Category(**dd.str2kw('name', _("Other")))
        ...
        product = Instantiator(
            'products.Product', "sales_price category name",
        ...
        self.PRICES = Cycler(Product.objects.filter(category=self.course_fees))
        ...
        yield self.kurse
        settings.SITE.site_config.update(default_event_type=self.kurse)
        # yield settings.SITE.site_config
        ...
        kw.update(dd.str2kw('name', _("Conference room")))
        self.konf = room(**kw)
        yield self.konf
...
class Loader2(Loader1):
    ...
    def objects(self):
        ...
        journey_options = Category(**dd.str2kw(
            'name', _("Hotel options")))
        yield journey_options
        option = Instantiator(Product, category=journey_options).build
        ...
        # trip_options = Category(**dd.str2kw('name', _("Trip options")))
        # yield trip_options
        # option = Instantiator(Product, category=trip_options).build
