...
def init_storage():
    segs = settings.STORAGE_CLASS.split(".")
    module_path = ".".join(segs[:-1])
    module = import_module(module_path)
    storage = getattr(module, segs[-1])
    return storage()
...
class BaseStorage:
    ...
    def url(self, filename):
        ...
        return f"{settings.STORAGE_PREFIX}/{settings.STORE_FOLDER}/{filename}"
...
class AliyunStorage(BaseStorage):
    def __init__(self, *args, **kwargs):
        ...
        self.config = getattr(settings, "ALIYUN", None)
    ...
    def path(self, filename):
        return os.path.join(settings.STORE_FOLDER, filename)
...
class AwsStorage(BaseStorage):
    def __init__(self, *args, **kwargs):
        ...
        self.config = getattr(settings, "AWS", None)
        if self.config is None:
