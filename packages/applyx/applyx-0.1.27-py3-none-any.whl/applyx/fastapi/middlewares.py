...
class RequestMiddleware(BaseHTTPMiddleware):
    ...
    def get_real_ip(self, request: Request):
        return request.headers.get("x-real-ip") or request.headers.get("x-forwarded-for") or request.client.host
        ...
        async def send_wrapper(message: Message, **kwargs) -> None:
            if message["type"] == "http.response.start":
                ...
                if scope["session"]:
                    ...
                    header_value = self._construct_cookie(
                        session_id=session_id, clear=False
                    )
