...
from typing import Tuple, Optional, Any, Union, Iterator, Mapping, MutableMapping, Sequence, MutableSequence, TypeVar, overload
import ovito.vis
...
```"""
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.trajectories: Optional[TrajectoryLines]
        ...
        self.trajectories_: Optional[TrajectoryLines]
...
Furthermore, it is possible to use the :py:meth:`!apply()` in the implementation of a user-defined modifier function
...
```"""
    ...
    "The :py:class:`TriangleMesh` object to be used as custom shape for rendering particles of this type. You can either programmatically create a :py:class:`TriangleMesh` from a list of vertices and faces and assign it to this field, or let the :py:meth:`load_shape` method read the shape mesh from a separate geometry file. Also some file readers (e.g. GSD and Aspherix) may generate the shape mesh automatically based on information found in the simulation file. \n\nThe :py:class:`ParticlesVis` element, which is responsible for visualizing a particle system, will render an instance of the mesh at each particle site, uniformely scaled by the particle's :py:attr:`radius`, translated by the coordinates taken from the ``Position`` particle property, and rotated by the quaternion transformation taken from the ``Orientation`` particle property. \n\nNote: This mesh will be ignored by the :py:class:`ParticlesVis` element unless the type's :py:attr:`shape` is set to ``ParticlesVis.Shape.Mesh``. \n\nDefault: ``None``"

    def load_defaults(self) -> None:
        """Given the type's chemical :py:attr:`name`, which must have been set before calling this method, initializes the type's :py:attr:`color`, :py:attr:`radius`, :py:attr:`vdw_radius` and , :py:attr:`mass` fields with default values from OVITO's internal database of chemical elements. This method is useful when creating new atom types while building up a molecule structure."""
        ...
...
The name may contain spaces, digits, or special characters, but no dots, because ``.`` is used in OVITO as a delimiter for vector :py:attr:`component_names`."""
        ...
...
OVITO has several concrete implementations of the abstract :py:class:`PropertyContainer` interface: 
    ...
    * :py:attr:`SurfaceMesh.regions`"""
    ...
    def create_property(self, name: str, dtype: Optional[DTypeLike]=None, components: Optional[int]=None, data: Optional[ArrayLike]=None) -> Property:
...
```"""
    ...
    def remap_indices(self, particles: Particles, indices: Optional[Sequence[int]]=None) -> Union[NDArray[numpy.int64], slice]:
        """In case the storage order of atoms or particles changes during the course of a simulation, this method can determine the mapping of particles from one snapshot of the trajectory to another. It uses the unique :py:attr:`identifiers` of the particles to do that. 
...
A dislocation network can be written to a data file in the form of polylines using the :py:func:`ovito.io.export_file` function (select the ``vtk/disloc`` output format). During export, a non-periodic version is produced by clipping dislocation lines at the domain boundaries."""
    ...
    def set_segment(self, index: int, true_burgers_vector: Optional[ovito.vis.Vector3]=None, cluster_id: Optional[int]=None, points: Optional[ArrayLike]=None, custom_color: Optional[ovito.vis.Color]=None) -> None:
...
This visual element is accessible through the :py:attr:`~DataObject.vis` attribute of the base class."""
    vis: ovito.vis.TrajectoryVis = ovito.vis.TrajectoryVis()
    "The :py:class:`DataVis` element currently associated with this data object, which is responsible for visually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in rendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to multiple data objects in order to synchronize their visual appearance."
