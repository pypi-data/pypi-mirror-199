...
class CutoffNeighborFinder(ovito.nonpublic.CutoffNeighborFinder):
    ...
    def find_all(self, indices=None, sort_by=None):
        """
        Vectorized version of the :py:meth:`find` method, computing the neighbor lists and neighbor vectors of several particles in a single operation.
        Thus, this method helps you avoid a slow, nested Python loop in your code and makes use of all available processor cores.
        ...
        :param sort_by: One of *"index"* or *"distance"*. Requests ordering of the output arrays based on central particle index and, optionally, neighbor distance.
                        If left unspecified, neighbor list entries will be returned in completely arbitrary order.
        ...
        .. tip:: Sorting of neighbor lists will incur an additional runtime cost and should only be requested if necessary.
                 In any case, however, this vectorized method will be much faster than an equivalent Python-based for-loop invoking the
