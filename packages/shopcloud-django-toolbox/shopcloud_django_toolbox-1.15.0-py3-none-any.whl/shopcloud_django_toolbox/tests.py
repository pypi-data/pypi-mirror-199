    ...
    def run_for_model(self, model: str, **kwargs):
        if kwargs.get('is_check_add', True):
            response = self.client.get(
                reverse(f'admin:{self.MODULE}_{model}_add'),
                follow=True
            )
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.template_name[0], f'admin/{self.MODULE}/{model}/change_form.html')

        response = self.client.get(
            reverse(f'admin:{self.MODULE}_{model}_changelist'),
            follow=True
        )
        self.assertEqual(response.status_code, 200)
        if kwargs.get('is_check_template', True):
            self.assertEqual(response.template_name[0], f'admin/{self.MODULE}/{model}/change_list.html')
