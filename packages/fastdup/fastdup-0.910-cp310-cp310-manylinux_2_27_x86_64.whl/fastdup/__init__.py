...
def check_params(work_dir, num_images, lazy_load, get_label_func, slice, save_path, max_width):
    ...
    if (get_label_func is not None):
        ...
        os.path.exists(get_label_func)) or (isinstance(work_dir, pd.DataFrame) and get_label_func in work_dir.columns), "get_label_func has to be a callable function or a dictionary, given the filename returns the "
