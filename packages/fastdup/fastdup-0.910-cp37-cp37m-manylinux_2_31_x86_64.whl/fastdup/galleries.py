    ...
    if (get_label_func is not None):
        if isinstance(get_label_func, str):
            if os.path.exists(get_label_func):
                ...
                assert False, f"Found str label {get_label_func} but it is neither a file nor a column name in the dataframe"
...
if __name__ == "__main__":
    ...
    if False:
        ...
        fd.vis.component_gallery(load_crops=False, sort_by='comp_size')
        files = fd.annotations()
        files['label'] = files['filename'].apply(lambda x: os.path.basename(x)[:2])
        sim = fd.similarity()
        df = merge_with_filenames(sim, files)
        fd.vis.duplicates_gallery(load_crops=False, label_col='label', external_df=df)
