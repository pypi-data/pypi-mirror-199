...
from habana_frameworks.mediapipe.backend.tensor_cpu import CPUTensors
...
class pipe_manager():
    ...
    def decoder_init(self, decoder_op, reader_op):
        ...
        if(self.decoder_op.decoder_type == dect.IMAGE_DECODER):
            decoder_type = mpt.DecoderType.IMAGE_DECODER
            max_frame_vid = 1
        ...
        elif(self.decoder_op.decoder_type == dect.VIDEO_DECODER):
        ...
        ret = self._pm_.decoderMemInit(largest_file,
                                       ...
                                       decoder_type,
                                       max_frame_vid,
                                       params['crop_after_resize'][0],
                                       params['crop_after_resize'][1],
                                       params['crop_after_resize'][2],
                                       params['crop_after_resize'][3])
...
class graph_handler():
    def __init__(self, batch_size, reader_nodes, cpu_nodes, outputs):
        ...
        for r in self.__reader_nodes__:
            self.__create_c_nodes__(r, mpn.NodeType_t.READER)
        for op in self.__cpu_nodes__:
            self.__create_c_nodes__(op, mpn.NodeType_t.CPU)
    ...
    def get_recipe(self):
        ...
        self.blob = self.gp.GetBlob()
        return self.blob

    def get_num_batches(self):
        return self.gp.len()

    def __create_c_nodes__(self, op, node_type):
        opnode = mpn.OpNode(op.name, node_type)
        if(op.guid == None):
...
class pipe_handler():
    def __init__(self, queue_depth, blob):
        self._queue_depth_ = queue_depth
        self.blob = blob
        self.graph_exe = mpn.GraphExecutor(self.blob)
    ...
    def __next__(self):
        return CPUTensors(self.graph_exe.runCpu())
