...
def add_to_prompt_database(text, description, parser, database_path, force_replace=False):
    ...
    if os.path.exists(database_path):
        print("Loading db from: ", database_path)
    ...
    if force_replace and h in hashes:
        print("Found an existing prompt with the same hash, it will be replaced with the new one.")
    ...
    if h not in hashes:
        ...
        print("The prompt is already in the database.")
