...
class ShynaDatabase:
    ...
    def add_speak_sentence(self, speak_sentence):
        try:
            speak_sentence = str(speak_sentence).replace("'", "\\'")
            self.default_database = os.environ.get("notify_db")
            device_id = os.environ.get('device_id')
            self.query = "Insert into speak_sentence (task_date, task_time,sentence_to_speak,device) VALUES ('" \
                         + str(self.s_time.now_date) + "','" + str(self.s_time.now_time) + "','" \
                         + str(speak_sentence) + "','" + str(device_id) + "')"
            self.create_insert_update_or_delete()
        except Exception as e:
            print(e)
