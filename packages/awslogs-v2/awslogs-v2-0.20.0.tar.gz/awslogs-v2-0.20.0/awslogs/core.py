...
class AWSLogs(object):
    ...
    def list_logs(self):
        streams = []
        if self.log_stream_name != self.ALL_WILDCARD or self.treat_stream_as_prefix:
            if self.treat_stream_as_prefix:
                streams = list(self._get_streams_from_prefix(self.log_group_name, self.treat_stream_as_prefix))
            else:
