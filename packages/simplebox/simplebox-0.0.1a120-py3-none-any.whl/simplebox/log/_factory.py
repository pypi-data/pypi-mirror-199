...
from logging import StreamHandler, Formatter, Logger, Handler, LoggerAdapter
from pathlib import Path
from threading import current_thread, RLock
from typing import Mapping, Any

from ._file_handler import _TimedRotatingFileHandlerWrapper, _RotatingFileHandlerWrapper
from .. import banner
...
class _LoggerWrapper(Logger):
    ...
    def __init__(self, name: str):
        self.__level = LogConfig.level
        self.__level_file = LogConfig.level_file
        self.__level_console = LogConfig.level_console
        ...
        if not LogConfig.off:
            for handler_type in ["ch", "fh"]:
                if handler_type == "fh" and LogConfig.off_file:
                    ...
                    continue
                self.addHandler(self.__handler(handler_type))
        self.setLevel(self.__level.value)

    def __handler(self, handler_type: str) -> Handler:
        if "ch" == handler_type:
            ...
            if cut_mode is 1:
                ...
                raise ValueError(f"unknown cut mode: {cut_mode}, excepted '1, 2'")
...
class _LoggerAdapterWrapper(LoggerAdapter):
    ...
    def __init__(self, logger: Logger, extra: Mapping[str, Any], name: str):
        ...
        def __inner(func):
            ...
            def __wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self.log(level=level.value, msg=content, stacklevel=3)
    ...
    def __set(value: V) -> bool:
        ...
        trace_id_map = CacheManager.get_data(_trace_id_map_key)
...
"""
    ...
    @staticmethod
    def get_logger(name: str = "root") -> '_LoggerAdapterWrapper':
        ...
        global log
        if log is None:
            log = _LoggerWrapper("root")
        return _LoggerAdapterWrapper(log, {}, name_)
