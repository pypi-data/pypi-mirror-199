...
def generate_database_schemas(args: Args, include_schema_names: list[str] = ["public"]):
    class ColumnNameDataType(BaseModel):
        ...
        def python_type(self) -> type:
            ...
            if sql_type_startswith("character varying"):
                ...
                result = float
            elif sql_type_startswith("double precision"):
                result = float
            elif sql_type_startswith("inet"):
                result = str
            elif sql_type_startswith("oid"):
                result = int
            elif sql_type_startswith("ARRAY"):
                result = list
            elif sql_type_startswith("json"):
                result = Union[dict, list]
            else:
                print(f'valueError = ValueError(f"Unsupported SQL data type: {self.sql_type}")'+"\ndefaulting to 'Any' result type")
                result = Any
    ...
    def generate_database_schemas(database_schemas_config: DatabaseSchemasConfig):
        ...
        def camel_cased(__tablename:str) -> str:
            return "".join(w.capitalize() for w in __tablename.split('_'))

        def get_schema_type(table: DatabaseTableSchemaConfig, offset: str = ""):
            components = []
            
            class_name = camel_cased(table.table_name)
            component = f"{offset}class {class_name}(BaseModel):"
            ...
            for column_data_type in table.columns:
                ...
                fieldtype = fieldtype.removeprefix("<class '").removesuffix("'>").replace("typing.","")
