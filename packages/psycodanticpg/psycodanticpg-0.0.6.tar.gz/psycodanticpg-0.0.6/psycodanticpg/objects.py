...
class SchemaSQL:
    ...
    def __init__(self, schema: SomeSchema, sql: Optional[SQL] = None,is_subschema:bool=False) -> None:
        self._schema = schema
        self._sql = sql
    ...
    def sql(self) -> SQL:
        ...
        if result is None:
            ...
            db_tablename = tablename.parent_tablename if self.is_subschema else tablename.snake_cased
            ...
            create_table = f"CREATE TABLE {db_tablename} ({', '.join(f'{field.fieldname} {field.sql_type}' for field in fields)});"
...
class Database(BaseModel):
    ...
    def droptable(self, schema: SomeSchema,is_subschema:bool=False):
        schema_sql = SchemaSQL(schema=schema,is_subschema=is_subschema)
        sql = schema_sql.sql
        statements = sql.statements
    ...
    def fetchmany(self, schema: SomeSchema, size: Optional[int] = ...,is_subschema:bool=False):
        connection = SchemaConnection(self.url.dsn)
        schema_sql = SchemaSQL(schema=schema,is_subschema=is_subschema)
        sql = schema_sql.sql.statements.select
    ...
    def insertmany(self, records: List[dict], schema: SomeSchema,is_subschema:bool=False):
        schema_sql = SchemaSQL(schema=schema,is_subschema=is_subschema)
