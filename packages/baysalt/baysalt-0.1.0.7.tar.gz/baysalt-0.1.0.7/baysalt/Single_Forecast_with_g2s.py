...
class Grid2Sca:
    ...
    def getJsonData(self, fc_time, json_ele):
        ...
        for i in range(len(self.value_list)):
            postData[self.title_list[i]] = json_ele[i]
    ...
    def judge_method(self, _date):
        ...
        if self.method == 'linear':  # 手动选择插值方法
            ...
            if self.s_lon in lons and self.s_lat not in lats:
                ...
                if 0 < test.count(0) < 2:
                    print('该点附近存在nan值，不满足线性插值条件，使用反距离权重插值')
                    method = 'idw'
                    cal_id, cal_pa=self.idw_para(lons, lats, value, self.s_lon, self.s_lat)
                    return method, cal_id, cal_pa
                elif test.count(0) == 2:
                    print('该点附近全是nan值，识别为陆地，使用临近插值')
                    method = 'nearest'
                    return method, fp, id
                else:
                    print('该点落在格点经线上，使用一次线性插值即可')
                    method='onelon'
                    return method, fp, id
...
def get_parser():
    ...
    parser.add_argument('-n',    required=False, type=str,   metavar='', default='try',                                                help='set output file name')
