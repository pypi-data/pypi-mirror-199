...
SPECIAL_COMMIT_TYPES = ["build", "deps", "feat"]
SPECIAL_COMMIT_LIMIT = 1

def check_max_allowed_special_commits(commits_string: List):
    spacial_commit_count = 0
    commits = [Commit(c) for c in commits_string]
    for commit in commits:
        if commit.type in SPECIAL_COMMIT_TYPES:
            spacial_commit_count += 1
    if spacial_commit_count > SPECIAL_COMMIT_LIMIT:
        print(
            f"Error: found {spacial_commit_count} special commits to check in the "
            f"current branch while the maximum allowed number is {spacial_commit_count}"
        )
        sys.exit(1)
    return True
