        ...
        while True:
            ...
            if time_prev < time_cur:
                ...
                if second == False or second == None:
                    ...
                    self.__pingDelay=str(int(second))+"ms"
    ...
    def __thread_readfun2(self):
        ...
        while True:
            ...
            if time_prev < time_cur:
                ...
                if not self.__haveModular:
                    if os.path.exists(acs):
                        with open(acs, encoding='utf-8') as file_obj:
                            contents = file_obj.read()
                        if "havenode:1" in contents.rstrip():
                            print("模块存在")
                            if os.path.exists(acminfo):
                                with open(acminfo, encoding='utf-8') as file_obj:
                                    contents = file_obj.read()
                                # print("模块信息", contents.rstrip())
                                s1 = contents.rstrip().rstrip("\r\nOK")
                                s2 = s1.strip("\n")
                                arr = s2.split(",")
                                # print("arr1 ", arr)
                                arr = s2.split("\n")
                                # print("arr2 ", len(arr), arr)
                                # 0 Quectel
                                # 1 EC20F
                                # 2 Revision: EC20CEFILGR06A03M1G
                                if len(arr) == 3:
                                    self.__haveModular = True
                                    print("厂家信息", arr[0])
                                    print("模块型号", arr[1])
                                    self.__modularMode=arr[1]
                                    revis = arr[2].split(":")
                                    if len(revis) == 2:
                                        # print(" ", revis[0].strip())
                                        self.__modularVersion = revis[1].strip()
                                        print("版本号:", revis[1].strip())
                                    else:
                                        print("无效的 版本号")
                                else:
                                    print("参数异常")
