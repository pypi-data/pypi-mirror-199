...
Q=['The figure below shows a vector, in red, with θ = {%theta%}° and a vector magnitude of {%mag%}. Submit the x coordinate component. figure(vector)<br>init({%prms%});<br> <br>',
   'The figure below shows a vector, in red, with θ = {%theta%}° and a vector magnitude of {%mag%}. Submit the y coordinate component. figure(vector)<br>init({%prms%});<br> <br>']
A='''data=[]
...
def example5A(iMultipleChoices=False):
...
def example5B(iMultipleChoices=False, iShuffling=False):
