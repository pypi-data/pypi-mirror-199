...
def generateChoices(ans, nChoices: int=4): # generateEm.work.makeSheetsWChoices()에서 사용

    if isinstance(ans, dict) and 'choices' in ans and ans['choices'] and isinstance(ans['choices'], (list, tuple)):
        try:
        ...
        if ans: # 정수 또는 실수인 일반적인 경우
            ...
            if isinstance(ans, int):
                if random.randint(0, 1):
                    ...
                    if not ans%5:
                        choices=variation0_int_5(str(ans)) if random.randint(0,1) else variation0_int(str(ans))
                    elif not ans%3:
                        choices=variation0_int_3(str(ans)) if random.randint(0,1) else variation0_int(str(ans))
                    elif not ans%2:
    ...
    if not random.choice(range(8)):
        ...
        return ChoicesWithRadiobuttons(answers), nChoices+1, ans
    else:
        answers=random.sample(choices, nChoices-1)
        answers.append(ans)
        random.shuffle(answers)
        return ChoicesWithRadiobuttons(answers), answers.index(ans)+1, ans
...
def ChoicesWithRadiobuttons(answers):
    choices = '<br> '
    for j, choice in enumerate(answers):
