...
def process_chunk(s_filtered, chunk, polygon, field_col_name, field_name, sentinel):
    ...
    for date in chunk:
        if (sentinel == 1):
            bsi = radar_features.calculate_bsi(s_filtered, date, polygon)
            pbsi = radar_features.calculate_bsi(s_filtered, date, polygon, 'P')
            cpbsi = radar_features.calculate_bsi(s_filtered, date, polygon, 'CP')
            tirs = radar_features.calculate_tirs(s_filtered, date, polygon)
...
def s2_process_field(field, start_date, end_date, already_occupied_threads):
    '''
    ...
    # Parallelize features read such that workers are considering different chunks of date to work on
    return parallelize_features_read(s2_filtered, date_range, polygon, field.index[0], field_name, 2, already_occupied_threads)


def s1_process_field(field, start_date, end_date, already_occupied_threads):
    '''
    ...
    return parallelize_features_read(s1_filtered, date_range, polygon, field.index[0], field_name, 1, already_occupied_threads)
    ...
    with concurrent.futures.ThreadPoolExecutor(fields_threads) as executor:
        ...
        if (len(fields_df) == 2):
            futures.append(executor.submit(s1_process_field if sentinel == 1 else s2_process_field, 
                                                fields_df, start_date, end_date, fields_threads))
        else:
            for index, field in fields_df.iterrows():
                futures.append(executor.submit(s1_process_field if sentinel == 1 else s2_process_field, 
                                                field, start_date, end_date, fields_threads))
