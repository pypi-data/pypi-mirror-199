...
from production.config_files_data import syslog_ng, promtail_conf, promtail_service, loki_conf, loki_service
...
async def retrieve_data_user(data: dict):
    ...
    elif config.connexion_mode == "bdd":
        if UserData.user_exists(username=name, password=password):
            user_auth = True
