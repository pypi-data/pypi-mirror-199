...
class DataFrame(abc.SQL):
    ...
    @property
    def columns(self):
        return [c.name for c in self.sql.subquery().c]
    ...
    def __getitem__(self, key):
        ...
        if isinstance(col_key, slice):
            ...
            sql = sa.select(*[self.table.c[k] for k in col_key])
        else:
        ...
        if isinstance(row_key, Column):
            ...
            raise TypeError(f"row_key must be an Column, int, slice, or Ellipsis, but {type(row_key)=}.")
            
        from_ = self.table.c[col_key] if result_class is Column else self.table
        return result_class(self.db, from_, sql)
    ...
    def merge(self, df, how='inner', sort=False, on=None, left_on=None, right_on=None, suffixes=('_x', '_y')):
        if how == 'cross':
            ...
            return DataFrame(self.db, sql.subquery(), sql)
        ...
        return DataFrame(self.db, table, sql)
    
    def sort_values(self, by):
        return DataFrame(self.db, self.table, self.sql.order_by(by))
...
class GroupByNthSelector:
    ...
    def __getitem__(self, key):
        ...
        return DataFrame(self.gb.df.db, self.gb.df.table, sa.select(*[df.c[col] for col in self.gb.df.columns]).where(df.c.row_number.between(start, stop)).order_by(df.c.order))
...
class ColumnGroupBy:
    ...
    def agg(self, *args, **kwargs):
        ...
        return result_class(self.gb.df.db, self.gb.df.table, sa.select(*[self.gb.df.table.c[col] for col in self.gb.columns], *columns).group_by(self.gb.columns))
    ...
    def transform(self, *args, **kwargs):
        ...
        return result_class(self.gb.df.db, self.gb.df.table, sa.select(*columns).order_by(sa.func.row_number().over()))
    
    
def decorator(func):
    @functools.wraps(func)
    def wrapped(self, *args):
        args = [arg.column if isinstance(arg, Column) else arg for arg in args]
        expr = func(self.column, *args)
        return Column(self.db, expr, sa.select(expr))
    return wrapped
...
class Column(abc.SQL):
    ...
    @property
    def str(self):
        return StringMethods(self.db, self.column, self.sql)
