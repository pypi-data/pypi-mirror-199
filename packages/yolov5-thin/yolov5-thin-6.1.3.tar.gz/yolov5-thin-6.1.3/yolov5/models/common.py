...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights, weights_type='pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False):
        '''
        model: bytes
        model_type: pt,onnx
        '''
    ...
    def forward(self, im, augment=False, visualize=False, val=False):
        ...
        if self.weights_type == 'pt':  # PyTorch
            y = self.model(im, augment=augment, visualize=visualize)[0]
        elif self.weights_type == 'onnx':  # ONNX Runtime
...
class AutoShape(nn.Module):
    ...
    def __init__(self, model, model_type):
        ...
        self.pt = not self.dmb or model_type =='pt'  # PyTorch model
