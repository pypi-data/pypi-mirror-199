...
class Resensys():
    ...
    def __init__(self,username,password):
        ...
        self.encoded_password = self.password_encoding_generator(password)
    ...
    def connect(self):
        ...
        self.conn = mysql.connector.connect(user=self.username,password=self.encoded_password,host="resensys.net",port=3306,use_pure=True)
        return self.conn
    
    def getCredentials(self):
        return self.username,self.password
    
    def password_encoding_generator(self,p):
        password_bytes = bytes(p, 'ascii')
        byte_list = []
