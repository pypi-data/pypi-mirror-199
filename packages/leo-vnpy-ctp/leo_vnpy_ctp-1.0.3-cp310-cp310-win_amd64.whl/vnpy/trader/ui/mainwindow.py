...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        ...
        self.window_title: str = f"VeighNa Trader RQSdk版 - {vnpy.__version__}   [{TRADER_DIR}]"
        ...
        self.event_engine.register(CUS_LOG, self.custom_event)

    def custom_event(self, event: Event):
        """
    ...
    def init_menu(self) -> None:
        ...
        action2: QtGui.QAction = QtWidgets.QAction("远程更新合约", self)
        action2.triggered.connect(self.query_rqsdk)
    ...
    def query_rqsdk(self) -> None:
        # 客户端发起 leo
        # RqsdkRpcEngine
        # 此处是异步的，等到服务端返回
        self.rpc_engine: RqsdkRpcEngine = self.main_engine.get_engine(APP_rqsdk)
        # ret = self.rpc_engine.get_contract_property(order_book_ids= ['10002752'], start_date='20210115', end_date='20210118')

        start = datetime.strptime('20221202', '%Y%m%d')
        end = datetime.strptime('20230202', '%Y%m%d')
        ...
        ret = self.rpc_engine.query_bar_history(req)
        print(f"get_contract_property 米筐：{ret}")
