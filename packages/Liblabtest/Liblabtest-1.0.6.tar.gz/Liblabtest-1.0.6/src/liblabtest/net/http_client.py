...
import requests
from http_exceptions import HTTPException, client_exceptions
...
class HTTPClient:
    ...
    def _make_http_request(self, method, endpoint_url, headers, body_input):
        ...
        if not serialized_body and method in {'get','delete'}:
    ...
    def delete(self, endpoint_url: str, headers: dict, retry: bool = True):
        ...
        response = self._make_http_request("delete",endpoint_url, headers, {})
        if response.status_code in self._retry_codes and retry:
            try_cnt = 1
            while response.status_code in self._retry_codes and try_cnt - 1 < self._max_retries:
                sleep(self._initial_delay ** (try_cnt - 1))
                response = self._make_http_request("delete",endpoint_url, headers, body_input)
                try_cnt += 1
    ...
    def get(self, endpoint_url: str, headers: dict, retry: bool = True):
        ...
        response = self._make_http_request("get",endpoint_url, headers, {})
        if response.status_code in self._retry_codes and retry:
            try_cnt = 1
            while response.status_code in self._retry_codes and try_cnt - 1 < self._max_retries:
                sleep(self._initial_delay ** (try_cnt - 1))
                response = self._make_http_request("get",endpoint_url, headers, body_input)
                try_cnt += 1
    ...
    def _raise_from_status(self, response) -> None:
        ...
        if response.status_code == 401:
            raise client_exceptions.UnauthorizedException(message=self._create_exception_message(response, 'WWW-Authenticate'))
        elif response.status_code == 405:
            # this indicates a bug in the spec if it allows a method that the server rejects
            raise client_exceptions.MethodNotAllowedException(message=self._create_exception_message(response, 'Allow'))
        elif response.status_code == 407:
            raise client_exceptions.ProxyAuthenticationRequiredException(message=self._create_exception_message(response, 'Proxy-Authenticate'))
        elif response.status_code == 413:
            raise client_exceptions.PayloadTooLargeException(message=self._create_exception_message(response, 'Retry-After'))
        elif response.status_code == 429:
            raise client_exceptions.TooManyRequestsException(message=self._create_exception_message(response, 'Retry-After'))
        elif response.status_code == 503:
            raise server_exceptions.ServiceUnavailableException(message=self._create_exception_message(response, 'Retry-After'))
        else:
