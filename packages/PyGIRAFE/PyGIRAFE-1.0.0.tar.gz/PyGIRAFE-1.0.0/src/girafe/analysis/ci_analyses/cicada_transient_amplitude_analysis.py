...
class CicadaTransientAmplitudeAnalysis(CicadaAnalysis):
    def __init__(self, config_handler=None):
        """
        """
        long_description = '<p align="center"><b>Compute mean calcium event amplitude for all cells</b></p><br>'
        long_description = long_description + 'Use the raster and the traces to return the mean transient amplitude of each cell' \
        ...
        CicadaAnalysis.__init__(self, name="Transients' amplitude", family_id="Descriptive statistics",
                                short_description="",
    ...
    def copy(self):
        ...
        """
        analysis_copy = CicadaTransientAmplitudeAnalysis(config_handler=self.config_handler)
        ...
        for data_to_analyse in self._data_to_analyse:
            ...
            for rrs_name in rrs_names:
                if 'raster' in rrs_name:
                    ...
                    continue
            for rrs_name in rrs_names:
                if 'trace' in rrs_name:
                    traces_in_rrs_keys = True
                else:
                    continue
            if (raster_in_rrs_keys is False) or (traces_in_rrs_keys is False):
                self.invalid_data_help = f"The analysis requires both spike raster and traces"
                return False
    ...
    def run_analysis(self, **kwargs):
        ...
        for session_index, session_data in enumerate(self._data_to_analyse):
            ...
            # Find cells that spikes at least once
            sum_spikes = np.sum(raster, axis=1)
            active_cells = np.where(sum_spikes >= 1)[0]
            no_active_cells = np.where(sum_spikes == 0)[0]

            if verbose:
                print(f"N active cells : {len(active_cells)}, N inactive cells: {len(no_active_cells)}")
            ...
            # Building mean transient's amplitude vector
            for trace_index, trace in enumerate(traces):
                traces[trace_index] = (trace - np.median(trace)) / np.median(trace)

            fluo_increases = []
            for cell in range(n_cells):
                tmp_tples = get_continous_time_periods(raster_dur[cell, :])
                fluo_increase_cell = []
                for tple in range(len(tmp_tples)):
                    onset = tmp_tples[tple][0]
                    peak = tmp_tples[tple][1]
                    if peak >= 1 and onset >= 1:
                        fluo_peak = np.max(traces[cell, (peak-1):(peak+1)])
                        fluo_onset = np.min(traces[cell, (onset-1):(onset+1)])
                        fluo_measured = fluo_peak - fluo_onset
                        if fluo_measured < 0:
                            fluo = 0
                        else:
                            fluo = fluo_measured
            ...
            sum_up_data = {'MeanProminence': mean_amplitude_by_cell, 'Cell#': np.arange(n_cells),
                           'Celltype': cell_type_list, 'Session': session_identifier_list,
                           'SubjectID': animal_id_list, 'Age': age_list, 'Weight': weight_list}
            ...
            if do_stats and len(unique_types_list) > 1:
                for index, name in enumerate(unique_types_list):
                    table = amplitude_data_table.query('Celltype == @name')
                    frequencies = table.get("MeanProminence")
        ...
        if cell_to_use == "all_cells":
            ...
            data_table = gobal_amplitude_data_table.query('Celltype == @type_to_use')
        ...
        if do_stats:
            ...
            if cell_to_use == "all_cells":
                ...
                if n_ages > 1:
                    if verbose:
                        print(f"-------------- Compare {cell_to_use} transient's amplitude across ages ---------------")
        ...
        ylabel = " Transient's mean amplitude (dF/F)"
