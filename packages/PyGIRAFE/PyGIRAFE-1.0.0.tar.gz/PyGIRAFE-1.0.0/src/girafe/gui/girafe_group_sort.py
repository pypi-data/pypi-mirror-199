...
from girafe.gui.girafe_metadata_widget import MetaDataWidget
...
class SessionsWidget(QWidget):
    ...
    def more_menu(self):
        ...
        self.createGroupAct = QAction("Create group")
    ...
    def uncheck_all(self):
        ...
        for idx in range(self.q_list.count()):
            flags = self.q_list.item(idx).flags()
            if flags & 1:  # Item is selectable, meaning it is a session
    ...
    def is_item_available(self, item):
        """
        Check if an item is in the list of displayed item
        :param item: (str) represent the data identifier
        :return:
        """
    ...
    def get_checked_items(self):
        """
        Return the str represeing the selected items
        :return:
        """
        checked_items = []
        for index in range(self.q_list.count()):
    ...
    def send_data_to_analysis_tree(self):
        ...
        # the data in data_to_analyse list, are instances of GirafeAnalysisNwbWrapper
        if data_to_analyse:
    ...
    def form_group(self, dict_group):
        ...
        """
        # TODO: keep list of selected items
        ...
        for group_name, group_members in dict_group.items():
            ...
            item.setFlags(QtCore.Qt.ItemIsEnabled)
            item.setFlags(item.flags() & ~QtCore.Qt.ItemIsSelectable & ~QtCore.Qt.ItemIsUserCheckable)  # item should not be selectable
            self.q_list.addItem(item)
            # member is an instance of CicadaAnalysisWrapper
            for member in group_members:
                item = QListWidgetItem()
                # so we keep the same selection as before the ordering
                if member.identifier in checked_items:
                    item.setCheckState(QtCore.Qt.Checked)
                else:
                    item.setCheckState(QtCore.Qt.Unchecked)
    ...
    def save_group(self):
        ...
        self.save_group_name = QLineEdit(self.nameBox)
        self.save_group_name.setText("Group_" + str(datetime.date.today()))
        self.saveButton = QPushButton('Save as')
        self.nameBoxLayout.addWidget(self.save_group_name)
    ...
    def save_group_names(self):
        ...
        group_name = self.save_group_name.text()
        group_to_save = []
        checked_items = []
        for index in range(self.q_list.count()):
            if self.q_list.item(index).checkState() == 2:
        ...
        # instances of the cicada_wrapper
        data_in_group = [data for data_id, data in self.cicada_main_window.data_dict.items() if data_id in checked_items]

        for session_data in data_in_group:
            group_to_save.append([session_data.identifier, session_data.data_ref])
            # for path in self.cicada_main_window.nwb_path_list.values():
            #     if path.endswith(item + ".nwb"):
            #         # we want to append each time a tuple containing the ID and the path of the file/directory
            #         group_to_save.append(path)
        if self.cicada_main_window.groups_data:
            self.cicada_main_window.groups_data.update({group_name: group_to_save})
        else:
            self.cicada_main_window.groups_data = {group_name: group_to_save}

        self.update_group_config_yaml_file()


    def update_group_config_yaml_file(self):
        """
        Update the yaml file that store the groups content.
        Update menu after saving the file
        :return:
        """

        my_path = os.path.abspath(os.path.dirname(__file__))
        group_yaml_file = os.path.join(my_path, "../config/group.yaml")
        group_yaml_file = os.path.realpath(group_yaml_file)

        # then we load the yaml file and update groups_data according to the content
        # Groups are categorized by their wrapper_id
        with open(group_yaml_file, 'r') as stream:
            data_from_yaml_group = yaml.safe_load(stream)

        if isinstance(data_from_yaml_group, dict):
            data_from_yaml_group[self.cicada_main_window.data_wrapper.WRAPPER_ID] = self.cicada_main_window.groups_data
        else:
            data_from_yaml_group = dict()
            data_from_yaml_group[self.cicada_main_window.data_wrapper.WRAPPER_ID] = self.cicada_main_window.groups_data

        with open(group_yaml_file, 'w+') as stream:
            yaml.dump(data_from_yaml_group, stream, default_flow_style=False)
