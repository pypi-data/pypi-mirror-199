...
try:
	...
	from pysrc.utils.utils import get_fully_qualified_name, safe_get
...
common_regex = {
	'SHA':
		"sha | SHA\\(",  #https://pythex.org/?regex=sha%20%7C%20SHA%5C(&test_string=hashes.SHA256()%0Ahashes.SHA()%0Ahashes.SHA512()%0Ahashes.SHA2()%0Ahashes.sha()&ignorecase=0&multiline=0&dotall=0&verbose=1
	'ARC': "arc | ARC\\(",
	'AES': "aes | AES\\(",
	'MD': "md | MD *",
	'SAFE': "^((?!safe_).)",
...
def unwravel(obj, search_dyct: dict = rule_groups):
	"""
	The unwravel function takes a dictionary object and returns the name, message, and severity of the rule.
	The unwravel function is used to extract information from a dictionary that contains rules.

	:param obj: Search the rule_groups dictionary for a matching key
	:param search_dyct:dict=rule_groups: Search the rule_groups dictionary for a matching key
	:return: The name of the rule, the message and severity
	:doc-author: frantzme
	**Verified: FALSE**
	"""
	try:
...
def rule_check(rule: dict, value: str, secure_values: dict) -> str:
	...
	if rule['live_type'] == "int":
		if int(value) < rule['value']:
			return "Value {0} is less than the required value of {1}".format(value, rule['value'])
	elif rule['live_type'] == "string":
		if utils.compare(value, rule['value']):
			return "Value {0} cannot be set to {1}".format(value, rule['value'])
	elif rule['live_type'] == "bool":
		if value == str(rule['value']):
			return "Value {0} should not be set to {1}".format(value, rule['value'])
		...
		if re.match(rule['value'], value):
			return "Value {0} should not be used, breaks the regex: {1}".format(value, rule['value'])
	elif rule['live_type'] == 'secure':
		# region Src
		try:
			from pysrc.utils.utils import get_fully_qualified_name, compare
		except Exception as e:
			pass
		try:
		...
		if not any(
				list(
					map(
						lambda sec: compare(
							"{0}.{1}".format(get_fully_qualified_name(sec), sec['criteria']),
							value,
							strip_start=True), secure_values))):
			return "Value {0} is not secure".format(value)
		...
		for union_value in list(rule_values.keys() & json_value.keys()):
			if rule_values[union_value] == json_value[union_value]:
				return "Value {0} should not be set to {1}".format(union_value, json_value[union_value])
