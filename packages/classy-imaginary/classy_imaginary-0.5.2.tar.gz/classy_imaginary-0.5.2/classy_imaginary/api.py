        ...
        for i, prompt in enumerate(prompts):
            ...
            for attempt in range(0, unsafe_retry_count + 1):
                ...
                result = _generate_single_image(
                    ...
                    codeformer_model=codeformer_net_model,
...
def _generate_single_image(
    ...
    from classy_imaginary.img_utils import (
    ...
    from classy_imaginary.model_manager import (
        get_model_default_image_size,
    )
    from classy_imaginary.modules.midas.api import torch_image_to_depth_map
    ...
    with ImageLoggingContext(
        ...
        if hasattr(model, "depth_stage_key"):
            ...
            from classy_imaginary.img_processors.control_modes import CONTROL_MODES
        ...
        if (
            ...
            if prompt.init_image:
                comp_image = _generate_composition_image(
                    ...
                    safety_feature_extractor=safety_feature_extractor,
                    safety_checker=safety_checker,
                    codeformer_model=codeformer_model,
        ...
        if not nsfw_filter or not safety_score.is_filtered:
            if prompt.fix_faces and codeformer_model:
                logger.info("Fixing 😊 's in 🖼  using CodeFormer...")
                with lc.timing("face enhancement"):
                    gen_img = enhance_faces(gen_img,
...
def _generate_composition_image(prompt,
                                target_height,
                                target_width,
                                model,
                                nsfw_filter: bool = False,
    ...
    result = _generate_single_image(prompt=composition_prompt,
                                    model=model,
                                    nsfw_filter=nsfw_filter,
                                    safety_feature_extractor=safety_feature_extractor,
                                    safety_checker=safety_checker,
...
def combine_image(original_img, generated_img, mask_img):
    ...
    from classy_imaginary.log_utils import log_img
