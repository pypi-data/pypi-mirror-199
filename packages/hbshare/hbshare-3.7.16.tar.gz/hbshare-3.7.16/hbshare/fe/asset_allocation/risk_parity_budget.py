...
class RiskParityBudget:
    def __init__(self, asset_type, asset_list, asset_type_dict, method, start_date, end_date, is_reblance, reblance_type, n, frequency, compute_risk_days,
                 lb_list=None, ub_list=None, asset_category_dict=None, category_lb_list=None, category_ub_list=None, total_weight=None, risk_budget_list=None, processes=16):
        ...
        if period_data['cov_df'].empty:
            ...
            try:
                if self.method == 'risk_parity':
                    ...
                    weight, status = RiskParity(self.asset_type, self.asset_list, self.cov, self.lb_list, self.ub_list, self.asset_category_df, self.category_lb_list, self.category_ub_list, self.total_weight).solve()
                    ...
                    self.cov = period_data['cov_df']
                    weight, status = RiskBudget(self.asset_type, self.asset_list, self.cov, self.lb_list, self.ub_list, self.asset_category_df, self.category_lb_list, self.category_ub_list, self.total_weight, self.risk_budget_list).solve()
                    weight = pd.DataFrame(weight, columns=[date]).T
                    reason = '{0}：模型当期无优化解！'.format(date) if status == 'infeasible' else np.nan
                    status = pd.DataFrame(index=[date], columns=['优化状态', '原因'], data=[[status, reason]])
            except:
...
if __name__ == '__main__':
    ...
    weight_df, status_df = RiskParityBudget(asset_type='fund',  # index, fund
