...
def Pgrad(P, T, oil_rate, wtr_rate, Gor, gas_grav, oil_grav, wtr_grav, d, angle):
    ...
    Z = zfact((T + 460) / Tc(gas_grav), P / Pc(gas_grav))               #Gas compressibility factor
    Wor = wtr_rate / oil_rate                                           #Water-oil ratio, stb/stb
    TDS = salinity(wtr_grav)                                            #Water salinity, wt% total dissolved solids
    Pb = Pbub(T, Tsep, Psep, gas_grav, oil_grav, Gor)                   #Bubble point pressure, psia
    Rso = sol_gor(T, P, Tsep, Psep, Pb, gas_grav, oil_grav)             #Solution gas-oil ratio, scf/stb
    Rsw = sol_gwr(P, T, TDS)                                            #Solution gas_water ratio, scf/stb
    Bo = oil_fvf(T, P, Tsep, Psep, Pb, Rso, gas_grav, oil_grav)         #Oil formation volume factor, rb/stb
    Bw = wtr_fvf(P, T, TDS)                                             #Water formation volume factor, rb/stb
    Bg = gas_fvf(P, T, gas_grav)                                        #Gas formation volume factor, ft_/scf
    muo = oil_visc(T, P, Tsep, Psep, Pb, Rso, gas_grav, oil_grav)       #Oil viscosity, cp
    muw = wtr_visc(P, T, TDS)                                           #Water viscosity, cp
    mug = gvisc(P, T + 460, Z, gas_grav)                                #Gas viscosity, cp
    rhoo = oil_dens(T, P, Tsep, Psep, Pb, Bo, Rso, gas_grav, oil_grav)  #Oil density, lb/ft_
    rhow = 62.368 * wtr_grav / Bw                                                  #Water density, lb/ft_
    rhog = 2.699 * gas_grav * P / (T + 460) / Z                                    #Gas density, lb/ft_
    sigo = oil_tens(P, T, oil_grav)                                     #Gas-oil interfacial tension, dynes/cm
    sigw = wtr_tens(P, T)                                               #Gas-water interfacial tension, dynes/cm
