...
GIT_PERSONAL_ACCESS_TOKEN_KEY = "GIT_PERSONAL_ACCESS_TOKEN"
...
class GitPackageMixin:
    def __init__(self, git: str) -> None:
        super().__init__()
        self.git = git
        # If env is darwin and url is using ssh, convert url to https, which accepts Personal Access Token (PAT)
        if os.getenv(PLATFORM_KEY, "") == DARWIN_PLATFORM and not self.git.startswith(
            ("https", "http")
        ):
            # Always set in darwin env
            pat = os.getenv(GIT_PERSONAL_ACCESS_TOKEN_KEY, "")
            # Get suffix like "linkedin.ghe.com:multiproduct/in-dbt-utils.git"
            # e.g "linkedin.ghe.com:multiproduct/in-dbt-utils.git"
            mp_suffix_with_colon = self.git.split("@")[1]
            # Remove colon from suffix, replace with '/' like any https url
            # e.g "linkedin.ghe.com/multiproduct/in-dbt-utils.git"
            mp_suffix = mp_suffix_with_colon.replace(":", "/")
            # Generate and set final https url
            self.git = "https://{pat}@{mp_suffix}".format(pat=pat, mp_suffix=mp_suffix)
