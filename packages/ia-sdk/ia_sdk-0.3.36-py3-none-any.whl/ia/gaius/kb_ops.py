...
def remove_abstracted_symbol(agent: AgentClient, symbol_name: str):
    ...
    Args:
        ...
        symbol_name (str): Abstracted symbol to be removed.

    Raises:
        Exception: If symbol is in improper format
    """
    
    symbol_split = symbol_name.split('|')
    
    if symbol_split[0] != 'PRIMITIVE':
        raise Exception(f'Symbol {symbol_name} does not begin with "PRIMITIVE|"')
    
    node_name = symbol_split[1]
