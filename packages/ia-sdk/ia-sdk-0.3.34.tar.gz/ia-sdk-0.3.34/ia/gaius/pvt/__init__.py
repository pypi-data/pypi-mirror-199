...
class PerformanceValidationTest():
            ...
            if j % 10 == 0:
                if self.task:
                    if self.task.is_aborted():
                        self.abort_test(current_record=j, record_count=train_seq_len)
    ...
    def test_agent(self):
        ...
        for k, _ in enumerate(self.dataset.test_sequences):
            ...
            if k % 10 == 0:
                if self.task:
                    if self.task.is_aborted():
                        self.abort_test(current_record=k, record_count=test_seq_len)
    ...
    def abort_test(self, current_record, record_count):
        if not self.QUIET:
