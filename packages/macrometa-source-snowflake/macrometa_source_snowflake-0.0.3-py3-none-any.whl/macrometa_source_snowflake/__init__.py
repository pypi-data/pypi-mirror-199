...
def do_sync_incremental(snowflake_conn, catalog_entry, state, columns, replication_method):
    ...
    incremental.sync_table(snowflake_conn, catalog_entry, state, columns, replication_method)
...
def sync_streams(snowflake_conn, catalog, state, replication_method):
    for catalog_entry in catalog.streams:
        ...
        replication_method = md_map.get((), {}).get('replication-method', replication_method)
...
def main_impl():
    ...
    if args.discover:
        ...
        state = args.state or {}
        do_sync(snowflake_conn, args.config, args.catalog, state, replication_method)
    elif args.properties:
        catalog = Catalog.from_dict(args.properties)
        state = args.state or {}
        do_sync(snowflake_conn, args.config, catalog, state, replication_method)
    else:
