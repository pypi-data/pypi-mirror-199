...
class Client():
  ...
  def get_inferences(self, model_id:str, model_version_ids=[], page_count=0, page_size=25, include_already_added=False, show_hidden=False):
    ...
    return self.api_get(model_inferences_api,  params=params)
  
  def add_inferences(self, dataset_id, dataset_version_id, dataset_split_id, keep_annotations=True, inferences=[]):
    self.requires_login()
    ...
    return self.api_post(add_inferences_api, inferences_obj)

  # def add_inference_image(self):
  #   self.requires_login()
  ...
  def get_stats_for_unlabelled(self, label_stats):
    return self.get_stats_for_label(label_stats, "")

  def get_stats_for_label(self, label_stats, label_id):
  ...
  def find_item_in_array(self, ar, prop, value, first_item=False):
