...
def create_database(name: str):
    ...
    if not os.path.isfile(f'datas/{name}.ezdb'):
        with open(f'datas/{name}.ezdb', 'w') as cdb:
...
def drop_database(name: str):
    if os.path.isfile(f'datas/{name}.ezdb'):
        os.remove(f'datas/{name}.ezdb')
...
def export_database(name: str, path_: str):
    if os.path.isfile(f'datas/{name}.ezdb'):
        shutil.copyfile(f'datas/{name}.ezdb', f"{path_}/{name}.ezdb")
    ...
    if os.path.isfile(path_):
        with open(path_, 'r') as rdb:
            try:
                ...
                db_ = rdb.read()
        with open(f'datas/{db_["name"]}.ezdb', 'w') as wdb:
            try:
...
class sayuDB:
    ...
    def save_table(self, name: str, content: object):
        if self.is_remote:
            ...
            with open(f'datas/{self.database}.ezdb', 'w') as wdb:
    ...
    def openDB(self):
        if self.is_remote:
            ...
            with open(f'datas/{self.database}.ezdb', 'r') as rdb:
