import functools, os, shutil, json, zlib, secrets, urllib.parse, requests
...
class HTTPUtils:
    @staticmethod
    def isValidResponse(expStatus, obsStatus):
        if not expStatus == obsStatus:
            raise KubefacetsAPIException(
                "Response status_code recd {}, but expected {}".format(
                    obsStatus, expStatus
                ), "unexpected_error", obsStatus
            )
        return True
    ...
    def request(method, url, expStatus, **kwargs):
        ...
        if HTTPUtils.isValidResponse(expStatus, resp.status_code):
            return resp
