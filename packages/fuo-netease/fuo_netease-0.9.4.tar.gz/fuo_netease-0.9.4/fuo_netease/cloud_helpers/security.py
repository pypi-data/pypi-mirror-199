...
WEAPI_WATCHMAN_SBOX = (
    "LPc4uQNptC2A6y.R90DOBIroS+qnx/eb3FM8fW1UZG7VmwvksgjhaTKzlXdiYEHJ"
)
...
def c_decrypt_abroad_message(hexstring):
    ...
    for i in range(0, len(source), 64):
        box = source[i : i + 64]
        boxB = c_apply_sbox(
            c_apply_sbox(box, WEAPI_ABROAD_SBOX), WEAPI_ABROAD_SBOX
        )
        ...
        result += boxE
    # last 4 bytes contains length
    # don't need them for now as we can just strip them away
