...
def masked_select(src: SparseTensor, dim: int, mask: torch.Tensor) -> SparseTensor:
    ...
    if dim == 0:
        ...
        mask = mask[row]
        row = torch.arange(rowcount.size(0), device=row.device).repeat_interleave(
            rowcount
        )
        ...
        mask = mask[col]
        col = torch.arange(colcount.size(0), device=col.device).repeat_interleave(
            colcount
        )
        row = row[mask]
...
def masked_select_nnz(
    src: SparseTensor, mask: torch.Tensor, layout: Optional[str] = None
) -> SparseTensor:
    assert mask.dim() == 1
...
def tmp(self, mask, layout=None):
    return masked_select_nnz(self, mask, layout)  # noqa
