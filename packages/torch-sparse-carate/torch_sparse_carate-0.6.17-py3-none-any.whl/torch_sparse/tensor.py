...
class SparseTensor(object):
    ...
    @classmethod
    def from_torch_sparse_coo_tensor(self, mat: torch.Tensor, has_value: bool = True):
        mat = mat.coalesce()
        index = mat._indices()
    ...
    def set_value_(self, value: Optional[torch.Tensor], layout: Optional[str] = None):
        self.storage.set_value_(value, layout)
        return self

    def set_value(self, value: Optional[torch.Tensor], layout: Optional[str] = None):
    ...
    def fill_value_(self, fill_value: float, dtype: Optional[int] = None):
        value = torch.full((self.nnz(),), fill_value, dtype=dtype, device=self.device())
    ...
    def to_dense(self, dtype: Optional[int] = None) -> torch.Tensor:
        ...
        if value is not None:
            mat = torch.zeros(self.sizes(), dtype=value.dtype, device=self.device())
        else:
        ...
        if value is not None:
            ...
            mat[row, col] = torch.ones(self.nnz(), dtype=mat.dtype, device=mat.device)

        return mat

    def to_torch_sparse_coo_tensor(self, dtype: Optional[int] = None) -> torch.Tensor:
        row, col, value = self.coo()
    ...
    def to_torch_sparse_csr_tensor(self, dtype: Optional[int] = None) -> torch.Tensor:
    ...
    def to_torch_sparse_csc_tensor(self, dtype: Optional[int] = None) -> torch.Tensor:
...
def cpu(self) -> SparseTensor:
    return self.device_as(torch.tensor(0.0, device="cpu"))


def cuda(self, device: Optional[Union[int, str]] = None, non_blocking: bool = False):
    return self.device_as(torch.tensor(0.0, device=device or "cuda"))
