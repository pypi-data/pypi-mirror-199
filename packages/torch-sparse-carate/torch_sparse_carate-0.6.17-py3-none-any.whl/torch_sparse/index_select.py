...
def index_select(src: SparseTensor, dim: int, idx: torch.Tensor) -> SparseTensor:
...
def index_select_nnz(
    src: SparseTensor, idx: torch.Tensor, layout: Optional[str] = None
) -> SparseTensor:
    assert idx.dim() == 1
...
def tmp(self, idx, layout=None):
    return index_select_nnz(self, idx, layout)  # noqa
