...
def init_work_space(workspace_path):
    ...
    file_list = []
    find_json_config(workspace_path, file_list)
...
def sync_project_data(workspace_path):
    ...
    if len(project_space_list) != len(origin_project_space_list):
        logger.info(f"projectSpace changed projectSpaceList: {project_space_list}          originProjectSpaceList:{origin_project_space_list}")
...
def overwrite_file(workspace_path):
    ...
    print(project_space_list, offer_map)
    for project_space in project_space_list:
...
def find_json_config(file_path, config_list):
    for file_name in os.listdir(file_path):
        ...
        if os.path.isdir(os.path.join(file_path, file_name)):
            find_json_config(os.path.join(file_path, file_name), config_list)
