...
async def documents_portion_iter_async(index_name: str, documents: AsyncGenerator, bulk_size: int):
    documents_portion = []
...
class SummaClient(BaseGrpcClient):
    ...
    async def index_document_stream(
            self,
            index_name: str,
            documents: Union[AsyncIterator, Iterable[str], str] = None,
            bulk_size: int = 100,
            request_id: Optional[str] = None,
            session_id: Optional[str] = None,
        ...
        if isinstance(documents, AsyncGenerator):
            d_iter = documents_portion_iter_async(index_name, documents, bulk_size)
        else:
            d_iter = documents_portion_iter(index_name, documents, bulk_size)
