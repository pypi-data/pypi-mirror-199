...
def start(project_data, port):
    with TappServer(project_data, port) as tapp_server:
        with Tunnel(port) as tunnel:
            for name, data in tapp_server.project_data.items():
                url_tapp = f'{tunnel.tunnel.public_url}/{name}.tapp'
                CODE_MASK_DEFAULT = 'tasmota.urlfetch("{url}")'
                code_snippet = data.get('code_mask', CODE_MASK_DEFAULT).format(url=url_tapp, name=name)
