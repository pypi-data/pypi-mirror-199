...
class FactorAnalysis:
    ...
    def get_Decile_result_sector(self, sector, col_name, drtion):
        ...
        for i in range(0, 10):
            ...
            n_q = n_q[n_q]
            n_q = n_q.stack('종목코드').reset_index().assign(w=lambda x: x.groupby('date')['종목코드'].count().rdiv(1)).pivot(columns='종목코드', values='w').dropna(axis=1, how='all').dropna(axis=0, how='all').fillna(0)
