...
class FactorAnalysis_4Universes:
    def __init__(self, df_with_factor, price_df, BM_data, BV_univ, SV_univ,BG_univ, SG_univ):
        """
    ...
    def get_Decile_result_4Universes(self, col_name, drtion):
        df_to_calc = self._df[['date', '종목코드', col_name]].copy()
        df_to_calc_BV = pd.concat([df_to_calc.set_index(['date', '종목코드']), self.BV_univ.set_index(['date', '종목코드'])], axis=1).dropna(axis=0)
        df_to_calc_SV = pd.concat([df_to_calc.set_index(['date', '종목코드']), self.SV_univ.set_index(['date', '종목코드'])], axis=1).dropna(axis=0)
        df_to_calc_BG = pd.concat([df_to_calc.set_index(['date', '종목코드']), self.BG_univ.set_index(['date', '종목코드'])], axis=1).dropna(axis=0)
        df_to_calc_SG = pd.concat([df_to_calc.set_index(['date', '종목코드']), self.SG_univ.set_index(['date', '종목코드'])], axis=1).dropna(axis=0)
