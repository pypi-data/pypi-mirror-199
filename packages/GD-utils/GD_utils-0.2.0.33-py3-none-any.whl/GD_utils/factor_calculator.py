...
class FactorAnalysis:
    ...
    def get_Decile_result_sector(self, sector, col_name, drtion):
        ...
        for i in range(0, 10):
            ...
            n_q = n_q.stack().reset_index().assign(w=lambda x:x.groupby('date').count().rdiv(1)).pivot(index='date', columns='종목코드', values='w')
            n_q = n_q.mul(1 / n_q.sum(1), axis='index').dropna(axis=1, how='all').dropna(axis=0, how='all').fillna(0)
