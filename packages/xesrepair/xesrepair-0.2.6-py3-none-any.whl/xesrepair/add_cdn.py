...
def addCdns(config_path="config.ini"):
    ...
    print("cur_cdns", cur_cdns, cf["cdns"]["test"])
    if "http://static0.xesimg.com" in cur_cdns:
        return
    # 2、添加cdn
    # 2.1、添加测试环境的cdn
    test_cdn_arr = ['http://static0-test.xesimg.com',
                    'https://static0-test.xesimg.com']
    test_cdn_str = concatCdns(test_cdn_arr, ",\n")
    cf['cdns']['test'] = test_cdn_str + cf['cdns']['test']
    # 2.2、添加线上环境的cdn
    online_cdn_arr = []
    for i in range(0, 11):
        cdn1 = 'http://static{}.xesimg.com'.format(i)
        online_cdn_arr.append(cdn1)
        cdn2 = 'https://static{}.xesimg.com'.format(i)
        online_cdn_arr.append(cdn2)
