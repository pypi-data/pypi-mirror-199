...
def get_region(market):
    return ml.market.__class__.__name__.replace('MarketInfo', '').lower()
...
def dump():
    ...
    dumper = DumpDataAll(csv_path, qlib_dir, include_fields=include_fields)
    dumper()

def init():
    region = get_region(ml.market)
