...
class TestCLI(BaseTest):
    ...
    async def _tb(self, args=None, input=None, env=None, replace_new_line=True, assert_exit_code: Optional[int] = None):
    ...
    async def _auth(self, token=None):
        ...
        _ = await self._tb(["auth", f'--host={self.host}'], input=f'{token}')
...
class TestCLIDatasourceDeleteRows(TestCLI):
    ...
    async def test_rm__delete_rows_with_empty_sql_condition_should_fail(self):
        await self._auth()
        output = await self._tb(["datasource", "delete", "test_table", "--sql-condition"], "y", assert_exit_code=2)
...
ENGINE "MergeTree"
    ...
    @tornado.testing.gen_test
    async def test_hard_delete_workspace(self):
        await self._auth()
