...
class SupplyChainStep:
    ...
    :param concurrency_tag: Job concurrency tag. The tag must be a string ending in two digits. For example Tag02.
       The tag limits maximum concurrency of active SdviMover jobs with the same tag. The limit is the trailing two digits.
...
def create_supply_chain_marker(description, icon, color):
    ...
    >>> supplyChain.create_supply_chain_marker('Yeti-Marker', 'fad fa-thumb-tack', 'burlywood')
