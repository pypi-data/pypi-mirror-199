...
# -*- coding: utf-8 -*-
#########################################################
## Thai Language Toolkit : version  1.6.6
...
#        print(dg)
        ...
        print('Maltparser_Path = "/path/to/maltparser-1.9.2"')
        sys.exit()
...
def TextAna2json(alltext,Filename,TextOption="par",WordOption="colloc",Mode="write"):
#def TextAna2json(txtfeats,Filename):
    txtfeats = TextAna(alltext,TextOption,WordOption)
    if Mode == "append":
...
# Case  xร - xon   except  Xรน (กรน ปรน)
    if re.search(r'[ก-ฮ] ร$',charmatch) and re.search(r'.an',phone): return(False)
    return(True)
...
dt = MaltParser("เขาพูดว่าวันนี้อากาศดีนะ")
print_dtree(dt)
