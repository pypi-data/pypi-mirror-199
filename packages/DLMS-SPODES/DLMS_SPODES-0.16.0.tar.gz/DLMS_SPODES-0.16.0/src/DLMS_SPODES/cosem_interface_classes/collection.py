...
class Collection:
    ...
    def set_spec(self):
        match self.dlms_ver:
            case 6: self.__spec = func_maps["DLMS_6"]
        ...
        if self.country == CountrySpecificIdentifiers.RUSSIA:
            if self.country_ver == AppVersion(3, 0):
                self.__spec = func_maps["SPODES_3"]
            if self.manufacturer == b"KPZ" or self.server_type.contents[:3] == b'\x4d\x32\x4d':
                if self.server_ver < AppVersion(1, 4, 0):
                    self.__spec = func_maps["KPZ1"]
                else:
                    self.__spec = func_maps["KPZ"]
            pass
        else:
    ...
    def get_instance(self, class_id: ut.CosemClassId,
        ...
        return get_type(class_id, version, ln, self.__spec)(ln)
