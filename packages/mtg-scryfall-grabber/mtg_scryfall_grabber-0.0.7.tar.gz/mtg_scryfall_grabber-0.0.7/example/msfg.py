...
def main():
    ...
    if pull_price_info :
        price_output = mtg_scryfall_grabber.grab_prices(user_set=user_set, 
                                                        response_data=response_data, page_num=1,
                                                        verbose_setting=bool(args.verbose))
        mtg_scryfall_grabber.merge(master_output, price_output)
    ...
    with open("output/" + file_name, "w", encoding="UTF8") as outfile:
