...
def portfolio_from_sample(n, means, cvs, rcm_p=1, positive=True, plot=True, log2=16, bs=1, **kwargs):
    ...
    # display(sample.corr())
    if plot is True:
        scatter_matrix(sample, marker='.', s=5, alpha=1,
    ...
    SampleResult = namedtuple(
        'SampleResult', ['sample', 'port', 'df_exa', 'rcm'])
...
def add_exa_sample(self, sample, S_calculation='forwards'):
    ...
    # TODO fix the means and other stats here? Only total changes!
...
def create_bounds(port, premium, a=0, n_tps=129, s=64):
    ...
    # :-6 takes out |total
    tot = port.density_df.filter(
        regex=f'loss|p_total|exeqa_({port.line_name_pipe})').copy()
...
def allocation_ranges(self, df_exa, a=np.inf):
    ...
    hinges = coo_matrix(np.nan_to_num(np.minimum(1.0, s.reshape(
        1, len(s)) / (1.0 - self.tps.reshape(len(self.tps), 1))), nan=0.0))
