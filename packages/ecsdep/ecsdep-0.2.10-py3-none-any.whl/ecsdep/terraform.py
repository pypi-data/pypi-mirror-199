    ...
    def generate_cluster_declares (self):
        ...
        for it in ("cpu", "memory", "target-capacity"):
            if it not in autoscaling:
                autoscaling [it] = 0
        if autoscaling ["target-capacity"] > 0:
            assert autoscaling ["cpu"] == 0 and autoscaling ["memory"] == 0
        if autoscaling ["cpu"] > 0 or autoscaling ["memory"] > 0:
            assert autoscaling ["target-capacity"] == 0
