...
class PlatformAPI:
    ...
    def get_dataset(self, id: str = None, write_access=False, old=False, **kwargs) -> Union[Dataset, List[Dataset]]:
        """
        ...
        if old:
            ...
            if id is None:
                return [DatasetV1(**x, backend=self.backend, resolve_access_rights=write_access)
                        for x in r.json() if all(x.get(k) == v for k, v in kwargs.items())]
            else:
                return DatasetV1(**r.json(), backend=self.backend, resolve_access_rights=write_access)
        else:
