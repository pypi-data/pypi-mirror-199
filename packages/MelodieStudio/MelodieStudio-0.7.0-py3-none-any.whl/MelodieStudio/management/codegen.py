...
def chain_property(method: str) -> ast.Attribute:
    ...
    for attr_name in chain[1:]:
        if attr is None:
            ...
            attr = ast.Attribute(value=attr, attr=attr_name)
    pprintast(attr)
    return attr


nodes = [
    {"type": "block", "code": "a+=1", "id": 0, "next": 1},
    {"id": 1, "type": "condition", "condition": "a==1", "true": 0, "false": 1},
]

unparsed = astunparse.unparse(
    [
        ast.Assign(
            targets=[chain_property("self.id")], value=ast.Constant(value=0, kind=None)
        )
    ]
)
print(unparsed)
