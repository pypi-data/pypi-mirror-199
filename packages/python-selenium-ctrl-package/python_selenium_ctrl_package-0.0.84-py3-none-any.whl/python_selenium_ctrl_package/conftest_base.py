...
# report.when value --setup, call, teardown
    if report.when == 'call':
        if item.funcargs['screenshot_count'] > 0:       # Handle non-breakable but script didn't handle fail case, therefore whole testcase will pass
...
def test_obj(base_url, browser, browser_version, os_version, os_name, remote_flag, testrail_flag, tesults_flag,
             ...
             test_env, screenshot_option, xlsx_config, product, breakable, capture):
