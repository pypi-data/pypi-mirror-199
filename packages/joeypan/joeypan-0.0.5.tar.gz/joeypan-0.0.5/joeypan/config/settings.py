...
class InjectSettings:
    ...
    @default_log_dir.setter
    def default_log_dir(self, log_dir):
...
class Datasource(AbstractSettings):
    url: str = Field(default="mysql+pymysql://user:password@localhost:3306/foo?charset=utf8mb4")
...
class UpgradeFormat(LogFormat):
    format: str = Field(default="%(asctime)s -Loc %(filename)s -Pid %(process)d -%(name)s -%(levelname)s - %(message)s")
...
class LogFileHandler(LogHandler):
    ...
    filename: StrPath = Field(default=os.path.join(inject_settings.default_log_dir, "all.log"))
...
class LogServerHandler(LogFileHandler):
    ...
    level: int = Field(default=logging.DEBUG)
    formatter: str = Field(default="simple")
    filename: StrPath = Field(default=os.path.join(inject_settings.default_log_dir, "server.log"))
    maxBytes: int = Field(default=4194304)  # 4MB
...
class LoggerInstances(AbstractSettings):
    server: LoggerInstance = Field(default=LoggerInstance(handlers=["server_handler"]))
    platform: LoggerInstance = Field(default=LoggerInstance(handlers=["platform_handler"]))
    security: LoggerInstance = Field(default=LoggerInstance(handlers=["security_handler"]))
    audit: LoggerInstance = Field(default=LoggerInstance(handlers=["audit_handler"]))
    access: LoggerInstance = Field(default=LoggerInstance(handlers=["access_handler"]))
...
class FileLoader(SettingsLoader):
    def __int__(self, file_path: StrPath):
        self.data = file_path
...
class SettingsFactory:
    ...
    def create(cls, data_json: dict) -> UserSettings:
        name = data_json.pop("joeypan_settings_name", None) or "default_joeypan_settings"
...
if __name__ == '__main__':
    if not os.path.exists(inject_settings.default_log_dir):
        os.makedirs(inject_settings.default_log_dir)
