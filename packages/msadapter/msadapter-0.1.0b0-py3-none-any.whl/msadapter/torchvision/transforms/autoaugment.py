...
msvision_policy_mapping = {
    AutoAugmentPolicy.IMAGENET: msvision.AutoAugmentPolicy.IMAGENET,
    AutoAugmentPolicy.CIFAR10: msvision.AutoAugmentPolicy.CIFAR10,
    AutoAugmentPolicy.SVHN: msvision.AutoAugmentPolicy.SVHN,
}
...
class AutoAugment(torch.nn.Module):
    ...
    def __init__(
        ...
        if fill is None:
            self.fill = 0
        elif isinstance(fill, Sequence) and len(fill) == 1:
            self.fill = fill[0]

        self.policies = self._get_policies(policy)

        self.auto_augment_op = msvision.AutoAugment(msvision_policy_mapping[self.policy],
    ...
    def _augmentation_space(self, num_bins: int, image_size: Tuple[int, int]) -> Dict[str, Tuple[Tensor, bool]]:
        return {
            ...
            "Posterize": (8 - (torch.arange(0, num_bins) / ((num_bins - 1) / 4)).round().int(), False),
    ...
    def get_params(transform_num: int) -> Tuple[int, Tensor, Tensor]:
        ...
        """
        policy_id = int(torch.randint(0, transform_num, (1,)).item())
        probs = torch.rand((2,))
        signs = torch.randint(0, 2, (2,))

        return policy_id, probs, signs

    def __call__(self, img):
        if _is_numpy(img):
            return self.auto_augment_op(img)
...
class RandAugment(torch.nn.Module):
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        for _ in range(self.num_ops):
            op_index = int(torch.randint(0, len(op_meta), (1,)).item())
            op_name = list(op_meta.keys())[op_index]
            magnitudes, signed = op_meta[op_name]
            magnitude = float(magnitudes[self.magnitude].item()) if magnitudes.ndim > 0 else 0.0
            if signed and torch.randint(0, 2, (1,)):
                magnitude *= -1.0
            img = _apply_op(img, op_name, magnitude, interpolation=self.interpolation, fill=fill)
...
class TrivialAugmentWide(torch.nn.Module):
    ...
    def _augmentation_space(self, num_bins: int) -> Dict[str, Tuple[Tensor, bool]]:
        return {
            ...
            "Posterize": (8 - (torch.arange(0, num_bins) / ((num_bins - 1) / 6)).round().int(), False),
            "Solarize": (torch.linspace(255.0, 0.0, num_bins), False),
            "AutoContrast": (torch.tensor(0.0), False),
            "Equalize": (torch.tensor(0.0), False),
        }
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        magnitudes, signed = op_meta[op_name]
        magnitude = (
            float(magnitudes[torch.randint(0, len(magnitudes), (1,), dtype=torch.long)].item())
...
class AugMix(torch.nn.Module):
    ...
    def _augmentation_space(self, num_bins: int, image_size: Tuple[int, int]) -> Dict[str, Tuple[Tensor, bool]]:
        s = {
            ...
            "Rotate": (torch.linspace(0.0, 30.0, num_bins), True),
            "Posterize": (4 - (torch.arange(0, num_bins) / ((num_bins - 1) / 4)).round().int(), False),
            "Solarize": (torch.linspace(255.0, 0.0, num_bins), False),
            "AutoContrast": (torch.tensor(0.0), False),
            "Equalize": (torch.tensor(0.0), False),
        }
    ...
    def forward(self, orig_img: Tensor) -> Tensor:
        ...
        for i in range(self.mixture_width):
            ...
            for _ in range(depth):
                ...
                magnitudes, signed = op_meta[op_name]
                magnitude = (
                    float(magnitudes[torch.randint(0, self.severity, (1,), dtype=torch.long)].item())
                    if magnitudes.ndim > 0
                    else 0.0
                )
                if signed and torch.randint(0, 2, (1,)):
