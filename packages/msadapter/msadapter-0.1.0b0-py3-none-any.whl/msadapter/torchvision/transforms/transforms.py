...
from .functional import InterpolationMode, _interpolation_modes_from_int, \
    msvision_mode_mapping, mstransforms_dtype_mapping
...
def _is_floating_numpy(img):
    if img.dtype in (np.float16, np.float32, np.float64):
        return True
    return False

def _max_value(dtype) -> int:
    if dtype == np.uint8:
        return 255
    elif dtype == np.int8:
        return 127
...
class ConvertImageDtype(torch.nn.Module):
    ...
    def __call__(self, image):
        if _is_numpy(image):
            target_np_dtype = mstransforms_dtype_mapping[self.dtype]
            type_cast_op = mstransforms.TypeCast(target_np_dtype)

            if image.dtype == target_np_dtype:
                return image

            if _is_floating_numpy(image):
                if target_np_dtype in (np.float16, np.float32, np.float64):
                    return type_cast_op(image)

                # float to int
                if (image.dtype == np.float32 and target_np_dtype in (np.int32, np.int64)) or (
                        image.dtype == np.float64 and target_np_dtype == np.int64
                ):
                    msg = f"The cast from {image.dtype} to {target_np_dtype} cannot be performed safely."
                    raise RuntimeError(msg)

                eps = 1e-3
                max_val = float(_max_value(target_np_dtype))
                result = image.__mul__(max_val + 1.0 - eps)
                return type_cast_op(result)
            else:
                input_max = float(_max_value(image.dtype))
                if target_np_dtype in (np.float16, np.float32, np.float64):
                    image = type_cast_op(image)
                    return image / input_max

                output_max = float(_max_value(target_np_dtype))

                # int to int
                if input_max > output_max:
                    factor = int((input_max + 1) // (output_max + 1))
                    image = np.floor_divide(image, factor)
                    return type_cast_op(image)
                else:
...
class Normalize(torch.nn.Module):
    ...
    def __init__(self, mean, std, inplace=False):
        ...
        # self.normalize_op = msvision.Normalize(mean=self.mean, std=self.std, is_hwc=True)

    def __call__(self, tensor):
        if _is_numpy(tensor):
            normalize_op = msvision.Normalize(mean=self.mean, std=self.std, is_hwc=True)
            out = normalize_op(tensor)
            return out
    ...
    def __call__(self, img):
        if _is_numpy(img):
            if self.max_size is not None:
                raise NotImplementedError("The parameter 'max_size' is not supported currently when img is a ndarray.")
            if self.antialias is not None:
                raise NotImplementedError("The parameter 'antialias' is not supported currently when img is a ndarray.")
            resize_op = msvision.Resize(self.size, msvision_mode_mapping[self.interpolation])
...
class CenterCrop(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            center_crop_op = msvision.CenterCrop(self.size)
            return center_crop_op(img)
        return super().__call__(img)
...
padding_mode_mapping = {
    "constant": msvision.Border.CONSTANT,
    "edge": msvision.Border.EDGE,
    "reflect": msvision.Border.REFLECT,
    "symmetric": msvision.Border.SYMMETRIC,
}
...
class Pad(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            if isinstance(self.fill, numbers.Number) and not isinstance(self.fill, int):
                self.fill = int(self.fill)
            pad_op = msvision.Pad(self.padding, self.fill, padding_mode_mapping[self.padding_mode])
            return pad_op(img)
        return super().__call__(img)
...
class RandomCrop(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_crop_op = msvision.RandomCrop(self.size, self.padding, self.pad_if_needed, self.fill,
                                               padding_mode_mapping[self.padding_mode])
            return rand_crop_op(img)
        return super().__call__(img)
...
class RandomHorizontalFlip(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_horizontal_flip_op = msvision.RandomHorizontalFlip(self.p)
            return rand_horizontal_flip_op(img)
        return super().__call__(img)
...
class RandomVerticalFlip(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_vertical_flip_op = msvision.RandomVerticalFlip(self.p)
            return rand_vertical_flip_op(img)
        return super().__call__(img)
...
class RandomPerspective(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            # rand_perspective_op = msvision.RandomPerspective(self.distortion_scale, self.p,
            #                                                  msvision_mode_mapping[self.interpolation])
            # return rand_perspective_op(img)
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def get_params(img: Tensor, scale: List[float], ratio: List[float]) -> Tuple[int, int, int, int]:
        ...
        for _ in range(10):
            target_area = area * torch.empty(1).uniform_adpater(scale[0], scale[1]).item()
            aspect_ratio = torch.exp(torch.empty(1).uniform_adpater(log_ratio[0], log_ratio[1])).item()
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_resize_crop_op = msvision.RandomResizedCrop(self.size, self.scale, self.ratio,
                                                             msvision_mode_mapping[self.interpolation])
            return rand_resize_crop_op(img)
        return super().__call__(img)
...
class FiveCrop(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            # five_crop_op = msvision.FiveCrop(self.size)
            # return five_crop_op(img)
            img = F.to_pil_image(img)
            tl, tr, bl, br, center = F.five_crop(img, self.size)
            return np.array(tl), np.array(tr), np.array(bl), np.array(br), np.array(center)
        return super().__call__(img)
...
class TenCrop(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            img = F.to_pil_image(img)
            ten_crop_op = F.ten_crop(img, self.size, self.vertical_flip)
            out = ()
            for item in ten_crop_op:
                out += (np.array(item),)
            return out
        return super().__call__(img)
...
class LinearTransformation(torch.nn.Module):
    ...
    def __init__(self, transformation_matrix, mean_vector):
        super().__init__()
        # _log_api_usage_once(self)
        if type(transformation_matrix) != type(mean_vector):
            raise ValueError(
                f"transformation_matrix and mean_vector should be the same type, ndarray or Tensor, but got"
                f" transformation_matrix with type {type(transformation_matrix)} and mean_vector with type "
                f"{type(mean_vector)}"
            )

        if _is_numpy(transformation_matrix):
            if transformation_matrix.shape[0] != transformation_matrix.shape[1]:
                raise ValueError(
                    "transformation_matrix should be square. Got "
                    f"{tuple(transformation_matrix.shape)} rectangular matrix."
                )

            if mean_vector.shape[0] != transformation_matrix.shape[0]:
                raise ValueError(
                    f"mean_vector should have the same length {mean_vector.shape[0]}"
                    f" as any one of the dimensions of the transformation_matrix [{tuple(transformation_matrix.shape)}]"
                )
        else:
    ...
    def __call__(self, tensor):
        if _is_numpy(tensor):
            if isinstance(self.transformation_matrix, torch.Tensor):
                self.transformation_matrix = self.transformation_matrix.numpy()
            if isinstance(self.mean_vector, torch.Tensor):
                self.mean_vector = self.mean_vector.numpy()
            tensor = tensor.transpose((2, 0, 1))
            linear_transformation_op = msvision.LinearTransformation(self.transformation_matrix, self.mean_vector)
            out = linear_transformation_op(tensor)
...
class ColorJitter(torch.nn.Module):
    ...
    def get_params(
        ...
        b = None if brightness is None else float(torch.empty(1).uniform_adpater(brightness[0], brightness[1]))
        c = None if contrast is None else float(torch.empty(1).uniform_adpater(contrast[0], contrast[1]))
        s = None if saturation is None else float(torch.empty(1).uniform_adpater(saturation[0], saturation[1]))
        h = None if hue is None else float(torch.empty(1).uniform_adpater(hue[0], hue[1]))

        return fn_idx, b, c, s, h

    def __call__(self, img):
        if _is_numpy(img):
            color_jitter_op = msvision.RandomColorAdjust(self.brightness, self.contrast, self.saturation, self.hue)
            return color_jitter_op(img)
        return super().__call__(img)
...
class RandomRotation(torch.nn.Module):
    ...
    def get_params(degrees: List[float]) -> float:
        ...
        """
        angle = float(torch.empty(1).uniform_adpater(float(degrees[0]), float(degrees[1])).item())
        return angle

    def __call__(self, img):
        if _is_numpy(img):
            rand_rotation_op = msvision.RandomRotation(self.degrees, msvision_mode_mapping[self.resample],
                                                       self.expand, self.center, self.fill)
            return rand_rotation_op(img)
        return super().__call__(img)
...
class RandomAffine(torch.nn.Module):
    ...
    def get_params(
        ...
        if translate is not None:
            ...
            tx = int(round(torch.empty(1).uniform_adpater(-max_dx, max_dx).item()))
            ty = int(round(torch.empty(1).uniform_adpater(-max_dy, max_dy).item()))
            translations = (tx, ty)
        else:
            translations = (0, 0)

        if scale_ranges is not None:
            scale = float(torch.empty(1).uniform_adpater(scale_ranges[0], scale_ranges[1]).item())
        else:
            scale = 1.0

        shear_x = shear_y = 0.0
        if shears is not None:
            shear_x = float(torch.empty(1).uniform_adpater(shears[0], shears[1]).item())
            if len(shears) == 4:
                shear_y = float(torch.empty(1).uniform_adpater(shears[2], shears[3]).item())
    ...
    def __call__(self, img):
        if _is_numpy(img):
            if self.center is not None:
                raise NotImplementedError("The parameter 'center' is not supported currently when img is a ndarray.")
            rand_affine_op = msvision.RandomAffine(self.degrees, self.translate, self.scale, self.shear,
                                                   msvision_mode_mapping[self.resample], self.fill)
            return rand_affine_op(img)
        return super().__call__(img)
...
class Grayscale(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            # gray_scale_op = msvision.Grayscale(self.num_output_channels)
            # return gray_scale_op(img)
            img = F.to_pil_image(img)
            out = F.rgb_to_grayscale(img, num_output_channels=self.num_output_channels)
            return np.array(out)
        return super().__call__(img)
...
class RandomGrayscale(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            _, _, num_output_channels = get_ndarray_dimensions(img)
            # gray_scale_op = msvision.Grayscale(num_output_channels)
            if np.random.rand(1) < self.p:
                img = F.to_pil_image(img)
                out = F.rgb_to_grayscale(img, num_output_channels=num_output_channels)
                return np.array(out)
            return img
        return super().__call__(img)
...
class RandomErasing(torch.nn.Module):
    ...
    def get_params(
        ...
        log_ratio = torch.log(torch.tensor(ratio))
        for _ in range(10):
            erase_area = area * torch.empty(1).uniform_adpater(scale[0], scale[1]).item()
            aspect_ratio = torch.exp(torch.empty(1).uniform_adpater(log_ratio[0], log_ratio[1])).item()
            ...
            i = np.random.randint(0, img_h - h + 1, size=(1,)).item()
            j = np.random.randint(0, img_w - w + 1, size=(1,)).item()
            return i, j, h, w, v
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_erasing_op = msvision.RandomErasing(self.p, self.scale, self.ratio, self.value, self.inplace)
            return rand_erasing_op(img)
        return super().__call__(img)
...
class GaussianBlur(torch.nn.Module):
    ...
    def get_params(sigma_min: float, sigma_max: float) -> float:
        ...
        """
        return torch.empty(1).uniform_adpater(sigma_min, sigma_max).item()

    def __call__(self, img):
        if _is_numpy(img):
            gaussian_blur_op = msvision.GaussianBlur(self.kernel_size, self.sigma)
            return gaussian_blur_op(img)
        return super().__call__(img)
...
class RandomInvert(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            invert_op = msvision.Invert()
            if np.random.rand(1).item() < self.p:
                return invert_op(img)
            return img
        return super().__call__(img)
...
class RandomPosterize(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_posterize_op = msvision.RandomPosterize(self.bits)
            return rand_posterize_op(img)
        return super().__call__(img)
...
class RandomSolarize(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            if np.random.rand(1).item() < self.p:
                invert_op = msvision.Invert()
                inverted_img = invert_op(img)
                out = np.where(img >= self.threshold, inverted_img, img)
                return out
            return img
        return super().__call__(img)
...
class RandomAdjustSharpness(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_adjust_sharpness_op = msvision.RandomAdjustSharpness(self.sharpness_factor, self.p)
            return rand_adjust_sharpness_op(img)
        return super().__call__(img)
...
class RandomAutocontrast(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_auto_contrast_op = msvision.RandomAutoContrast(prob=self.p)
            return rand_auto_contrast_op(img)
        return super().__call__(img)
...
class RandomEqualize(torch.nn.Module):
    ...
    def __call__(self, img):
        if _is_numpy(img):
            rand_equalize_op = msvision.RandomEqualize(self.p)
            return rand_equalize_op(img)
        return super().__call__(img)
