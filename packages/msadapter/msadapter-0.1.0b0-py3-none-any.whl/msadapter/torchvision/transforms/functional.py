...
msvision_mode_mapping = {
    InterpolationMode.NEAREST: Inter.NEAREST,
    InterpolationMode.BILINEAR: Inter.BILINEAR,
    InterpolationMode.BICUBIC: Inter.BICUBIC,
}

mstransforms_dtype_mapping = {
    torch.float32: np.float,
...
def to_tensor(pic) -> Tensor:
    ...
    Note that this is not consistent with the default behavior of torch, we remain the ndarray type for
    data processing, and convert the final data to the Tensor type before use.
    """
    # if not torch.jit.is_scripting() and not torch.jit.is_tracing():
    ...
    if isinstance(pic, np.ndarray):
        ...
        #
        # # TODO: if isinstance(img, torch.ByteTensor):
    ...
    if accimage is not None and isinstance(pic, accimage.Image):
        nppic = np.zeros([pic.height, pic.width, pic.channels], dtype=np.float32)
        pic.copyto(nppic)
        # return torch.from_numpy(nppic).to(dtype=default_float_dtype)
        return nppic.astype(np.float32)
...
def convert_image_dtype(image: torch.Tensor, dtype = torch.float) -> torch.Tensor:
...
def _get_perspective_coeffs(startpoints: List[List[int]], endpoints: List[List[int]]) -> List[float]:
    ...
    res, _, _, _ = np.linalg.lstsq(a_matrix.numpy(), b_matrix.numpy())
