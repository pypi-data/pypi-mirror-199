...
from PySide2.QtWidgets import QToolButton, QMenu
...
def find_mayagpt_key():
    key_file_path = get_mayagpt_file_path()
    if key_file_path:
...
class ChatGPTMayaUI(QWidget):
    ...
    def init_ui(self):
        ...
        # Add a file menu
        # Create a menu button and a menu
        self.menu_button = QToolButton(self)
        self.menu_button.setText("File")
        self.menu = QMenu(self)

        # Add actions to the file menu
        update_api_key_action = QAction('Update API Key', self)
        update_api_key_action.triggered.connect(self.update_api_key)
        self.menu.addAction(update_api_key_action)

        open_in_browser_action = QAction('Open in Browser', self)
        open_in_browser_action.triggered.connect(self.open_in_browser)
        self.menu.addAction(open_in_browser_action)
        # Assign the menu to the menu button
        self.menu_button.setMenu(self.menu)
        self.menu_button.setPopupMode(QToolButton.InstantPopup)

        # Add the menu button to the top layout
    ...
    @staticmethod
    def open_in_browser():
        file_path = get_mayagpt_file_path()
        if not file_path:
        ...
        if current_platform == "Windows":
            ...
            subprocess.Popen(f'explorer /select,"{windows_file_path}"')
