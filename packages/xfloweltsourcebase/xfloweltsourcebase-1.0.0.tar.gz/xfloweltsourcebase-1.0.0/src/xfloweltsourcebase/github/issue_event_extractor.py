...
class IssueEventExtractor(ExtractorBase):
    def __init__(self, base_url: str, auth: Auth, dest: DataSink, evt: dict):
        super().__init__('github', '', 'issue_events', base_url, auth, dest)
        self.event = evt
        self.owner = evt['owner']
        self.fetched_at = evt['fetchedAt']
        self.urls = evt['events']
        self.repo = evt['repo']

    def extract(self):
        # print(f'To retrieve issue events for repo {self.repo}')
        ...
        for url in self.urls:
            ...
            # events.append(json.loads(resp.text))
            events = [*events, *json.loads(resp.text)]

        if len(events):
            # print(f'# of events extracted: {len(events)}. Orgnization: {self.owner}, repo: {self.repo}')
            self.dest.sink(events)
        else:
            # print(f'No events retrieved. Orgnization: {self.owner}, repo: {self.repo}')
            pass
