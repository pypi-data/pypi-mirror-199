...
class SprintIssueExtractor(ExtractorBase):
    def __init__(
        self,
        base_url: str,
        auth: Auth,
        ...
        super().__init__('jira', '', 'sprint_issues', base_url, auth, dest)

        self.owner = evt['owner']
        self.fetched_at = evt['fetchedAt']
        self.sprint = evt['sprint']
        self.url_base = f'{self.url_base}/rest/agile/1.0/sprint/'
    ...
    def extract(self):
        url = f'{self.url_base}{self.sprint}/issue'
        ...
        while True:
            ...
            if status == 429:
                # print(f'Too many request sent to JIRA for sprint issues. Owner: {self.owner}')
