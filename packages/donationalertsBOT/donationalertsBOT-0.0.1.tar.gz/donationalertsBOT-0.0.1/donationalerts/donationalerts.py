	...
	def event(self):
		def wrapper(function):
			...
			def on_message(data):
				...
				for parameter in ['id', 'alert_type', 'is_shown', 'additional_data', 'billing_system',
								  'billing_system_type', 'username', 'amount', 'amount_formatted', 'amount_main',
								  'currency', 'message', 'header', 'date_created', 'emotes', 'ap_id', '_is_test_alert',
								  'message_type', 'preset_id']:
					if not parameter in data: data[parameter] = ''
				function(
					Event(
						data["id"],
						data["alert_type"],
