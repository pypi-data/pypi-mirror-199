...
class Extractor:
    ...
    def extract_financial_index(self, cut_words):
        res = utils.max_backward_match(cut_words, self.financial_dict_keys, 15)
        res = [(elem[0], self.financial_dict[elem[0]]) for elem in res]
        return res


    def extract_fund_name(self, cut_words, word_set=None):
        res = utils.max_backward_match(cut_words, self.fund_name if word_set is None else word_set, 40)
    ...
    def extract_stock_name(self, cut_words, word_dict=None):
        ...
        keys = di.keys()
        res = utils.max_backward_match(cut_words, keys, 40)
