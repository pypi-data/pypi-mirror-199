...
class ConditionBase:
    """Base class for conditions accepted by InterpretCommand. Used to implement custom condition cases. 
    
    Subclasses must implement the `test` static method.
    """
    getter: Callable
    tokens: tuple[str]
    invert: bool
...
class InstructionCommand(commands.CommandBase):
    ...
    Requires subclasses to implement `parse_arguments` static method.

    `validate_arguments` is useful for sanity-checking arguments before execution.

    `syntax` is used to generate documentation for the command.
    """
    @staticmethod
    def parse_arguments(args: list[str]) -> list[Any]:
    ...
    @staticmethod
    def syntax() -> str:
        """Returns a string giving the syntax of the Command, as expected by `parse_arguments`.
        Note that this string should not include the name you register the command to under the InterpretCommand.
        """
        raise NotImplementedError()
