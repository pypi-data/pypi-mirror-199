...
class VltiInstrument(Instrument):
    ...
    def checkIssVltiType(self, acqTSF):
        ...
        if self.facility.a2p2client.preferences.getConfigKeys(psection):
            vltitypePref = [v for v in self.facility.a2p2client.preferences.getConfigKeys(
                psection) if self.isInRange(acqTSF.tpl, "ISS.VLTITYPE", v)]
            if vltitypePref:
                self.ui.addToLog(
                    f"Using {psection} preference value : '{vltitypePref}'")
