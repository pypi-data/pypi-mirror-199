...
class StorableModel(BaseModel):
    ...
    async def update(self: TModel, data: Dict[str, Any], skip_callback: bool = False, invalidate_cache: bool = True) -> TModel:
    ...
    @classmethod
    async def find(cls: Type[TModel], query: Optional[Dict[str, Any]] = None, **kwargs: Dict[str, Any]) -> ObjectsCursor[TModel]:
        if not query:
            query = {}
        return cls._db().meta.get_objs(
    ...
    async def aggregate(cls,
        ...
        return cls._db().meta.get_aggregated(cls.collection, pipeline, **kwargs)
    ...
    async def find_projected(cls,
        ...
        if not query:
            query = {}
        return cls._db().meta.get_objs_projected(
            cls.collection,
    ...
    async def find_one(cls: Type[TModel], query: Dict[str, Any], **kwargs: Dict[str, Any]) -> Optional[TModel]:
        return await cls._db().meta.get_obj(
            cls, cls.collection, cls._preprocess_query(query), **kwargs
        )
    ...
    async def count(cls, query: Optional[Dict[str, Any]] = None) -> int:
        if query is None:
            query = {}
        return await cls._db().meta.count_docs(cls.COLLECTION, query)

    @classmethod
    async def destroy_all(cls) -> None:
        await cls._db().meta.delete_query(cls.collection, cls._preprocess_query({}))
    ...
    async def destroy_many(cls, query: Dict[str, Any]) -> None:
        ...
        # objects
        await cls._db().meta.delete_query(cls.collection, cls._preprocess_query(query))
    ...
    async def update_many(cls, query: Dict[str, Any], attrs: Dict[str, Any]) -> None:
        ...
        # this method doesn't provide any lifecycle callback for independent
        # objects
        await cls._db().meta.update_query(cls.collection, cls._preprocess_query(query), attrs)
