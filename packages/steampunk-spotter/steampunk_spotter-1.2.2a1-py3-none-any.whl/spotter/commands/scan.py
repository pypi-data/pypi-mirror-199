...
class CheckResult:
    ...
    def construct_output(self, disable_colors: bool = False, disable_docs_url: bool = False) -> str:
        ...
        out_prefix = f"{metadata.file_name}:{metadata.line}:{metadata.column}: {result_level}"
...
def add_parser(subparsers: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    ...
    parser.add_argument(
        "--ansible-version", type=float,
        choices=[2.0, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9, 2.10, 2.11, 2.12, 2.13, 2.14, 2.15],
...
def _parser_callback(args: argparse.Namespace) -> None:
    ...
    scan(api_endpoint, storage_path, api_token, username, password, args.no_colors, args.project_id, args.config,
         config_vars, args.upload_values, args.upload_metadata, args.rewrite, args.display_level, args.no_docs_url,
         args.format, args.output, args.import_payload, args.export_payload, scan_paths)
...
def scan(api_endpoint: str, storage_path: Path, api_token: Optional[str], username: Optional[str],
         ...
         config_vars: Dict[str, Any], upload_values: bool, upload_metadata: bool, rewrite: bool,
         display_level: DisplayLevel, no_docs_url: bool, output_format: OutputFormat, output_path: Optional[Path],
         import_payload: Optional[Path], export_payload: Optional[Path], scan_paths: List[Path]) -> None:
    """
