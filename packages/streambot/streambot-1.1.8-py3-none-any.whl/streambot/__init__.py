...
class StreamBot:
    ...
    def build_req_body(self, messages, model=None):
      req_body = {
    ...
    def chat(self, messages = [], model = None):
        reqBody = self.build_req_body(messages, model)
        reqHeaders = {
        ...
        try:
            ...
            with requests.post(self.openai_url, stream=True, headers=reqHeaders, json=reqBody) as response:
                  ...
                  try:
                      ...
                      if data.get('choices',{}):
                        ...
                        if choices.get('delta',{}).get('role',{}):
                            print(self.bot_name + ": ")
                        if choices.get('delta',{}).get('content') and choices['finish_reason'] != 'stop':
                            response_text.append(choices['delta']['content'])
                            print(choices['delta']['content'], end="", flush=True)
          ...
          for event in client.events():
              try:
                  ...
                  if data.get('choices',{}):
                    ...
                    # Also check for ['finish_reason'] to break on last message
                    if choices.get('delta',{}).get('role',{}):
                        if last_yielded_message.strip() != '[START_OF_STREAM]':
                          yield '[START_OF_STREAM]' #Helper string to know when to render new chat content
                          last_yielded_message = '[START_OF_STREAM]'
                    if choices.get('delta',{}).get('content') and choices['finish_reason'] != 'stop':
                        yield choices['delta']['content']
                        last_yielded_message = choices['delta']['content']
