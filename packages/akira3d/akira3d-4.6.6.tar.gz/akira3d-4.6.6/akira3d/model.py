...
class ExBaseModel(BaseModel):
    ...
    def on_init(self):
        ...
        self.program['u_resolution'].write(glm.vec2(self.app.size))
...
class MovingCube(Cube):
    def __init__(self, app, tex_id, pos=(0, 0, 0), rot=(0, 0, 0), scale=(1, 1, 1), vao_name='cube', intelect=None,
                 hitbox=None):
        super().__init__(app, tex_id, pos, rot, scale, vao_name, hitbox)
        if intelect:
            self.ii = intelect(self)
...
class MovingUniverseModel(UniverseModel):
    def __init__(self, app, vao_name, tex_id, pos=(0,0,0), rot=(0,0,0), scale=(1,1,1),intelect=None, hitbox=None):
        super().__init__(app, vao_name, tex_id, pos, rot, scale, hitbox)
        if intelect:
            self.ii = intelect(self)
