...
class VAO:
    def __init__(self, ctx):
        ...
        for name in self.vbo.vbos:
            self.vaos[name] = self.get_vao(program = self.program.programs['default'], vbo = self.vbo.vbos[name])
            self.vaos['shadow_'+name] = self.get_vao(program=self.program.programs['shadow_map'], vbo=self.vbo.vbos[name])
