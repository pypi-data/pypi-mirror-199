...
from macrometa_source_mongo.utils import filter_streams_by_replication_method, get_attribute_type, \
...
def do_discover(client: MongoClient, config: Dict):
    ...
    if is_view:
        raise KeyError('Views are not supported.')
...
def sync_traditional_streams(client: MongoClient, traditional_streams: List[Dict], state: Dict,
    ...
    """
    Sync traditional streams that use FULL_TABLE one stream at a time.
    Args:
        client: MongoDB client instance
