...
class ETL:
    ...
    def __get_transform(self, transformer):
        ...
        non_setters_attributes = [
            f'set{attribute.replace("_", " ").title().replace(" ", "")}'
            for attribute in attributes
            if not attribute.startswith("set")
        ]
        transformer_objects = [
            "_".join(
                re.sub(r"([a-z1-9])([A-Z])", r"\1 \2", attribute).split(" ")[1:]
            ).lower()
            for attribute in list(set(setters_attributes) & set(non_setters_attributes))
        ]
