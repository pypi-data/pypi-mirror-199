...
from qudo_quantipy.core.tools.dp.prep import (
    hmerge as _hmerge,
    vmerge as _vmerge,
    recode as _recode,
    frequency as fre,
...
from qudo_quantipy.sandbox.sandbox import ChainManager
...
class DataSet(object):
    ...
    def read_confirmit_api(self, projectid, public_url, idp_url=None, client_id=None, client_secret=None, reset=True,
                           schema_vars=None, schema_filter=None, verbose=False):
        ...
        self._meta, self._data = r_confirmit_api(projectid, public_url, idp_url, client_id, client_secret, schema_vars,
                                                 schema_filter, verbose)
        self._set_file_info('', reset=reset)

    def write_confirmit(self, path_meta, path_data, schema_vars=None, verbose=False):
        """Converts qudo_quantipy dataset into Confirmit format"""
        try:
    ...
    def write_confirmit_api(self, projectid, public_url, idp_url, client_id, client_secret, schema_vars):
        """Converts qudo_quantipy dataset into Confirmit format and uploads it to the confirmit API"""
    ...
    def from_stack(self, stack, data_key=None, dk_filter=None, reset=True):
        """
        Use ``qudo_quantipy.Stack`` data and meta to create a ``DataSet`` instance.
            ...
            for tk, ed in product(self.valid_tks, [None, 'x', 'y']):
                if (any(self.text(i, True, tk, ed) == self.text(i, False, tk, ed)
                    ...
                    self.set_item_texts(a, rename_items, tk, ed)
                             ...
                             for i in sources if self.text(i, False, tk, ed)) and self.text(a, text_key=tk,
                                                                                            axis_edit=ed):
                    rename_items = {self.item_no(i): self.text(i, True, tk, ed)
                                    for i in sources if self.text(i, True, tk, ed)}
                    self.set_item_texts(a, rename_items, tk, ed)
    ...
    def compare(self, dataset, variables=None, strict=False, text_key=None):
        ...
        Parameters
        ----------
        dataset : qudo_quantipy.DataSet instance
    ...
    def weight(self, weight_scheme, weight_name='weight', unique_key='identity',
        ...
        Parameters
        ----------
        weight_scheme : qudo_quantipy.Rim instance
    ...
    def hmerge(self, dataset, on=None, left_on=None, right_on=None,
        ...
        Returns
        -------
        None or new_dataset : ``qudo_quantipy.DataSet``
    ...
    def vmerge(self, dataset, on=None, left_on=None, right_on=None,
        ...
        Parameters
        ----------
        dataset : (A list of multiple) ``qudo_quantipy.DataSet``
