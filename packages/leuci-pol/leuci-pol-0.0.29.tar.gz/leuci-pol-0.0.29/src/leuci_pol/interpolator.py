...
class Interpolator(ABC):
    ...
    def build_cube_around(self, x, y, z, width,shift=0):        
        # 1. Build the points around the centre as a cube - width points
        vals = []            
        xp,yp,zp = 0,0,0         
        start = int(-1*width/2 + 1)
        end = int(width/2 + 1)
        if shift < 0:
            end = end + shift
        elif shift > 0:
...
class Multivariate(Interpolator):                
    ...
    def make_coeffs(self, x, y, z,shift=0):                                        
        # 1. Build the points around the centre as a cube - 8 points
        vals = self.build_cube_around(x, y, z, self.points,shift)
        #2. Multiply with the precomputed matrix to find the multivariate polynomial
        ABC = self.mult_vector(self.inv.get_invariant(), vals)
        # 3. Put the 8 values back into a cube
        polyCoeffs = np.zeros((self.points-abs(shift), self.points-abs(shift), self.points-abs(shift)))
        pos = 0
        for i in range(self.points-abs(shift)):
            for j in range(self.points-abs(shift)):         
                for k in range(self.points-abs(shift)):             
...
class Mspline(Multivariate):
    def init(self):
        self.points = 4
        self.dimsize = math.pow(self.points, 4)        
        self.inv = iv3.InvariantVandermonde()        
        self.need_new = True
        self._xfloor = -1
        self._yfloor = -1
    ...
    def get_value(self, x, y, z):
        xn,yn,zn,coeffs = self.make_coeffs(x,y,z)
        a = self.get_value_multivariate(zn, yn, xn, coeffs)
        coeffs_1 = self.rev_mat(coeffs)
        b = self.get_value_multivariate(zn, yn, xn, coeffs_1)
        return (a+b)/2
    ...
    def rev_mat(self,mat):
        i,j,k = mat.shape
        revved = np.zeros((i,j,k))
        for x in range(i):
            for y in range(j):
                for z in range(k):
                    revved[x,y,z] = mat[z,y,x]
        return revved
...
class Bspline(Interpolator):                
    ...
    *******************************************************************************
    ...
    def make_periodic_coeffs(self, inplace):           
        ...
        if inplace:
            ...
            self.create_coeffs_copy(tmpF,tmpM,tmpS)        
            self._coeffs = self.reduce_vals_with_buffer(self._buffer, self._coeffs)
    ...
    def create_coeffs_inplace(self, thisF, thisM, thisS):
        ...
        for y in range(thisM):        
            for z in range(thisS):                        
                ...
                self.convert_to_interp_coeffs_inplace(pole, num_poles, thisF, row,self.mirror)
        #Y-wise
        for x in range(thisF):        
            for z in range(thisS):                        
                row = self._coeffs[x:x+1,:,z:z+1].reshape([thisM])
                self.convert_to_interp_coeffs_inplace(pole, num_poles, thisM, row,self.mirror)
        #Z-wise
        for x in range(thisF):
            for y in range(thisM):                        
                row = self._coeffs[x:x+1,y:y+1,:].reshape([thisS])
                line = self.convert_to_interp_coeffs_inplace(pole, num_poles, thisS, row,self.mirror)
    ...
    def convert_to_interp_coeffs_inplace(self,pole, num_poles, width, row,mirror):        
