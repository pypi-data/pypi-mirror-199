...
class DIP(BaseModel):
    ...
    def __init__(self, env:Environment=None, **kwargs):
        ...
        if 'source' not in kwargs:
            ...
            if caller.filename == '<stdin>':                
                kwargs['source'] = Source(lineno=caller.lineno, filename=os.getcwd())
            else:
                kwargs['source'] = Source(lineno=caller.lineno, filename=caller.filename)
        if 'docs' in kwargs:
            kwargs['env'].docs = kwargs['docs']
            del kwargs['docs']
    ...
    def parse(self):
        ...
        for node in self.env.nodes:
            ...
            if node.defined and node.value is None and not self.env.docs:
