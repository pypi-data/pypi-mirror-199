...
import sys
from FeSeEL.utils import get_numerical_features, get_categorical_features
                        ...
                        pd.DataFrame(
                            {
                                ...
                                "value": numerical_categorical_correlation(
                                    df[var2].fillna("Missing"), df[var1].fillna(0)
                                ),
                            }
                        ),
                    ]
...
def Cramers_pair_correlation(feature_1, feature_2, df, verbose):
    ...
    if verbose:
        print(feature_1, feature_2, correlation)
...
def remove_correlated(
    ...
    if flat_corr_db.loc[lambda x: x.value > correlation_threshold].shape[0] > 0:
        ...
        if verbose:
            if prog_bar is None:
                print(f"The feature removed is: {feat_to_drop}")
            else:
...
def vif_ok_cycle(db, vif_threshold=5, verbose=True, prog_bar=None):
    ...
    tmp = add_constant(db)
    results = pd.Series(
        [variance_inflation_factor(tmp.fillna(0).values, i) for i in range(tmp.shape[1])], index=tmp.columns
    )
...
def recursive_vif(
    ...
    for ix, col in enumerate(pb):
        vif_col = variance_inflation_factor(tmp.fillna(0).values, tmp.columns.get_loc(col))
