...
import re
from FeSeEL.utils import remove_features, time_step, get_categorical_features
...
def Shap_RFE_full(
    ...
    if algo_type == "classification":
        try:
            CVSel_algo = cv_funct(model, grid, n_iter=n_iter, cv=cv_type, scoring="roc_auc")
        except Exception:
            CVSel_algo = cv_funct(model, grid, cv=cv_type, scoring="roc_auc")  # for GridSearchCV
    elif algo_type == "regression":
        if str(cv_type.__class__()).startswith("StratifiedKFold"):
            raise ValueError("Fold Cross Validation uncorrect for regression algorithm, use KFold() or GroupKFold()")
        try:
            CVSel_algo = cv_funct(model, grid, n_iter=n_iter, cv=cv_type, scoring="r2")
        except Exception:
            CVSel_algo = cv_funct(model, grid, cv=cv_type, scoring="r2")  # for GridSearchCV
    elif algo_type == "multiclass":
        try:
            CVSel_algo = cv_funct(model, grid, n_iter=n_iter, cv=cv_type, scoring="balanced_accuracy")
        except Exception:
            CVSel_algo = cv_funct(model, grid, cv=cv_type, scoring="balanced_accuracy")  # for GridSearchCV
    else:
        ...
        # Categorical Preprocessing without LGBM - Hot-One-Encoder
            ...
            for col in cats_feat:
                if X_train_tsf[col].nunique()>dim_cat_threshold:
                    values_cat = list(X_train_tsf.groupby(col,sort=False)[col].count().sort_values(ascending=False).index)[:dim_cat_threshold]
