...
import sys
from FeSeEL.utils import get_categorical_features, time_step
...
def retrieve_psis(base_df, compare_df, features, psi_nbins=20, psi_bin_min_pct=0.02):
    ...
    for ix, col in enumerate(features_pb):
        if col in get_categorical_features(base_df):
            base_df = base_df.fillna("Missing")
            compare_df = compare_df.fillna("Missing")
            ...
            base_df = base_df.fillna(-999999)
            compare_df = compare_df.fillna(-999999)
