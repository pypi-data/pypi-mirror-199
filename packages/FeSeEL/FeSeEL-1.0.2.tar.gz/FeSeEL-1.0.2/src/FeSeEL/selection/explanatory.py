...
from FeSeEL.utils import get_categorical_features
from FeSeEL.selection.multivariate import numerical_categorical_correlation


# @time_step
def find_not_explanatory(X_train, X_test, y_train, y_test, threshold=0.05, algo_type="classification", dim_cat_threshold=10):
    ...
    if algo_type == "classification":
        return find_not_explanatory_auc(X_train, X_test, y_train, y_test, min_auc_tgt=auc, dim_cat_threshold=dim_cat_threshold)
    elif algo_type == "regression":
        return find_not_explanatory_corr(X_train, X_test, y_train, y_test, min_corr_tgt=threshold)
    else:
        return find_not_explanatory_auc_multi(X_train, X_test, y_train, y_test, min_auc_tgt=auc, dim_cat_threshold=dim_cat_threshold)
...
def find_not_explanatory_corr(X_train, X_test, y_train, y_test, min_corr_tgt=0.05):
    ...
    for ix, col in enumerate(train_col_pb):
        if col in get_categorical_features(X_train):
            if with_test:
                results[col] = [
                    numerical_categorical_correlation(X_train[col].fillna(0), y_train),
                    numerical_categorical_correlation(X_test[col].fillna(0), y_test)
                ]
            else:
                results[col] = numerical_categorical_correlation(X_train[col].fillna(0), y_train)
        else:
            if with_test:
                results[col] = [
