...
def sensitivity_analysis(df: DataFrame = pd.DataFrame(), 
        ...
        for effect in effects:
            ...
            if has_year and has_week:
                temporal = yearly_season(bootstrap_samples = simulations, intervention = test_period, 
                                        predict_df = temp_test, trend = True)
                prio_simulations = weekly_season(bootstrap_samples = temporal, intervention = test_period, 
                                        predict_df = temp_test, trend = False)
            elif has_year:
                prio_simulations = yearly_season(bootstrap_samples = simulations, intervention = test_period, 
                                        predict_df = temp_test)
            elif has_week:
                prio_simulations = weekly_season(bootstrap_samples = simulations, intervention = test_period, 
                                        predict_df = temp_test)
            else:
                prio_simulations = simulations.copy()
            
            stadisticts, stats_ranges, samples_means = bootstrap_p_value(control = temp_test[test_mask].yhat, 
                                                                                        treatment = temp_test[test_mask].y, 
                                                                                        simulations = prio_simulations
                                                                                        )
