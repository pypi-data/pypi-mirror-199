...
class tscausalinference:
    ...
    def run(self):
        ...
        my_dict = self.hyper_parameters.copy()
        
        # Check if there are any keys with the word "year"
        has_year = any("year" in key for key in my_dict.keys())

        # Check if there are any keys with the word "week"
        has_week = any("week" in key for key in my_dict.keys())

        # Execute functions based on keys
        if has_year and has_week:
            temporal = yearly_season(bootstrap_samples = self.simulations, intervention = self.intervention, 
                                     predict_df = self.data, trend = True)
            self.prio_simulations = weekly_season(bootstrap_samples = temporal, intervention = self.intervention, 
                                     predict_df = self.data, trend = False)
        elif has_year:
            self.prio_simulations = yearly_season(bootstrap_samples = self.simulations, intervention = self.intervention, 
                                     predict_df = self.data)
        elif has_week:
            self.prio_simulations = weekly_season(bootstrap_samples = self.simulations, intervention = self.intervention, 
                                     predict_df = self.data)
        else:
            self.prio_simulations = self.simulations.copy()
        ...
        self.stadisticts, self.stats_ranges, self.samples_means = bootstrap_p_value(control = self.data.query(self.string_filter).yhat, 
                                                                                    ...
                                                                                    simulations = self.prio_simulations
                                                                                    )
