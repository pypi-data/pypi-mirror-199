...
def key_splitter(key: str):
    ...
    return project_name, filepath, database_name, table_name, filetype
...
def paths_to_tables(list_of_objects):
    ...
    for item in list_of_objects:
        ...
        try:
            ...
            paths[input_path]["project_name"] = project_name
...
if __name__ == "__main__":
    ...
    response = list_of_data_objects_to_process(
        bucket=args["source_bucket"], project=args["project"]
    )
    ...
    if len(response) > 0:
        ...
        for path in paths:
            destination_cleaner(
                bucket=args["destination_bucket"], path=path.replace("raw_history/", "")
            )
            ...
            if args["multiple_db_in_bucket"] == "True":
                db_name = (
                    paths[path]["project_name"] + "_" + paths[path]["database_name"]
                )
            else:
                db_name = paths[path]["project_name"]
