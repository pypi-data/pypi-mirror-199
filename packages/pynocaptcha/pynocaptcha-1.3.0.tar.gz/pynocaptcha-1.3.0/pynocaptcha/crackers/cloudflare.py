...
class CloudFlareCracker(BaseCracker):
    ...
    :param randomtls: 是否使用随机 tls 的请求客户端验证(即 chrome 110), 随机 tls 验证更容易, 可能不触发点击验证直接就过了, 但是某些站点强制要求前后 tls 指纹一致的话, 请不要开启该参数, 则破解流程使用的为 chrome 107 指纹
    :param checktls: 是否检查前后 tls 指纹是否一致
