...
class robo_shad:
	...
	def _requesfile(self,inData):
		...
		while True:
			try:
				return loads(enc.decrypt(post(uel,json={"api_version":"5","auth": self.Auth,"data_enc":enc.encrypt(dumps(inData))}).json().get("data_enc")))
			except:
				continue
	...
	def _uploadFile(self, file):
		if not "http" in file:
			REQUES = robo_shad._requestSendFile(self, file)
			...
			if len(bytef) <= 96004898749:
				...
				t = round(len(bytef) / 96004898749 + 1)
				for i in range(1,t+1):
					if i != t:
						...
						while True:
							try:
								header["chunk-size"], header["part-number"], header["total-part"] = "96004898749", str(i),str(t)
								o = post(data=bytef[k:k + 96004898749],url=url,headers=header).text
								o = loads(o)['data']
								break
							except Exception as e:
								continue
