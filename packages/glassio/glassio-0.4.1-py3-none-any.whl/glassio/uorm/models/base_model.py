...
class BaseModel(MetaModel):
    ...
    def __init__(self, attrs: Optional[Dict[str, Any]] = None, **_kwargs: Any) -> None:
        if attrs is None:
            attrs = {}
    ...
    @staticmethod
    async def __get_possible_databases() -> list["Shard"]:
        _db = await db()
        return [_db.meta]

    @classmethod
    async def ensure_indexes(cls, loud: bool = False, overwrite: bool = False) -> None:
        dbs = await cls.__get_possible_databases()
