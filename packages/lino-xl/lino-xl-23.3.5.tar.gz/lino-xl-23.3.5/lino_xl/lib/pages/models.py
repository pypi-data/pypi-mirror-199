...
class Page(Hierarchical, Sequenced, Previewable, Publishable):
    ...
    child_node_depth = models.IntegerField(default=0)
    # body = dd.BabelTextField(_("Body"), blank=True, format='plain')
    # raw_html = models.BooleanField(_("raw html"), default=False)

    def as_page(self, ar, display_mode="list", hlevel=1):
        if display_mode == "detail":
            yield "<h{0}>{1}</h{0}>".format(hlevel, self.title)
            yield self.body_full_preview
            for obj in self.children.order_by('seqno'):
                for i in obj.as_page(ar, "list", hlevel=hlevel+1):
                    yield i
        else:
            # the whole list item is clickable
            url = ar.renderer.obj2url(ar, self)
            if url is not None:
                yield """<a href="{}"
    ...
    def get_child_nodes_in_order(self):

        def get_next_layer(depth, parent, qs):
            if depth >= 1:
                objs = self.__class__.objects.filter(parent=parent).order_by('seqno')
                if objs.count():
                    for obj in objs:
                        qs.append(obj)
                        depth_from_parent = depth - 1
                        if obj.child_node_depth > depth_from_parent:
                            depth_from_parent = obj.child_node_depth
                        qs.extend(get_next_layer(depth_from_parent, obj, []))
            return qs

        return get_next_layer(self.child_node_depth, self, [])
...
class PageDetail(dd.DetailLayout):
    ...
    title child_node_depth
    body:70
    pages.PagesByParent:30
    """
...
PublisherViews.add_item_lazy("p", PublicPages)
