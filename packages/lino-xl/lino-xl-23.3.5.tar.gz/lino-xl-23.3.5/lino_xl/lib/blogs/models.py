...
class Entry(UserAuthored, Controllable, CombinedDateTime,
    ...
    def get_primary_image(self):
        ...
        file_type = ContentType.objects.get_for_model(rt.models.albums.File)
        qs = Mention.objects.filter(**gfk2lookup(
    ...
    def as_page(self, ar):
        ...
        yield """<span><em>{}</em></span>""".format(pub_info)
        yield """<hr/>"""
    ...
    def as_paragraph(self, ar):
        ...
        url = escape(ar.renderer.obj2url(ar, self))
        s = """<a href="{}"><strong>{}</strong> — """.format(url, ar.get_title())
        s += """<span style="font-size:70%;"><em>{}</em></span></a>""".format(pub_info)
        s += """<div style="display:flex;">"""
        if img is not None:
            s += f"""<div><a href="{url}">{img.thumbnail}</a></div>"""
        else:
            s += """<div>no image</div>"""
        s += """<div style="margin-left:20px;"><p style="text-align:justify;">"""
