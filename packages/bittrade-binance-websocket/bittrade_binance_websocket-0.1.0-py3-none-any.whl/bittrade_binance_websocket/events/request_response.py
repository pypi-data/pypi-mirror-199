...
def add_keys(secret: str, key: str) -> Callable[[Observable[EnhancedWebsocket]], Observable[EnhancedWebsocket]]:
...
def wait_for_response(id: str, timeout: float) -> Callable[[Observable[R]], Observable[R]]:
    return compose(
...
def _response_ok(response: R, good_status: Any ="ok", bad_statuses: List[Any]=["error"]):
    r = cast(Dict, response)
    try:
        if r['status'] in bad_statuses:
            raise RequestResponseError(r['errorMessage'])
...
def response_ok(good_status=200, bad_statuses=[400, 409]) -> Callable[[Observable[R]], Observable[R]]:
    return operators.map(lambda response: _response_ok(response, good_status, bad_statuses))