...
def cvImg2bytes(img):
  ...
  img_PIL = PIL.Image.fromarray(img, 'RGBA')
  iobuf = io.BytesIO()
  # format bbox into png for return
  img_PIL.save(iobuf, format='png')
  # format return string
  ret = 'data:image/png;base64,{}'.format((str(b64encode(iobuf.getvalue()), 'utf-8')))
  return ret
...
def video_frame(label, imgData):
  data = eval_js(f'stream_frame("{label}", "{imgData}")')
  return data

def cvImg2bbox(cvImg):
  cvImg[:,:,3] = (cvImg.max(axis = 2) > 0 ).astype(int) * 255
  # convert overlay of bbox into bytes
  return cvImg2bytes(cvImg)
