...
def dns_resolver_measure():
    ...
    if domain_extra_has == 'YES' or domain_extra_has == 'y' or domain_extra_has == 'yes' or domain_extra_has == 'Y':
        domain_finish = input('\nHave you added all desired domains for testing[y/n]: \n')
    ...
    if resolver_extra_has == 'YES' or resolver_extra_has == 'y' or resolver_extra_has == 'yes' or resolver_extra_has == 'Y':
        resolver_finish = input('\nHave you added all desired resolvers for testing[y/n]: \n')
    ...
    if not (transfer_agreement == 'YES' or transfer_agreement == 'y' or transfer_agreement == 'yes' or transfer_agreement == 'Y'):
    ...
    for resolver in customized_resolvers:
    ...
    for domain in domains.keys():
        ...
        for resolver_index in range(len(resolvers)):
            ...
            output_file.write(str(domains[domain][1][resolver_index]) + SPACE + str(domains[domain][0][resolver_index]) + SPACE +
    ...
    for cdn in cdn_map_tcp_delay.keys():
        for resolver in resolvers:
            if cdn == CDNS[0]:
                cdn_map_tcp_delay[cdn][resolver] = cdn_map_tcp_delay[cdn][resolver] / cdn_map_tcp_delay_count[cdn][resolver]
            elif cdn == CDNS[1]:
                cdn_map_tcp_delay[cdn][resolver] = cdn_map_tcp_delay[cdn][resolver] / cdn_map_tcp_delay_count[cdn][resolver]
            elif cdn == CDNS[2]:
                cdn_map_tcp_delay[cdn][resolver] = cdn_map_tcp_delay[cdn][resolver] / cdn_map_tcp_delay_count[cdn][resolver]
