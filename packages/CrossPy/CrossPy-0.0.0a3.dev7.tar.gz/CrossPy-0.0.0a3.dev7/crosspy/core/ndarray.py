    ...
    def __getitem__(self, index: IndexType):  # -> Union[Array, List[Array]]
        ...
        if self._concat_dim == 0 and len(index) == 1:
            if isinstance(index[0], slice):
                ...
                assert is_partial_head == (left < len(self._original_data) and head_start)
                assert is_partial_tail == (right < len(self._original_data) and tail_stop) # partial tail <-> tail stop != 0
