    ...
    *,
    ...
    if is_array(obj):  # numpy, cupy, crosspy
        if partition is not None:
            obj = partition_with(obj, partition=partition, wrapper=wrapper, unwrapper=unwrapper)
...
def distribute_to(arr, placement, wrapper=lambda x: x, unwrapper=lambda x: x):
...
def partition_with(arr, partition, wrapper=lambda x: x, unwrapper=lambda x: x):
