...
class EtsySession:
    ...
    def __init__(self,
        ...
        if token is not None:
            ...
            self.__async_oauth2_session = self.__create_async_oauth2_session()
            self.__me = None
    ...
    def __create_async_oauth2_session(self) -> OAuth2Session:
        refresh_kwargs = {
            'client_id': self.CLIENT_KEY,
            'client_secret': self.CLIENT_SECRET,
        }
    ...
    async def async_request(self, endpoint: str, method="GET", headers: Optional[dict] = None, *args, **kwargs) -> dict:
        
        url = f"{self.__base_endpoint}{endpoint}"
        if headers is None:
            headers = {"x-api-key": self.CLIENT_KEY}

        async with self.__async_oauth2_session.request(url=url, method=method, headers=headers, *args, **kwargs) as res:
            
            json = await res.json()
            
            if "error" in json.keys():
                raise EtsyRequestException(res.status_code, json["error"])
