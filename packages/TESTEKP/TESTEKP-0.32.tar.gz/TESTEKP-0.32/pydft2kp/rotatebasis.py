...
class basis_transform():
    '''
    This module compares the QSYMM and IRREP data to
    find the unitary matrix U that transforms
    the DFT basis into the representation informed
    to te QSYMM package.
    '''

    def __init__(self, qsymm, irrep, nullspace_thresh=1e-4, diagonal=True):
        '''
        ...
        num_phases : int
            ...
            compose U.
        U : callable (phi[0], ..., phi[num_phases-1])
            Calculates the matrix U as a linear combination of the
            null space entities. The coefficients are set by the phases.
        Q : array
    ...
    def match_qsymm_irrep(self, qsymm, irrep):
        ...
        Parameters
        ----------
        qsymm, irrep: the objects described in __init__()
        ...
        for i in range(len(qsymm.symm_pg)):
            for j in range(len(irrep.symm_pg)):
                # if labels (1,I,S,R,M) don't match, keep searching..
                if qsymm.symm_pg[i][0] != irrep.symm_pg[j][0]: continue
                # if angles don't match, keep searching...
                if qsymm.symm_pg[i][1] != irrep.symm_pg[j][1]: continue
                # if labels and angle match, check axis
                qs_axis = array(qsymm.symm_pg[i][2], dtype=float)
                ...
                if (qsymm.symm_pg[i][0] == '1') or (qsymm.symm_pg[i][0] == 'I'):
                    # (1,I): axis is irrelevant, always [0,0,1]
                    table += [[i,j]]
                elif qsymm.symm_pg[i][1] == 180: # both M and C2 have angle = 180
    ...
    def get_basis_transform_with_trs(self, qsymm, irrep, nullspace_thresh=1e-4):
        '''
        Finds the set of transformation matrices US that compose the 
        transformation U as a linear combination. This is the generalized
    ...
    def errorU(self, pars, qs, kp, diagonal=True):
        '''
        ...
        if diagonal:
            emin = kp.energies[kp.setA][0]
            emax = kp.energies[kp.setA][-1] # assuming ordered
            # scale energies to 0..1
            energies = (kp.energies[kp.setA] - emin) / (emax-emin)
    ...
    def findU(self, qs, kp, diagonal=True):
        '''
        ...
        # minimizes the error
        sol = minimize(self.errorU, x0, tol=1e-12, args=(qs, kp, diagonal))
        # returns the solution and U
        return sol, self.getU(sol.x)

    def getHeff(self, qs, kp):
        '''
        ...
        ----------
        Hdict : dict
            The Hamiltonian as a dict of terms of k-powers.
        ...
        Heff : callable
            ...
            as a function of k.
        '''
        coeffs, keys = self.get_coeffs(qs, kp)
        tosub = []
        for n in range(len(coeffs)):
            tosub += [('c'+str(n), coeffs[n])]
        return coeffs, keys, lambdify(['k_x', 'k_y', 'k_z'], qs.Hmodel.subs(tosub))
    
    def get_coeffs(self, qs, kp):
        '''
        ...
        for i in range(N):
            for j in range(N):
                for qsk, qek in zip(QSkeys, DFTkeys):
                    line = [q[qsk][i,j] for q in qs.model]
                    Haux = self.U @ kp.Hdict[qek] @ self.U.T.conj()
        ...
        ncoeffs = len(qs.model)
        keys = {}
        for n in range(ncoeffs):
            keys[n] = [] # init empty
        for qsk, qek in zip(QSkeys, DFTkeys):
            if qek not in kp.Hdict.keys():
                continue
            for n in range(ncoeffs):
                ij = argwhere(abs(qs.model[n][qsk]) > 1e-4)
                if len(ij) > 0:
                    keys[n] += [qek]
