...
def R_to_spin(R):
    '''
    Calculates the spin inv/rotation matrix from
    the O(3) inv/rotation matrix in r-space.
...
def R_to_bvec(R, bs):
    '''
    Calculates the rotation matrix in the basis
...
def convert_units_coeffs(coeffs, keys, verbose=True, sigdigits=5):
    ...
    verbose : bool
        If true, prints a report
    sigdigits : int
        Number of significant digits printed in the report
    
    Returns
    -------
    coeffs_with_units : array
...
def convertpi(thestr):
    '''
    Uses sympy to convert a string (e.g. 2pi/3) into a numerical angle
    in degrees (e.g. 120).
    '''
    # import locally for compatibility
    ...
    return round(parse_expr(thestr,transformations=transformation).subs(pi,180))


###############################################
# DEFINE A CONTEXT TO LOCALY CHANGE DIR
###############################################
import os
from contextlib import contextmanager

#based on: https://stackoverflow.com/questions/299446/
@contextmanager
def cwd(path):
    try:
        oldpwd = os.getcwd()
