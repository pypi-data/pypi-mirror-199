...
def getHpowers(kp, NB=None, maxorder=2):
    '''
    Returns a dictionary representing H terms that multiply
    ...
    '''
    H0 = np.diag(kp.energies)
    # factor 2 below due to H = H0 + 2k.p + k²
    return lowdin(kp.setA, H0, 2*kp.px, 2*kp.py, 2*kp.pz, NB, maxorder)

# Build Hamiltonians
def H_of_k(kp, Hpow=None):
    '''
    Builds and return a function that calculates H(k).

    Using Rydberg units (hbar = 1, m = 1/2).
    The kp Hamiltonian is: H = H0 + 2k.p + k²,
    where H0 contains the QE eigenstates.

    If Hpow is none, the model will use all bands and 1st order Löwdin scheme.
    If Hpow is informed, it builds the model from its Löwdin expansion.
    ...
    Hpow : dict
        Dictionary with the matrices that multiply the powers of momentum.
        Generated by getHpowers(...)

    Returns
    -------
    Callable H(kx, ky, kz, [maxorder=2]).
    
    The option maxorder is available only if Hpow is informed.
    '''

    if Hpow is None: # returns full H
        H0 = np.diag(kp.energies)
        I0 = np.eye(len(kp.energies))
        def H(kx=0, ky=0, kz=0):
            k2 = kx**2 + ky**2 + kz**2
            # building H = H0 + 2k.p + k²
            return (H0 + k2*I0 + 2*(kp.px*kx + kp.py*ky + kp.pz*kz))
        return H
...
def lowdin(A, H0, Hx, Hy, Hz, NB=None, maxorder=2):
    ...
    INPUT:
        A: (int list/array) set of states considered as Löwdin's set A
        H0: (NxN array) assuming diagonal
        Hx, Hy, Hz: (NxN arrays) perturbation terms proportional to kx, ky, kz
        NB: number of bands in set B [below, and above] A
        maxorder: calculate the expansion up to this order
