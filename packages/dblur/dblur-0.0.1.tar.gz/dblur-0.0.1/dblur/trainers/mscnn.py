...
from data.augment import RandomCrop, RandomHorizontalFlip, RotationTransform, RandomSwapColorChannels, RandomSaturation, \
    AddRandomNoise, RandomVerticalFlip
from trainers.base_trainer import BaseTrainer
from models.mscnn import MSCNN, MSCNNDiscriminator
from losses.mscnn import MSCNNLoss, MSCNNDiscriminatorLoss
from schedulers.mscnn import MSCNNScheduler
...
class MSCNNTrainer(BaseTrainer):
    ...
    def train(
        ...
        if val_dataloader is None:
            raise Exception("val_dataloader was not provided.")
        ...
        if model is None and model_optimizer is None:
            ...
            optimizer = self.get_model_optimizer(model.parameters())
        ...
        for epoch in range(epochs):
            ...
            for train_batch, (X, y) in enumerate(train_dataloader):
                ...
                if lr_scheduler is not None and scheduler_step_every_batch == True:
                    lr_scheduler.step()

            if lr_scheduler is not None and scheduler_step_every_batch == False:
                lr_scheduler.step()
    ...
    def get_model_optimizer(self, params, learning_rate=5e-5, betas=(0.9, 0.999), weight_decay=0):
        """Returns Adam optimizer used for training MSCNN specified by the 
        arguments below:
    ...
    def get_discriminator_optimizer(self, params, learning_rate=5e-5, betas=(0.9, 0.999), weight_decay=0):
        """Returns Adam optimizer used for MSCNN discriminator which is used in 
        training.
    ...
    def get_lr_scheduler(self, optimizer, steps_for_change=3e5, decrease_factor=0.1):
        """Learning rate scheduler used for training MSCNN referred to in the
      paper."""
