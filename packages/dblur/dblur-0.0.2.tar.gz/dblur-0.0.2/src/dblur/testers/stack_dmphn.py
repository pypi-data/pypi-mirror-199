...
from models.stack_dmphn import StackDMPHN, Encoder, Decoder
from losses.stack_dmphn import StackDMPHNLoss
import torch
...
class StackDMPHNTester(BaseTester):
    ...
    def sliding_window_deblur(self, model, blur_img, window_size=256, overlap_size=0):
        ...
        pred = [torch.zeros(blur_img.shape).to(device) for i in range(num_of_stacks)]  # change here
