    ...
    try:
        ...
        if update == False and potential_key_found == True:
            ...
            if existing_mongo_id:
                ...
                # now write the record to the historical collection
                written_object = history_collection.insert_one(history_object)
            ...
            myobjectid = collection.insert_one(document)
        # now fetch the record we just wrote so that we write it back to the
        # DB.
        myobject = collection.find_one({'_id': myobjectid.inserted_id}) 
        response_dict['code'] = 200
        ...
        for row in csvhandle:
            ...
            if rowindex == 0:
                ...
                try:
                    ...
                    myobjectid = collection.insert_one(kwargs)
                    success += 1
                except Exception:
