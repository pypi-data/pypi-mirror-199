...
class PrSpiders(settions):
    ...
    def Requests(cls, url=None, callback=None, headers=None, retry_time=3, method='GET', meta=None,
        ...
        cls.SpiderPool(cls, request=request, headers=headers, retry_time=retry_time, method=method, meta=meta,
                       encoding=encoding, retry_interval=retry_interval, timeout=timeout, **kwargs)
        # for requ in request:
        #     print(requ, cls.executor)
        #     # cls.fetch(requ, **kwargs)
        #     cls.executor.submit(cls.fetch, requ, **kwargs)
    ...
    def SpiderPool(cls, request, headers=None, retry_time=3, method='GET', meta=None,
                   encoding='utf-8', retry_interval=1, timeout=3, **kwargs):
        for u in request:
            cls.executor.submit(cls.fetch(request=u, headers=None, retry_time=3, method='GET', meta=None,
                                          encoding='utf-8', retry_interval=1, timeout=3, **kwargs))
