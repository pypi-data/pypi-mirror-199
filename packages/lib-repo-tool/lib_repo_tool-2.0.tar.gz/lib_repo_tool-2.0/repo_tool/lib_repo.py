...
class LibData:
    ...
    def gen_path(self):
        ...
        return os.path.join(self.name, self.version, 'lib', self.platform, self.arch, self.build)
    ...
    def gen_local_path(self):
        ...
        return os.path.join(self.name, self.version, 'lib', self.arch, self.build)
    
    def is_equal(self, other):
        '''
        Not compare build number
        '''
        return self.name == other.name and self.version == other.version and self.platform == other.platform and \
            self.arch == other.arch and self.build == other.build and self.lib_type == other.lib_type
    ...
    def __str__(self):
        return f'{self.name}\t{self.version}\t{self.platform}\t{self.arch}\t{self.build}\t{self.lib_type}\t{self.datetime}'
...
class LibRepo:
    ...
    def bulk_remove_libs(self, libs: List[LibData]):
        ...
        file_keys.extend([l.file_key_for_header() for l in libs])
        file_keys.extend([l.file_key_for_source() for l in libs])
    ...
    def query_libs(self, name: str=None, version: str=None, platform: str=None, arch: str=None, build: str=None) -> list[LibData]:
        if not self.libs:
            return []
    ...
    def __str__(self):
        def sort_key(lib: LibData):
            return f'{lib.name}-{lib.version}-{lib.platform}-{lib.arch}-{lib.build}-{lib.lib_type}-{lib.datetime}'
        ...
        for l in sorted_libs:
            cells = [l.name, l.version, l.platform, l.arch or '', l.build or '', l.lib_type or '', l.datetime or '']
...
def parse_args():
    ...
    add_parser.add_argument('-p', '--path', help='The library folder you want to upload.')
...
def __validate_header_only_lib_args(args) -> tuple[bool, str]:
    if args.path or args.platform or args.arch or args.build or args.source:
...
def __validate_source_lib_args(args) -> tuple[bool, str]:
    if args.path or args.platform or args.arch or args.build:
        return False, 'Too many arguments. Only need -n -v --source for source.'
    
    return True, ''

def __validate_binary_lib_args(args) -> tuple[bool, str]:
    if not args.platform or not args.arch or not args.build or not args.header:
        return False, 'Too few arguments. Need -s, -a, -b, -h for prebuilt library.'
    
    return True, ''

def eval_lib_type(args) -> LibType:
    if not args.path and not args.header and not args.source:
...
def wait_user_input(message: str, valid_inputs: tuple[str]) -> str:
    ...
    while True:
        result = input(f'{message}, input {options}: ')
...
def execute_add(args):
    ...
    if exist is None:
        ...
        update_lib(args.path, used_header_path, used_source_path, lib_data)
    else:
