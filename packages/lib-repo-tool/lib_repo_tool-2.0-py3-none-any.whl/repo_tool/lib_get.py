...
from .common import get_bucket, zip_dir, unzip_file, upload_file, download_file, OSS_BASE_PATH, get_platform_name, available_build_names, available_arches
...
def valid_get_args(args):
    ordered_args = ['platform', 'arch', 'build']
...
def lib_get():
    ...
    for name, ver in deps.items():
        ...
        if potential_libs[0].lib_type in (LibType.HEADER_ONLY.value, LibType.SOURCE.value):
            ...
            found_libs = repo.query_libs(name=name, version=ver, platform=platform, arch=arch, build=build)
            if not found_libs:
    ...
    for lib in downloading_libs:
        logger.info(f'---------------Begin download library {lib.name}, version: {lib.version}, platform: {lib.platform}, arch: {lib.arch}, config: {lib.build}----------------')
        download_lib(lib, libs_dest_dir)
        logger.info(f'---------------End download library {lib.name}, version: {lib.version}, platform: {lib.platform}, arch: {lib.arch}, config: {lib.build}----------------')
