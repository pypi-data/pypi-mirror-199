...
class Logger:
	...
	# 	"""
	# 	log = ""
	# 	log += f"\033[1m" if bold else ""
	# 	log += f"\033[3m" if italic else ""
	# 	log += f"\033[35m*{datetime.datetime.now()}\t" if self.time else ""
	# 	log += f"\033[35m${platform.node()}^{os.getlogin()}\t" if self.name else ""
	# 	log += f"\033[33m#STATUS: " if self.status else ""
	# 	log += f"\033[33m{status_message_text}\t" if self.status_message else ""
	# 	log += f"\033[38;5;242m@DEBUG - " if self.status_type else ""
	# 	log += f"\033[38;5;238m{message_text}" if self.message else ""
	# 	log += f"\033[0m"
	# 	return log
