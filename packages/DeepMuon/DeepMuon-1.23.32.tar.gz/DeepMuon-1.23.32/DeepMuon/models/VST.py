...
def window_partition(x, window_size):
    ...
    """
    B_s, D_s, H_s, W_s, C_s = x.shape
    x = x.view(B_s, D_s // window_size[0], window_size[0], H_s // window_size[1],
               window_size[1], W_s // window_size[2], window_size[2], C_s)
    windows = x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous(
    ).view(-1, reduce(mul, window_size), C_s)
    return windows
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask=None):
        ...
        """
        B_x, N_x, C_x = x.shape
        qkv = self.qkv(x).reshape(B_x, N_x, 3, self.num_heads, C_x //
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index[:N_x, :N_x].reshape(-1)].reshape(
            N_x, N_x, -1)  # Wd*Wh*Ww,Wd*Wh*Ww,nH
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_x // nW, nW, self.num_heads, N_x,
                             N_x) + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N_x, N_x)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward_part1(self, x: torch.Tensor, mask_matrix):
        B_x, D_x, H_x, W_x, C_x = x.shape
        window_size, shift_size = get_window_size(
            (D_x, H_x, W_x), self.window_size, self.shift_size)

        x = self.norm1(x)
        # pad feature maps to multiples of window size
        pad_l = pad_t = pad_d0 = 0
        pad_d1 = (window_size[0] - D_x % window_size[0]) % window_size[0]
        pad_b = (window_size[1] - H_x % window_size[1]) % window_size[1]
        pad_r = (window_size[2] - W_x % window_size[2]) % window_size[2]
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, *(window_size+(C_x,)))
        shifted_x = window_reverse(
            attn_windows, window_size, B_x, Dp, Hp, Wp)  # B D' H' W' C
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        # padding
        pad_input = (H_x % 2 == 1) or (W_x % 2 == 1)
        if pad_input:
            x = F.pad(x, (0, 0, 0, W_x % 2, 0, H_x % 2))
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        B_x, C_x, D_x, H_x, W_x = x.shape
#        shortcut = x
        window_size, shift_size = get_window_size(
            (D_x, H_x, W_x), self.window_size, self.shift_size)
        x = torch.permute(x, (0, 2, 3, 4, 1))
        Dp = int(np.ceil(D_x / window_size[0])) * window_size[0]
        Hp = int(np.ceil(H_x / window_size[1])) * window_size[1]
        Wp = int(np.ceil(W_x / window_size[2])) * window_size[2]
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, x.device)
        for blk in self.blocks:
            x = blk(x, attn_mask)
        x = x.view(B_x, D_x, H_x, W_x, -1)
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if W_x % self.patch_size[2] != 0:
            x = F.pad(x, (0, self.patch_size[2] - W_x % self.patch_size[2]))
        if H_x % self.patch_size[1] != 0:
            x = F.pad(
                x, (0, 0, 0, self.patch_size[1] - H_x % self.patch_size[1]))
        if D_x % self.patch_size[0] != 0:
            x = F.pad(
                x, (0, 0, 0, 0, 0, self.patch_size[0] - D_x % self.patch_size[0]))
        ...
        if self.norm is not None:
            ...
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.embed_dim, D_x, Wh, Ww)
