...
def _run_simulation_common(
    model, linearization, parameters, wait, no_sync, ignore_cache, snapshot_uuid, recorded_signals: list = None
) -> Simulation:
    ...
    for name_path, hashed_file in hashed_files.items():
        block_override = {
            "path": name_path,
            "parameters": {
                "file_name": {"value": f'__hashed_file("{hashed_file.hash}", "{hashed_file.content_type}")'}
            },
        }
...
def run_simulation(
    ...
    """
    return _run_simulation_common(model, None, parameters, wait, no_sync, ignore_cache, snapshot_uuid, recorded_signals)
