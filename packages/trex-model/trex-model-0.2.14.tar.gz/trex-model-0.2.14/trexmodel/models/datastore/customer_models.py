...
class CustomerMembership(BaseNModel, DictModel):
    ...
    joined_datetime             = ndb.DateTimeProperty(required=True, auto_now_add=True)
    joined_date                 = ndb.DateProperty(required=True, auto_now_add=True)
    ...
    dict_properties     =[
                            'customer', 'merchant_membership_entity', 'joined_date', 'expiry_date'
                            ]
    ...
    def is_new_joined(self, purchased_datetime=None):
        ...
        time_difference = self.purchased_datetime -  self.joined_datetime
    ...
    @staticmethod
    def list_merchant_membership_by_joined_date(merchant_memberships_list, joined_date, limit=conf.MAX_FETCH_RECORD, start_cursor=None, return_with_cursor=False):
        ...
        query = CustomerMembership.query(ndb.AND(
                                        ...
                                        CustomerMembership.joined_date==joined_date,
                                        ),
                                    )
    ...
    @staticmethod
    def list_active_merchant_membership_by_joined_date(merchant_memberships_list, joined_date, limit=conf.MAX_FETCH_RECORD, start_cursor=None, return_with_cursor=False):
        ...
        if return_with_cursor:
            (search_results, next_cursor) = CustomerMembership.list_merchant_membership_by_joined_date(merchant_memberships_list, joined_date, limit=limit, start_cursor=start_cursor, return_with_cursor=return_with_cursor)
            
        else:
            search_results = CustomerMembership.list_merchant_membership_by_joined_date(merchant_memberships_list, joined_date, limit=limit, start_cursor=start_cursor);
    ...
    @staticmethod
    def create(customer, merchant_membership, joined_datetime=None, assigned_outlet=None, assigned_by=None):
        if joined_datetime is None:
            joined_datetime = datetime.utcnow()
            
        expiry_date = merchant_membership.calc_expiry_date(start_date=joined_datetime)
