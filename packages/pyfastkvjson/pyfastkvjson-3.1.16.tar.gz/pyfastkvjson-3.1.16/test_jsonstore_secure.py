...
class Tests(unittest.TestCase):
    ...
    TEST_PASSWORD = "fooTestIzDaBest"
    BUFFER_SIZE = 64 * 1024

    def setUp(self):
        self._store_file = mktemp() + ".aes"
        self.store = JsonStore(self._store_file, indent=None, auto_commit=True, password=self.TEST_PASSWORD)
    ...
    def test_new_store(self):
        ...
        decryptedTemp = store_file + "~2"
        pyAesCrypt.decryptFile(self._store_file, decryptedTemp, self.TEST_PASSWORD, self.BUFFER_SIZE)
        with open(decryptedTemp) as handle:
    ...
    def test_load(self):
        for good_data in ("{}", '{"key": "value"}'):
            data = good_data.encode("utf-8")
            if self._store_file.endswith(".aes"):
                tempStoreFile = self._store_file + "_TEST"
                with open(tempStoreFile, "wb") as temp:
                    temp.write(data)
                pyAesCrypt.encryptFile(tempStoreFile, self._store_file, self.TEST_PASSWORD, self.BUFFER_SIZE)
                os.remove(tempStoreFile)
            else:
                with open(self._store_file, "wb") as handle:
                    handle.write(data)
            self.store._load()
    ...
    def test_transaction_write(self):
        with self.store:
            ...
            pyAesCrypt.decryptFile(self._store_file, decryptedFile, self.TEST_PASSWORD, self.BUFFER_SIZE)
            with open(decryptedFile) as handle:
                self.assertEqual(handle.read(), "{}")
