...
def main(pdf: Path, output: OutputOptions) -> None:
    ...
    if reader.is_encrypted:
        pdf_stat = pdf.stat()
        meta = MetaInfo(
            encryption=EncryptionData(algV=reader._encryption.algV, algR=reader._encryption.algR) if reader.is_encrypted else None,
    ...
    if output == OutputOptions.json:
        ...
        if meta.encryption:
            enc_table.add_row("algR", str(meta.encryption.algR))
            enc_table.add_row("algV", str(meta.encryption.algV))
