...
class WButton():
    ...
    def SelfStyesManipulate(self):
        self.B2_position = [None,None]
        if self.Placed: 
            
            self.B2_position[0]=self.B1_position[0] + self.Parent_position[0]
            self.B2_position[1]=self.B1_position[1] + self.Parent_position[1]
        else:
            self.B2_position = self.B1_position
            
        self.selfstyle.stylebuf['position'] = self.B2_position
        
    def Render(self):
        self.ParentStylesUpdate()
        self.SelfStyesManipulate()
        
        self.rect = pygame.Rect(self.B2_position,[self.B1_width,self.B1_height])
        
        if self.rect.collidepoint(*GL.om.pos):
            
         
            if GL.om.GET_PRESS_ON_PYGL_WINDOW():
                self.win.GL.Rect(self.B1_press_color,self.B2_position,[self.B1_width,self.B1_height],'s',0,'F',R=self.B1_press_rougres)
            else:
                self.win.GL.Rect(self.B1_release_color,self.B2_position,[self.B1_width,self.B1_height],'s',0,'F',R=self.B1_release_rougres)
        else:
            self.win.GL.Rect(self.B1_color,self.B2_position,[self.B1_width,self.B1_height],'s',0,'F',R=self.B1_rougres)
...
class WText():
    ...
    def Update(self):
        ...
        if self.T1_outline:
            ...
            self.T1_surface = self.T2_text_surface
