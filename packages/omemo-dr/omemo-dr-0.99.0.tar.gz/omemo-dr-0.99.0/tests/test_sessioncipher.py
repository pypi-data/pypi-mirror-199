...
from omemo_dr.ratchet.aliceaxolotlparameters import AliceAxolotlParameters
...
class SessionCipherTest(unittest.TestCase):
    def test_basicSessionV3(self):
        ...
        self.initializeSessionsV3(
            aliceSessionRecord.getSessionState(), bobSessionRecord.getSessionState()
        )
    ...
    def runInteraction(self, aliceSessionRecord, bobSessionRecord):
        ...
        aliceCipher = SessionCipher(
            aliceStore, aliceStore, aliceStore, aliceStore, 2, 1
        )
        ...
        for i in range(0, 50):
            ...
            aliceCiphertextMessages.append(
                aliceCipher.encrypt("смерть за смерть %s" % i)
            )
        ...
        for i in range(0, int(len(aliceCiphertextMessages) / 2)):
            receivedPlaintext = bobCipher.decryptMsg(
                WhisperMessage.from_bytes(aliceCiphertextMessages[i].serialize())
            )
            self.assertEqual(receivedPlaintext.decode(), alicePlaintextMessages[i])
    ...
    def initializeSessionsV2(self, aliceSessionState, bobSessionState):
        ...
        aliceIdentityKey = IdentityKeyPair.new(
            IdentityKey(aliceIdentityKeyPair.getPublicKey()),
        ...
        bobIdentityKey = IdentityKeyPair.new(
            IdentityKey(bobIdentityKeyPair.getPublicKey()),
        ...
        RatchetingSession.initializeSessionAsAlice(
            aliceSessionState, 2, aliceParameters
        )
    ...
    def initializeSessionsV3(self, aliceSessionState, bobSessionState):
        ...
        RatchetingSession.initializeSessionAsAlice(
            aliceSessionState, 3, aliceParameters
        )
