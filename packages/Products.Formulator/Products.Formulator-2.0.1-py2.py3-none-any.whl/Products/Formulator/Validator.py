...
class PatternValidator(StringValidator):
    ...
    pattern = fields.StringField(
        'pattern', title="Pattern", required=1, default="", description=(
            "The pattern the value should conform to. Patterns are "
            "composed of digits ('d'), alphabetic characters ('e') and "
            "alphanumeric characters ('f'). Any other character in the "
            "pattern should appear literally in the value in that place. "
            "Internal whitespace is checked as well but may be included in "
            "any amount. Example: 'dddd ee' is a Dutch zipcode (postcode). "))
...
class LinesValidator(StringBaseValidator):
    ...
    max_linelength = fields.IntegerField(
        'max_linelength', title="Maximum length of line", description=(
            "The maximum length of a line. If set to 0 or is left empty,"
            " there is no maximum."), default="", required=0)
    max_length = fields.IntegerField(
        'max_length',
...
class FileValidator(Validator):
    ...
    required_not_found = _('Input is required but no input given.')
    incorrect_enctype = _(
        'Form enctype appears to be either unset or set to'
        ' application/x-www-form-urlencoded.  For FileUpload types this needs'
        ' to be set to "multipart/form-data"')
...
class DateTimeValidator(Validator):
    ...
    required = fields.CheckBoxField(
        'required', title='Required', description=(
            "Checked if the field is required; the user has to enter"
            " something in the field."), default=1)
    ...
    def validate(self, field, key, REQUEST):
        ...
        if field.get_value('ampm_time_style'):
            ...
            # handling hour > 12
            if (not (ampm == 'am' or ampm == 'AM') and not (
                    ampm == 'pm' or ampm == 'PM')) or (hour > 12):
                self.raise_error('not_datetime', field)
