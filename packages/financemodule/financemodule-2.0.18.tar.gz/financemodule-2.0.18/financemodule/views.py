...
class FinanceModuleHandler:
    ...
    def gets_entity(self,currency):
        return Entity.objects.filter(rate_currency__description = currency).exists()
    ...
    def currency_conversion(self , from_currency , from_amount , to_currency , from_rate = None, to_rate = None ,exch_rate = None) -> None:
        ...
        if from_currency and to_currency != bool(self.gets_entity(from_currency)):
            if exch_rate:
                to_amount = from_amount * exch_rate
            to_amount = (from_amount * from_rate ) / to_rate
            exch_rate = to_amount / from_amount
        elif from_currency and to_currency ==  bool(self.gets_entity(from_currency)):
            to_amount = from_amount 
            exch_rate = 1
        elif from_currency ==  bool(self.gets_entity(from_currency)):
            if not exch_rate:
                exch_rate = to_rate
            to_amount = from_amount / exch_rate
        else:
