...
def acsf_creator(n_samples, rcut_factor, data_file):
    file = open(data_file)
    lines = file.readlines()
    file.close()
            ...
            for at2 in range(len(atom_types)):
                if at1>=at2:
                    ...
                    for p in range(positions_shifted_0.shape[0]):
                        ...
                        idx1 = np.where(r01_<rmax/1.8897259886)[0]
                        idx2 = np.where(r02_<rmax/1.8897259886)[0]
                        ...
                        r12_ = np.reshape(r12_, r12_.shape[0]*r12_.shape[1])
                        r01 = np.append(r01, r01_)
                        r02 = np.append(r02, r02_)
                        r12 = np.append(r12, r12_)
                    r = np.mean(r01)**2 + np.mean(r02)**2 + np.mean(r12)**2
                    etha = np.log(2)/r
