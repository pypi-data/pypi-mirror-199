...
from .immunx_graphql import AlertsResponse
...
def get_immunx_config():
    config = {}
    # try to read global config
    global_config_path = os.path.join(
        os.path.expanduser('~'), '.immunx', 'immunx.config.json')
    ...
    local_config_path = os.path.join(
        os.getcwd(), local_config_file if local_config_file else 'immunx.config.json')
...
def get_alerts(dict):
    from .immunx_graphql import AlertQueryOptions
    immunx_api = "https://api.immunx.network/graphql"
    headers = {"content-type": "application/json"}
    query_options = AlertQueryOptions(dict)
    payload = query_options.get_query()

    response = requests.request(
        "POST", immunx_api, json=payload, headers=headers)
