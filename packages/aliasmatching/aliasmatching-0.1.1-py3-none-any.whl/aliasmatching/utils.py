...
def sim_users(u1, u2,
    ...
    if u1['name'] is np.nan or u2['name'] is np.nan:
        ...
        email_name_score *= email_name_coef
    ...
    login_email_score *= login_email_coef
    ...
    login_name_score *= login_name_coef
...
def get_clusters(users,
    ...
    if 'full_id' not in users:
        users['full_id'] = users.apply(lambda x: f"{x['name']}:{x['email']}:{x['login']}")
