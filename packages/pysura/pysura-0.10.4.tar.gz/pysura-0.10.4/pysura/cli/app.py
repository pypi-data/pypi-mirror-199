...
class GoogleRoot(RootCmd):
    ...
    def gcloud_list_choice(command_str, model):
        ...
        try:
            ...
            env[env_names["list"]] = gcloud_list
    ...
    def do_gcloud_create_project(_):
        ...
        if GoogleRoot.confirm_loop(project_name):
            ...
            if use_org:
                ...
                cmd_str += f" --organization={env.organization.name}"
            PysuraStd.log(cmd_str, level=logging.DEBUG)
            os.system(cmd_str)
        else:
    ...
    def do_gcloud_create_vpc_peering(_):
        ...
        if GoogleRoot.confirm_loop(peering_name):
            ...
            PysuraStd.log(cmd_str, level=logging.DEBUG)
            os.system(cmd_str)
            cmd_str = f"gcloud services vpc-peerings list --network={env.network.name} --format=json"
            PysuraStd.log(cmd_str, level=logging.DEBUG)
            response = os.popen(cmd_str).read()
            gcloud_list = json.loads(response)
    ...
    def do_gcloud_choose_vpc_peering(_):
        ...
        peering, peerings = GoogleRoot.gcloud_list_choice(
            f"gcloud services vpc-peerings list --network={env.network.name} --format=json", GooglePeering
        )
        env.peering = peering
        GoogleRoot.set_env(env)
    ...
    def do_gcloud_create_database(_):
        ...
        PysuraStd.log(f"You are preparing to create a database with the following parameters: "
                      ...
                      f"Network: {env.network.name}, Project: {env.project.name}, "
        ...
        cmd_str = (
            ...
            f"--project={env.project.name} "
            f"--network=projects/{env.project.name}/global/networks/{env.network.name} "
    ...
    def do_gcloud_create_serverless_connector(_):
        ...
        for connector in gcloud_list:
            ...
            if connector_data.name == connector_name:
    ...
    def do_gcloud_choose_serverless_connector(_):
        ...
        for connector in gcloud_list:
            ...
            connector_set.append(connector_data)
            if connector_data.name == env.connector.name:
                connector_selected = connector_data
    ...
    def do_gcloud_deploy_hasura(_):
        ...
        if env.hasura is None:
            ...
            if account_id is None:
                ...
                return
            cmd_log_str = (f"gcloud projects add-iam-policy-binding {env.project.name} "
            ...
            PysuraStd.log(cmd_log_str, level=logging.DEBUG)
            os.system(cmd_log_str)
            cmd_log_str = f"docker tag hasura/graphql-engine:latest gcr.io/{env.project.name}/hasura:latest"
            PysuraStd.log(cmd_log_str, level=logging.DEBUG)
            os.system(cmd_log_str)
            cmd_log_str = f"docker push gcr.io/{env.project.name}/hasura:latest"
            PysuraStd.log(cmd_log_str, level=logging.DEBUG)
            os.system(cmd_log_str)
            ...
            hasura = Hasura(
                ...
                vpc_connector=env.connector.name,
                timeout=timeout,
                project_id=env.project.name,
                cpu=cpu,
                memory=memory,
...
""")
        deploy_command = (f"gcloud run deploy hasura "
                          f"--image=gcr.io/{env.project.name}/hasura:latest "
                          f"--env-vars-file=env.yaml "
                          f"--min-instances=1 "
                          f"--max-instances=10 "
                          f"--cpu=1 "
                          f"--memory=2048Mi "
                          f"--vpc-connector={env.connector.name} "
                          f"--port=8080 "
