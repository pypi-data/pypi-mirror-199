...
class GoogleRoot(RootCmd):
    ...
    def gcloud_list_typed_choice(self, command_str, prompt_str, default="name"):
        ...
        if len(gcloud_list) > 0:
            log_str = ""
            for k in gcloud_list[0].keys():
                log_str += f"{k:<30}"
            for i in gcloud_list:
                log_str += f"{i.get('name', 'unknown')}\t"
                for k, v in i.items():
                    log_str += json.dumps(v)
    ...
    def gcloud_list_choice(self, command_str, model):
        ...
        try:
            ...
            choice = self.gcloud_list_typed_choice(command_str,
            ...
            self.log("No items found. Falling back to index.")
        except IndexError:
            self.log("No items found.  Falling back to index.")
        except TypeError:
            self.log("No items found.  Falling back to index.")
