...
async def read_file(blobpath: str) -> bytes:
...
async def read_file_cached(blobpath: str) -> bytes:
    ...
    if cache_dir == "":
        # disable caching
        return await read_file(blobpath)
...
async def data_gym_to_mergeable_bpe_ranks(
    ...
    vocab_bpe_contents = (await read_file_cached(vocab_bpe_file)).decode()
    bpe_merges = [
    ...
    # as merge priority
    encoder_json = json.loads(await read_file_cached(encoder_json_file))
...
async def load_tiktoken_bpe(tiktoken_bpe_file: str) -> dict[bytes, int]:
    # NB: do not add caching to this function
