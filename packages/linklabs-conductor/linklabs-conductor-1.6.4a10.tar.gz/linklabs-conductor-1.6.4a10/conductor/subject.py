...
class ConductorSubject(object):
    ...
    def _validate_http_rsp(self, resp):
        ...
        try:
            ...
            try:
                json = resp.json()
                msg = "HTTP {} | {}: {}".format(resp.status_code, json.get("description"), json.get("message"))
            except Exception:
                txt = resp.text
...
class UplinkMessage(ConductorSubject):
    ...
    def __repr__(self):
        return "{}({}), {}".format(self.__class__.__name__, self, self.signal_data)
...
class UplinkSubject(ConductorSubject):
    ...
    def _build_msg(self, *args):
        ...
        And update any equivalent metadata in the object."""
        if len(args) == 4:
    ...
    def get_messages_time_range(self, start, stop=None):
        ...
        while more_results:
            ...
            messages.extend(
                [self._build_msg(self.session, m.get("uuid"), self.instance, m) for m in resp_data["results"]]
            )
    ...
    def get_last_messages(self, count=1):
        ...
        """
        base_url = "{}/data/{}/{}/{}/mostRecentEvents?maxResults={}".format(
            self.client_edge_url, self.uplink_type, self.subject_name, self.subject_id, count
        )
        messages = [
            self._build_msg(self.session, m.get("uuid"), self.instance, m) for m in self._get(base_url)["results"]
        ]
...
class DownlinkMessage(ConductorSubject):
    ...
    def get_events(self, route=None):
        ...
        route_urls = [
            "{}/issuedCommand/{}/statusDetail/{}".format(self.client_edge_url, self.issuance_id, rte) for rte in routes
        ]
        ...
        for url in route_urls:
            ...
            events = [(event["state"], parse_time(event["stateTime"])) for event in resp_json["downlinkEvents"]]
...
class DownlinkSubject(ConductorSubject):
    ...
    def query_downlink(self, start, stop=None):
        ...
        stop = stop or datetime.utcnow()
        url = "{}/commands/{}/{}/{}/{}".format(
            self.client_edge_url, self.subject_name, self.subject_id, format_time(stop), format_time(start)
        )
        return [
            DownlinkMessage(self.session, result["issuanceId"], self.instance, _data=result)
