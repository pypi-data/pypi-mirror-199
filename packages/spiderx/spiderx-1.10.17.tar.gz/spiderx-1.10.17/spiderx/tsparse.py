...
class TSParser():
    ...
    def __init__(self, data:bytes, key:bytes,decode_func=None):
        self.stream = TSStream(data, key)
        if decode_func:
            self.decrypt_func=decode_func
    def decrypt_func(self, content:bytes, key:bytes): #ECB
    ...
    def decrypt_pes(self, byte_buf:bytes, pes_fragments, key:bytes):
        for pes in pes_fragments:
            ...
            if length % 16 > 0:
                ...
                decrypted = self.decrypt_func(decrypted_byte_string[:new_length], key)
                decrypted_byte_string = decrypted + decrypted_byte_string[new_length:]
            else:
                decrypted = self.decrypt_func(decrypted_byte_string, key)
