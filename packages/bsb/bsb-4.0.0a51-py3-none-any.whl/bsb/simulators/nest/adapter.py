...
class NestResult(SimulationResult):
    def record_nodes(self, nc, **annotations):
        import nest
        ...
        def flush(segment):
            ...
            spikes = {node: [] for node in nc.tolist()}
            for event, sender in zip(events["times"], events["senders"]):
                spikes[sender].append(event)
            for node, node_spikes in spikes.items():
...
class NestAdapter(SimulatorAdapter):
    ...
    def prepare(self, simulation, comm=None):
        ...
        try:
            simdata.result = NestResult(simulation)
    ...
    def create_neurons(self, simulation):
        ...
        for cell_model in simulation.cell_models.values():
            simdata.populations[cell_model] = pop = cell_model.create_population()
            pop.__dict__["model"] = cell_model
    ...
    def connect_neurons(self, simulation):
        ...
        for connection_model in iter:
            try:
                ...
                pass
            cs, pre_nodes, post_nodes = self._determine_nodes(
                simulation, connection_model
            )
            try:
                simdata.connections[
                    connection_model
    ...
    def create_devices(self, simulation):
        simdata = self.simdata[simulation]
        for model, nc in simdata.populations.items():
            simdata.result.record_nodes(nc, population=model.name)
    ...
    def _determine_nodes(self, simulation, connection_model):
        simdata = self.simdata[simulation]
        try:
            ...
            cs = None
            pre_nodes = simdata.populations[connection_model.pre_model]
            post_nodes = simdata.populations[connection_model.post_model]
        else:
            try:
        ...
        return cs, pre_nodes, post_nodes