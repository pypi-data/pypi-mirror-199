...
from .Tools.interp import interp
from .Tools.finite_differences import fd_d1_o4
from .Tools.Gaussian_fit import gaussian_fit
from .Tools.Gaussian_fit import gaussian_fit_GUI
from .Tools.max_pedestal_finder import find_pedestal_from_data
from .Tools.max_pedestal_finder import find_pedestal
from .Tools.read_profiles import read_profile_file
from .Tools.read_profiles import read_geom_file
from .Tools.read_EFIT_file import get_geom_pars
from .Tools.write_iterdb import output_iterdb
from .Tools.max_stat_tool import Poly_fit
from .Tools.max_stat_tool import coeff_to_Poly
from .Tools.DispersionRelationDeterminantFullConductivityZeff import VectorFinder_auto
from .Tools.DispersionRelationDeterminantFullConductivityZeff import VectorFinder_manual
...
class mode_finder:
    ...
    def q_scale_for_alignment_n_mode(self,n):
        x=self.x
        q=self.q
        ome=self.ome 

        index_tmp_=np.argmax(ome)
        q_norm_peak=q[index_tmp_]

        m_list=np.arange(1,300,dtype=float)
        q_list=m_list/n
        q_diff=abs(q_list-q_norm_peak)
        q_list=q_diff

        order_index=np.argsort(q_list)

        q_list_sort=q_list[order_index]

        q_scale_list=q_list_sort/q_norm_peak
