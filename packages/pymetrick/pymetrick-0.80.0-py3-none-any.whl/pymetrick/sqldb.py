        ...
        try:
            ...
            if sql:
                ...
                if parameters is not None:
                    ...
                    sql_list = sql.split(';')
                    _sql = [ x for x in sql.split(';') if (x[:6].upper().strip() in ('SELECT','INSERT','UPDATE','DELETE') or x[:4].upper().strip() in ('CALL')) and len(x.strip())>0]
                    if len(sql.split(';'))-1 == len(_sql) and len(_sql)>1:
                        sql = ';'.join(_sql)+';'  
                        request = []
