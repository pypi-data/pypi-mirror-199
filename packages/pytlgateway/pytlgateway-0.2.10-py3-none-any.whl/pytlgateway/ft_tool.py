                ...
                for file in csv_path.glob("*.csv"):
                    if file.stem.startswith(path_head):
                        ...
                        with open(file, encoding='utf-8-sig')as f:
                            ...
                            for row in reader:
                                row = Row(*row)
                                if str(row.报单状态) == "全部成交" or str(row.报单状态) == "部分撤单":
                                    traded_vol = int(row.成交数量)
                ...
                if equity_positions.count() == 0:
                    ...
                    for position in equity_positions:
                        ...
                        if sid.startswith('[ADD]'):
                            ...
                            else: #找不到sync_trade 直接将add持仓分到所有持仓上
                                ...
                                if actual_yd_pos_long > 0:
                                    ...
                                    if ticker_positions.count() > 0:
                                        for ticker_position in ticker_positions:
                                            pos = ticker_position['actual_yd_pos_long'] + actual_yd_pos_long
                                            ...
                                            if pos > 0 :
                                                ...
                                                query_for_delete = {'_id': _id}
                                                res = equity_position_collection.delete_one(query_for_delete)
                                                self.logger.info(f'[sync_position_add] delete (res){res} ')
                                                #yd_pos_long = pos
...
if __name__ == "__main__":
    ...
    td = FtTool(args.config_filename, args.update_trade_date, args.sync_pos, args.sync_trade, args.sync_add, args.sync_trade_in_trading)
