...
def instagram(ctx, inputfile, outputfile, dpi, transparent, ranges, pages):
    ctx.invoke(topng, inputfile=inputfile, ranges=ranges, transparent=transparent)
    for i in range(ranges[0], ranges[1]+1):
        ctx.invoke(blur, inputimage=f'main-{i}.png', outputimage=f'main-{i}b.png', opacity=0.35, radius=2)

    for i in range(ranges[0], ranges[1]+1):
        ctx.stack(image=f'main-{i}.png', background=f'main-{i}b.png', outputfile=f'main-{i}bs.png', position=(-2, -2))

    for i in range(ranges[0], ranges[1]+1):
        ctx.addbg(image=f'main-{i}bs.png', outputfile=f'main-{i}bsb.png')
